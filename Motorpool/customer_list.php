<?php
// customer_list.php - Customer Management (Motorpool - No Map/Location)
...
if ($check_table && $check_table->num_rows == 0) {
    $create_table = "CREATE TABLE IF NOT EXISTS customers (
        ...
        price_level VARCHAR(50) DEFAULT 'Walk In',
        latitude VARCHAR(50),
...
function customerListGetEffectiveCustomerCreditLimit($conn, $customer_id) {
    ...
    $sql = "SELECT credit_limit FROM customers WHERE customer_id = ? AND created_by IN (SELECT user_id FROM users WHERE role = 'motorpool') LIMIT 1";
    $stmt = $conn->prepare($sql);
    if ($stmt) {
        $stmt->bind_param("i", $customer_id);
        $stmt->execute();
...
if (!function_exists('mergedEnsureCustomerGroupMaster')) {
    function mergedEnsureCustomerGroupMaster($conn, $group_name, $branch_id, $user_id) {
        ...
        $stmt = $conn->prepare("SELECT group_id FROM customer_groups_master WHERE LOWER(TRIM(group_name)) = LOWER(TRIM(?)) AND (branch_id = ? ) LIMIT 1");
        if ($stmt) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && in_array($_POST['action'], $merged_customer_save_actions, true)) {
    ...
    try {
        ...
        $price_level = trim($_POST['price_level'] ?? 'Walk In');
        ...
        if ($action === 'add_customer') {
            ...
            $dup = $conn->prepare("SELECT customer_id FROM customers WHERE customer_code = ? AND created_by IN (SELECT user_id FROM users WHERE role = 'motorpool') LIMIT 1");
            if ($dup) {
        ...
        if ($existing_image === '' && mergedCustomerColumnExists($conn, 'store_image')) {
            $img_stmt = $conn->prepare("SELECT store_image FROM customers WHERE customer_id = ? AND created_by IN (SELECT user_id FROM users WHERE role = 'motorpool') LIMIT 1");
            if ($img_stmt) {
...
// Determine data scope for Motorpool credit requests.
// IMPORTANT: This is MERGED, not mirrored. The Approve Credit Request tab now reads
// the same shared credit_discount_requests table, but only for customers created by Motorpool users.
$branch_condition = "AND c.created_by IN (SELECT user_id FROM users WHERE role = 'motorpool')";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && in_array($_POST['action'], $credit_ajax_actions, true)) {
    ...
    try {
        ...
        if ($_POST['action'] === 'approve_request') {
            ...
            // Verify Motorpool data access only. Do not use branch mirror here.
            $scope_stmt = $conn->prepare("SELECT c.customer_id FROM customers c WHERE c.customer_id = ? AND c.created_by IN (SELECT user_id FROM users WHERE role = 'motorpool') LIMIT 1");
            if ($scope_stmt) {
                $scope_stmt->bind_param("i", $request_data['customer_id']);
                $scope_stmt->execute();
                $scope_res = $scope_stmt->get_result();
                if (!$scope_res || $scope_res->num_rows === 0) {
                    throw new Exception("Access denied: This request is not a Motorpool customer request");
                }
                $scope_stmt->close();
            }
        ...
        elseif ($_POST['action'] === 'view_request') {
            ...
            // Motorpool scope only; no branch mirroring.
            $query .= " AND c.created_by IN (SELECT user_id FROM users WHERE role = 'motorpool')";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'update_customer_fast') {
    ...
    try {
        ...
        if (isset($_FILES['store_image']) && $_FILES['store_image']['error'] === UPLOAD_ERR_OK) {
            ...
            $old_img_stmt = $conn->prepare("SELECT store_image FROM customers WHERE customer_id = ? AND created_by IN (SELECT user_id FROM users WHERE role = 'motorpool') LIMIT 1");
            if ($old_img_stmt) {
...
// Check if Motorpool Walk-in customer exists, if not create it.
// Motorpool customer_list must always have the default Walk-in customer even when branch_id is 0.
function ensureWalkinCustomer($conn, $branch_id, $user_id) {
    $created_by_filter = "created_by IN (SELECT user_id FROM users WHERE role = 'motorpool')";

    $check_walkin = $conn->query("SELECT customer_id FROM customers WHERE customer_code = 'WALKIN-001' AND {$created_by_filter} LIMIT 1");
    if ($check_walkin && $check_walkin->num_rows > 0) {
        // Make sure old Walk-in record remains active and uses Walk In price level.
        $row = $check_walkin->fetch_assoc();
        $walkin_id = (int)($row['customer_id'] ?? 0);
        if ($walkin_id > 0) {
            @$conn->query("UPDATE customers SET status = 'active', price_level = 'Walk In' WHERE customer_id = {$walkin_id}");
        }
        return;
    }

    $target_branch_id = ((int)$branch_id > 0) ? (int)$branch_id : null;
    $insert_walkin = $conn->prepare("INSERT INTO customers (customer_code, customer_name, email, phone_number, address, price_level, status, branch_id, created_by)
                                     VALUES ('WALKIN-001', 'Walk-in Customer', 'walkin@example.com', 'N/A', 'Walk-in Customer - No fixed address', 'Walk In', 'active', ?, ?)");
    if ($insert_walkin) {
        $insert_walkin->bind_param("ii", $target_branch_id, $user_id);
    ...
    exit;
        ...
        elseif ($_POST['action'] === 'update_customer_group') {
            ...
            if ($customer_groups_master_table_exists) {
                ...
                if (!$view_all_branches && $customers_branch_column_exists && $branch_id > 0) {
                    $master_scope_sql = " AND (branch_id = ? )";
                    $master_types .= "i";
        ...
        elseif ($_POST['action'] === 'delete_customer') {
            ...
            $check_orders = $conn->prepare("SELECT COUNT(*) as order_count FROM sales_orders WHERE customer_id = ? AND created_by IN (SELECT user_id FROM users WHERE role = 'motorpool')");
        ...
        elseif ($_POST['action'] === 'get_customer') {
            ...
            $stmt = $conn->prepare("SELECT c.*, u.first_name, u.last_name, b.branch_name 
                                   ...
                                   WHERE c.customer_id = ? AND c.created_by IN (SELECT user_id FROM users WHERE role = 'motorpool')");
        ...
        elseif ($_POST['action'] === 'get_customer_orders') {
            ...
            $orders_sql = "SELECT so_id, so_number, si_number, order_date, total_amount, order_status
                          FROM sales_orders 
                          WHERE customer_id = ? AND created_by IN (SELECT user_id FROM users WHERE role = 'motorpool')
...
// FETCH CUSTOMERS
// Motorpool does not mirror BranchAdmin branch filtering. It reads only Motorpool-created customers.
$customers_query = "SELECT c.*, b.branch_name,
                    (SELECT COUNT(*) FROM sales_orders WHERE customer_id = c.customer_id AND created_by IN (SELECT user_id FROM users WHERE role = 'motorpool')) as total_orders,
...
if ($view_all_branches) {
    $groups_query = "SELECT DISTINCT customer_group FROM customers WHERE created_by IN (SELECT user_id FROM users WHERE role = 'motorpool') AND customer_group IS NOT NULL AND customer_group != '' ORDER BY customer_group ASC";
    $groups_result = $conn->query($groups_query);
} else {
    $groups_query = "SELECT DISTINCT customer_group FROM customers WHERE created_by IN (SELECT user_id FROM users WHERE role = 'motorpool') AND customer_group IS NOT NULL AND customer_group != '' AND (branch_id = ? ) ORDER BY customer_group ASC";
...
if ($customer_groups_master_table_exists) {
    if ($view_all_branches || !$customers_branch_column_exists || (int)$branch_id <= 0) {
        ...
        $master_groups_stmt = $conn->prepare("SELECT group_name FROM customer_groups_master WHERE group_name IS NOT NULL AND TRIM(group_name) != '' AND (branch_id = ? ) ORDER BY group_name ASC");
...
if (!empty($customer_groups)) {
    if ($view_all_branches || !$customers_branch_column_exists || (int)$branch_id <= 0) {
        $counts_query = "SELECT TRIM(customer_group) AS customer_group, COUNT(*) AS customer_count
                         FROM customers
                         WHERE created_by IN (SELECT user_id FROM users WHERE role = 'motorpool') AND customer_group IS NOT NULL AND TRIM(customer_group) != ''
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'get_order_details') {
    ...
    try {
        ...
        if ($view_all_branches) {
            $check_sql = "SELECT so_id FROM sales_orders WHERE so_id = ? AND created_by IN (SELECT user_id FROM users WHERE role = 'motorpool')";
            $check_stmt = $conn->prepare($check_sql);
            $check_stmt->bind_param('i', $order_id);
        } else {
            $check_sql = "SELECT so_id FROM sales_orders WHERE so_id = ? AND created_by IN (SELECT user_id FROM users WHERE role = 'motorpool') AND branch_id = ? AND created_by IN (SELECT user_id FROM users WHERE role = 'motorpool')";
            $check_stmt = $conn->prepare($check_sql);
        ...
        $sql = "SELECT 
                ...
                WHERE so.so_id = ? AND so.created_by IN (SELECT user_id FROM users WHERE role = 'motorpool')";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'print_order') {
    ...
    try {
        ...
        $sql = "SELECT 
                ...
                LEFT JOIN drivers d ON pl.driver_id = d.driver_id
                JOIN motorpool_inventory_items i ON soi.item_id = i.item_id
                WHERE soi.so_id = ? AND so.created_by IN (SELECT user_id FROM users WHERE role = 'motorpool')
                ORDER BY soi.so_item_id";
...
$price_levels = ['Walk In', 'Standard', 'Premium', 'Wholesale'];
...
if (!in_array('Walk In', $price_levels, true)) {
    array_unshift($price_levels, 'Walk In');
}
$price_levels = array_values(array_unique(array_filter($price_levels, fn($level) => trim((string)$level) !== '')));
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Customer List - Motorpool</title>
            ...
            <li class="nav-item dropdown-more">
                ...
                <div class="more-dropdown" id="customerMobileMenu">
                    ...
                    <a class="dropdown-item" href="collections.php"><i class="bi bi-cash-stack"></i><span>Receive
                            Payment</span></a>
                    <a class="dropdown-item active" href="customer_list.php"><i class="bi bi-person-badge"></i><span>Customer
                            List</span></a>
                </div>
            </li>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select class="form-select" name="price_level" id="addPriceLevel">
                                    <?php foreach ($price_levels as $level): ?>
                                        <option value="<?php echo htmlspecialchars($level); ?>" <?php echo ($level === 'Walk In') ? 'selected' : ''; ?>><?php echo htmlspecialchars($level); ?></option>
                                    <?php endforeach; ?>
                                </select>
...
<div class="modal fade no-print" id="orderDetailsModal" tabindex="-1">
   ...
   <script>
    ...
    window.keepActiveMobileNav = function() {
        ...
        document.querySelectorAll('.mobile-nav .dropdown-item').forEach(function(item) {
            ...
            if (href === currentPage) {
                ...
                const dropdown = item.closest('.more-dropdown');
                if (dropdown && dropdown.id) {
                    const parentBtn = document.querySelector('.mobile-nav .more-btn[onclick*="' + dropdown.id + '"]');
                    if (parentBtn) {
        ...
        document.querySelectorAll('.mobile-nav > .nav .nav-item > .nav-link:not(.more-btn)').forEach(function(link) {
    ...
    window.toggleMobileDropdown = function(event, dropdownId) {
        ...
        const btn = event ? event.currentTarget : document.querySelector('.mobile-nav .more-btn[onclick*="' + dropdownId + '"]');
...
function escapeHtml(str) {
        ...
        fetch(window.location.href, { method: 'POST', body: formData })
            ...
            .then(data => {
                ...
                if (data.success) {
                    ...
                    setElemValue('editPriceLevel', c.price_level || 'Walk In');
    ...
    function saveEditCustomer() {
        ...
        try {
            ...
            formData.append('price_level', getValue('editPriceLevel', 'Walk In'));
    ...
    function toggleDropdown(event, dropdownId) {
        if (typeof window.toggleMobileDropdown === 'function') {
            return window.toggleMobileDropdown(event, dropdownId);
        }
        if (event) {
            event.preventDefault();
...
function confirmLogout() {
        if (typeof window.closeAllMobileDropdowns === 'function') window.closeAllMobileDropdowns();
