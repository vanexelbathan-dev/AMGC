...
requireLogin();
requireRole(['branch_admin', 'motorpool']);
...
// ===== MOTORPOOL RIS SALES ORDER SOURCE SAFETY =====
// Used by request_handler.php when repair parts are auto-synced as Sales Orders.
amgcAddColumnIfMissing($conn, 'sales_orders', 'fulfillment_type', "fulfillment_type VARCHAR(40) NOT NULL DEFAULT 'pickup' AFTER order_status");
amgcAddColumnIfMissing($conn, 'sales_orders', 'payment_status', "payment_status VARCHAR(40) NOT NULL DEFAULT 'unpaid' AFTER fulfillment_type");
amgcAddColumnIfMissing($conn, 'sales_orders', 'source_module', "source_module VARCHAR(80) DEFAULT NULL AFTER payment_status");
amgcAddColumnIfMissing($conn, 'sales_orders', 'source_ris_id', "source_ris_id INT DEFAULT NULL AFTER source_module");
amgcAddColumnIfMissing($conn, 'sales_orders', 'source_ris_number', "source_ris_number VARCHAR(80) DEFAULT NULL AFTER source_ris_id");
amgcAddColumnIfMissing($conn, 'sales_orders', 'remarks', "remarks TEXT DEFAULT NULL AFTER source_ris_number");
amgcAddColumnIfMissing($conn, 'sales_order_items', 'source_module', "source_module VARCHAR(80) DEFAULT NULL");
amgcAddColumnIfMissing($conn, 'sales_order_items', 'source_ris_id', "source_ris_id INT DEFAULT NULL AFTER source_module");
amgcAddColumnIfMissing($conn, 'sales_order_items', 'source_repair_hash', "source_repair_hash VARCHAR(64) DEFAULT NULL AFTER source_ris_id");
...
function amgcOrderProductGetItemCostForAccounting($conn, $item_id, $unit_type, $branch_id = 0) {
    ...
    // 1) Primary source: motorpool_item_unit_inventory for the exact selected UoM.
    if (amgcOrderProductTableExists($conn, 'motorpool_item_unit_inventory') && amgcOrderProductTableExists($conn, 'unit_types')) {
        $cost_col = amgcOrderProductFirstExistingColumn($conn, 'motorpool_item_unit_inventory', ['unit_cost', 'ave_cost', 'average_cost', 'cost', 'purchase_cost']);
        if ($cost_col !== '') {
            $branch_filter = '';
            if ($branch_id > 0 && amgcColumnExists($conn, 'motorpool_item_unit_inventory', 'branch_id')) {
            ...
            $sql = "SELECT COALESCE(NULLIF(iui.`$cost_col`, 0), 0) AS unit_cost
                    FROM motorpool_item_unit_inventory iui
            ...
            $sql = "SELECT AVG(NULLIF(`$cost_col`, 0)) AS avg_cost FROM motorpool_item_unit_inventory WHERE item_id = ?";
            if ($branch_id > 0 && amgcColumnExists($conn, 'motorpool_item_unit_inventory', 'branch_id')) {
    ...
    if (amgcOrderProductTableExists($conn, 'items')) {
        ...
        if ($cost_col !== '') {
            $sql = "SELECT COALESCE(NULLIF(`$cost_col`,0),0) AS unit_cost FROM motorpool_inventory_items WHERE item_id = ? LIMIT 1";
            $stmt = $conn->prepare($sql);
            if ($stmt) {
                $stmt->bind_param('i', $item_id);
                $stmt->execute();
        ...
        $sql = "
            ...
            FROM (
                ...
                WHERE so.customer_id = ?
                  AND so.created_by IN (SELECT user_id FROM users WHERE role = 'motorpool')
                ...
                WHERE customer_id = ?
                  ...
                  AND customer_id IN (SELECT customer_id FROM customers WHERE created_by IN (SELECT user_id FROM users WHERE role = 'motorpool'))
            ) unpaid_rows";
        $stmt = $conn->prepare($sql);
        if ($stmt) {
...
// Check if branch_id column exists in motorpool_inventory_items table
$items_branch_column_exists = true;

/**
 * Get default UOM info for a Motorpool item.
 */
function getItemDefaultUOMInfo($conn, $item_id, $branch_id = 0, $items_branch_column_exists = false) {
    $stmt = $conn->prepare("SELECT COALESCE(NULLIF(TRIM(unit_type), ''), NULLIF(TRIM(base_unit_type), ''), 'Piece') AS unit_type_name FROM motorpool_inventory_items WHERE item_id = ? AND status = 'active' LIMIT 1");
    if ($stmt) {
        $stmt->bind_param('i', $item_id);
        $stmt->execute();
        $row = $stmt->get_result()->fetch_assoc();
        $stmt->close();
        if ($row) {
            return ['unit_type_name' => $row['unit_type_name'], 'multiplier' => 1, 'unit_type_id' => 0];
        }
    }
    return ['unit_type_name' => 'Piece', 'multiplier' => 1, 'unit_type_id' => 0];
}

// Get Motorpool inventory items only
$items = [];
$items_query = "SELECT i.item_id, i.item_code, i.item_name, i.description, i.category,
                COALESCE(i.stock_in_default_uom, i.stock, i.current_stock, 0) AS stock,
                COALESCE(NULLIF(TRIM(i.unit_type), ''), NULLIF(TRIM(i.base_unit_type), ''), 'Piece') AS unit_type,
                COALESCE(i.unit_price, i.unit_cost, 0) AS unit_price,
                COALESCE(i.unit_price, i.unit_cost, 0) AS price_case,
                COALESCE(i.unit_price, i.unit_cost, 0) AS price_inner_pack,
                COALESCE(i.unit_price, i.unit_cost, 0) AS price_box,
                COALESCE(i.unit_price, i.unit_cost, 0) AS price_carton,
                i.reorder_level, i.status,
                COALESCE(i.item_image, '') AS product_image_url,
                'Motorpool' AS branch_name
...
if ($items_result) {
    ...
    error_log("Motorpool items query error: " . $conn->error);
}
...
// This makes Branch orderproduct stock display accurate because it reads motorpool_item_unit_inventory.
$inventory_data = [];
foreach ($items as $item) {
    ...
    $stock_stmt = $conn->prepare("
        SELECT COALESCE(NULLIF(TRIM(iui.unit_type), ''), 'Piece') AS unit_type_name, 1 AS quantity_smallest_pack, iui.current_inventory
        FROM motorpool_item_unit_inventory iui
        WHERE iui.item_id = ?
        ORDER BY iui.unit_type ASC
    ");
    if ($stock_stmt) {
    ...
    try {
        ...
        if ($customer_id === 0 && !empty($customer_name)) {
            if ($branch_column_exists && !$view_all_branches) {
                $check_sql = "SELECT customer_id FROM customers WHERE customer_name = ? AND branch_id = ? AND status = 'active' AND created_by IN (SELECT user_id FROM users WHERE role = 'motorpool')";
                $check_stmt = $conn->prepare($check_sql);
                $check_stmt->bind_param('si', $customer_name, $branch_id);
            } else {
                $check_sql = "SELECT customer_id FROM customers WHERE customer_name = ? AND status = 'active' AND created_by IN (SELECT user_id FROM users WHERE role = 'motorpool')";
                $check_stmt = $conn->prepare($check_sql);
            ...
            if ($check_result->num_rows > 0) {
                ...
                if ($branch_column_exists && !$view_all_branches) {
                    $sql_new_cust = "INSERT INTO customers (customer_name, customer_code, email, phone_number, address, status, branch_id, created_by) 
                                VALUES (?, ?, ?, ?, ?, 'active', ?, ?)";
                    $stmt_new_cust = $conn->prepare($sql_new_cust);
                    $stmt_new_cust->bind_param('sssssii', $customer_name, $customer_code, $email, $phone, $address, $branch_id, $user_id);
                } else {
                    $sql_new_cust = "INSERT INTO customers (customer_name, customer_code, email, phone_number, address, status, created_by) 
                                VALUES (?, ?, ?, ?, ?, 'active', ?)";
                    $stmt_new_cust = $conn->prepare($sql_new_cust);
                    $stmt_new_cust->bind_param('sssssi', $customer_name, $customer_code, $email, $phone, $address, $user_id);
                }
        ...
        if ($customer_id > 0 && $address !== '') {
            $customer_address_sql = "SELECT address FROM customers WHERE customer_id = ? AND created_by IN (SELECT user_id FROM users WHERE role = 'motorpool')";
        ...
        $check_so_stmt = $conn->prepare("SELECT so_id FROM sales_orders WHERE so_number = ? AND created_by IN (SELECT user_id FROM users WHERE role = 'motorpool') LIMIT 1");
        if (!$check_so_stmt) {
        ...
        if ($document_type === 'SI' && $si_number !== '') {
            $check_si_stmt = $conn->prepare("SELECT so_id FROM sales_orders WHERE si_number = ? AND created_by IN (SELECT user_id FROM users WHERE role = 'motorpool') LIMIT 1");
            if ($check_si_stmt) {
        ...
        foreach ($items_data as $item) {
            ...
            $stock_stmt = $conn->prepare("
                SELECT iui.inventory_id, iui.current_inventory, COALESCE(iui.unit_cost, 0) AS unit_cost, COALESCE(iui.unit_type_id, 0) AS unit_type_id, COALESCE(NULLIF(TRIM(iui.unit_type), ''), ?) AS unit_type_name
                FROM motorpool_item_unit_inventory iui
                WHERE iui.item_id = ? AND LOWER(COALESCE(NULLIF(TRIM(iui.unit_type), ''), 'piece')) = LOWER(?)
                LIMIT 1
            ");
            if (!$stock_stmt) {
                throw new Exception('Database prepare error while fetching unit inventory');
            }
            $stock_stmt->bind_param('sis', $unit_type, $item_id, $unit_type);
            $stock_stmt->execute();
            $stock_result = $stock_stmt->get_result();
            $stock_row = $stock_result ? $stock_result->fetch_assoc() : null;
            $stock_stmt->close();
            if (!$stock_row) {
                $unit_type_name_for_inventory = $unit_type !== '' ? $unit_type : 'Piece';
                $create_inventory_stmt = $conn->prepare("
                    INSERT INTO motorpool_item_unit_inventory (item_id, unit_type_id, unit_type, current_inventory, as_of_date, unit_cost, total_cost, created_at, updated_at)
                    VALUES (?, NULL, ?, 0, CURDATE(), 0, 0, NOW(), NOW())
                ");
                if (!$create_inventory_stmt) {
                    throw new Exception('Database prepare error while creating motorpool unit inventory row');
                }
                $create_inventory_stmt->bind_param('is', $item_id, $unit_type_name_for_inventory);
                $create_inventory_stmt->execute();
                $create_inventory_stmt->close();

                $stock_stmt = $conn->prepare("
                    SELECT inventory_id, current_inventory, COALESCE(unit_cost, 0) AS unit_cost, COALESCE(unit_type_id, 0) AS unit_type_id, COALESCE(NULLIF(TRIM(unit_type), ''), ?) AS unit_type_name
                    FROM motorpool_item_unit_inventory
                    WHERE item_id = ? AND LOWER(COALESCE(NULLIF(TRIM(unit_type), ''), 'piece')) = LOWER(?)
                    LIMIT 1
                ");
                if (!$stock_stmt) {
                    throw new Exception('Database prepare error while refetching motorpool unit inventory');
                }
                $stock_stmt->bind_param('sis', $unit_type_name_for_inventory, $item_id, $unit_type_name_for_inventory);
                $stock_stmt->execute();
                $stock_result = $stock_stmt->get_result();
                $stock_row = $stock_result ? $stock_result->fetch_assoc() : null;
                $stock_stmt->close();
                ...
                if (!$stock_row) {
                    throw new Exception("Unable to create motorpool inventory row for unit type: '$unit_type_name_for_inventory'");
                }
            }
            ...
            $update_stmt = $conn->prepare("UPDATE motorpool_item_unit_inventory SET current_inventory = ?, updated_at = NOW() WHERE inventory_id = ?");
            if (!$update_stmt) {
            ...
            $item_stock_update = $conn->prepare("UPDATE motorpool_inventory_items SET stock = COALESCE(stock, 0) - ?, current_stock = COALESCE(current_stock, 0) - ?, stock_in_default_uom = COALESCE(stock_in_default_uom, 0) - ? WHERE item_id = ?");
            if ($item_stock_update) {
                $item_stock_update->bind_param('dddi', $pieces_to_deduct, $pieces_to_deduct, $pieces_to_deduct, $item_id);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'get_customer_discount') {
    ...
    try {
        ...
        if (!$view_all_branches && $branch_column_exists) {
            $check_sql = "SELECT customer_id FROM customers WHERE customer_id = ? AND branch_id = ? AND created_by IN (SELECT user_id FROM users WHERE role = 'motorpool')";
            $check_stmt = $conn->prepare($check_sql);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'update_sales_order_si_from_tab') {
    ...
    try {
        ...
        $check_stmt = $conn->prepare("SELECT so_id, order_status, branch_id, si_number FROM sales_orders WHERE so_id = ? AND created_by IN (SELECT user_id FROM users WHERE role = 'motorpool') LIMIT 1");
        ...
        if (amgcColumnExists($conn, 'sales_orders', 'si_number')) {
            $dup_stmt = $conn->prepare("SELECT so_id FROM sales_orders WHERE si_number = ? AND so_id <> ? AND created_by IN (SELECT user_id FROM users WHERE role = 'motorpool') LIMIT 1");
            if ($dup_stmt) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'update_sales_order_from_tab') {
    ...
    try {
        ...
        $check_sql = "SELECT so_id, order_status, branch_id, si_number, registered_business_name, tin, business_address FROM sales_orders WHERE so_id = ? AND created_by IN (SELECT user_id FROM users WHERE role = 'motorpool') LIMIT 1";
        $check_stmt = $conn->prepare($check_sql);
        if (!$check_stmt) throw new Exception('Prepare failed: ' . $conn->error);
        $check_stmt->bind_param('i', $so_id);
        $check_stmt->execute();
        ...
        if (amgcColumnExists($conn, 'sales_orders', 'fulfillment_type')) {
            $fulfillment_stmt = $conn->prepare("SELECT COALESCE(NULLIF(TRIM(fulfillment_type), ''), 'delivery') AS fulfillment_type FROM sales_orders WHERE so_id = ? AND created_by IN (SELECT user_id FROM users WHERE role = 'motorpool') LIMIT 1");
            if ($fulfillment_stmt) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'delete_sales_order_from_tab') {
    ...
    try {
        ...
        $check_stmt = $conn->prepare("SELECT so_id, order_status, branch_id FROM sales_orders WHERE so_id = ? AND created_by IN (SELECT user_id FROM users WHERE role = 'motorpool') LIMIT 1");
        if (!$check_stmt) throw new Exception('Prepare failed: ' . $conn->error);
        $check_stmt->bind_param('i', $so_id);
        $check_stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'get_order_details') {
    ...
    try {
        ...
        $items_sql = "SELECT
                     ...
                     JOIN motorpool_inventory_items i ON soi.item_id = i.item_id
                     WHERE soi.so_id = ?
        ...
        if (amgcOrderProductTableExists($conn, 'payments')) {
            ...
            if (!empty($payment_filters)) {
                $pay_sql = "SELECT * FROM payments WHERE created_by IN (SELECT user_id FROM users WHERE role = 'motorpool') AND (" . implode(' OR ', $payment_filters) . ") ORDER BY payment_date DESC, payment_id DESC";
        ...
        $query = "
            SELECT
                ...
                COALESCE(NULLIF($soi_ave_cost_expr, 0), (
                    ...
                    FROM motorpool_item_unit_inventory iui2
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'cancel_order') {
    ...
    try {
        ...
        if ($items_branch_column_exists && !$view_all_branches) {
            $check_sql = "SELECT so_id, order_status FROM sales_orders WHERE so_id = ? AND branch_id = ? AND created_by IN (SELECT user_id FROM users WHERE role = 'motorpool')";
            $check_stmt = $conn->prepare($check_sql);
...
$quantity = $quantity * $pieces_multiplier;
            ...
            $update_sql = "UPDATE motorpool_inventory_items SET stock = COALESCE(stock, 0) + ?, current_stock = COALESCE(current_stock, 0) + ?, stock_in_default_uom = COALESCE(stock_in_default_uom, 0) + ? WHERE item_id = ?";
            $update_stmt = $conn->prepare($update_sql);
            $update_stmt->bind_param('dddi', $quantity, $quantity, $quantity, $item_id);
            $update_stmt->execute();
            $update_stmt->close();
        }
...
try {
    ...
    $op_so_source_module_exists = amgcColumnExists($conn, 'sales_orders', 'source_module');
    $op_so_source_ris_number_exists = amgcColumnExists($conn, 'sales_orders', 'source_ris_number');
    ...
    $op_source_module_select = $op_so_source_module_exists ? "COALESCE(so.source_module,'') AS source_module" : "'' AS source_module";
    $op_source_ris_number_select = $op_so_source_ris_number_exists ? "COALESCE(so.source_ris_number,'') AS source_ris_number" : "'' AS source_ris_number";

    $op_branch_join = $op_so_branch_exists ? "LEFT JOIN branches b ON so.branch_id = b.branch_id" : "";
    $op_where = "WHERE 1=1 AND so.created_by IN (SELECT user_id FROM users WHERE role = 'motorpool')";
    ...
    if (!$view_all_branches && (int)$branch_id > 0) {
        if ($op_so_branch_exists) {
            $op_where .= " AND (so.branch_id = " . (int)$branch_id . ")";
        } elseif ($op_customers_branch_exists) {
            $op_where .= " AND (c.branch_id = " . (int)$branch_id . ")";
        }
    }
    ...
    $op_sales_order_sql = "
        SELECT
            ...
            $op_cogs_select,
            $op_gp_select,
            $op_source_module_select,
            $op_source_ris_number_select,
            c.customer_code,
            c.customer_name,
            c.store_name,
                                    ...
                                    ?>
                                        <tr class="op-so-clickable-row" data-status="<?php echo htmlspecialchars($status); ?>" data-date="<?php echo htmlspecialchars($orderDateFilter); ?>" data-customer="<?php echo (int)($order['customer_id'] ?? 0); ?>" data-search="<?php echo htmlspecialchars($searchBlob); ?>" onclick="openSalesOrderRowDetails(event, <?php echo (int)$order['so_id']; ?>)">
                                            <td class="fw-bold text-success">
                                                <?php echo htmlspecialchars($order['so_number'] ?? ''); ?>
                                                <?php if (($order['source_module'] ?? '') === 'motorpool_ris'): ?>
                                                    <div class="small text-muted fw-normal">RIS: <?php echo htmlspecialchars($order['source_ris_number'] ?? ''); ?></div>
                                                <?php endif; ?>
                                            </td>
...
<div class="modal fade" id="cartModal" tabindex="-1">
    ...
    <div class="mobile-nav" id="mobileNav">
        <ul class="nav">
            ...
            <li class="nav-item dropdown-more">
                <a class="nav-link active" href="#" onclick="toggleMobileDropdown(event, 'customerMobileMenu')">
                    <i class="bi bi-people-fill"></i>
                    <span>Customer</span>
                </a>
                <div class="more-dropdown" id="customerMobileMenu">
                    <a class="dropdown-item active" href="orderproduct.php"><i class="bi bi-receipt"></i><span>Create
                            Invoice</span></a>
...
<div class="modal fade no-print" id="siAttachmentPreviewModal" tabindex="-1" aria-hidden="true">
   ...
   <script>
    ...
    // Inventory data from PHP (reads motorpool_item_unit_inventory, same as Sales orderproduct)
    ...
    // ===== FAST NAVIGATION / AJAX SAFETY FIX =====
    // orderproduct.php used to fire one fetch per product for UoM + images during page load.
    // When the user clicked another sidebar/mobile nav item while those requests were still running,
    // the browser cancelled them and the page sometimes showed "Failed to fetch" or kept loading.
    let orderProductPageUnloading = false;
    const orderProductAbortControllers = new Set();

    function abortOrderProductAjaxRequests() {
        orderProductPageUnloading = true;
        orderProductAbortControllers.forEach(controller => {
            try { controller.abort(); } catch (e) {}
        });
        orderProductAbortControllers.clear();
    }

    function opAjaxFetch(url, options = {}) {
        const controller = new AbortController();
        orderProductAbortControllers.add(controller);
        const finalOptions = Object.assign({}, options, { signal: controller.signal });
        finalOptions.headers = Object.assign({ 'X-Requested-With': 'XMLHttpRequest' }, finalOptions.headers || {});
        return fetch(url, finalOptions).finally(() => orderProductAbortControllers.delete(controller));
    }

    function isRealNavigationLink(anchor) {
        if (!anchor) return false;
        const href = (anchor.getAttribute('href') || '').trim();
        if (!href || href === '#' || href.toLowerCase().startsWith('javascript:')) return false;
        if (anchor.hasAttribute('data-bs-toggle') || anchor.hasAttribute('data-bs-target')) return false;
    ...
    document.addEventListener('click', function(e) {
        ...
        if (isRealNavigationLink(anchor)) {
            abortOrderProductAjaxRequests();
            if (typeof hideProductsLoading === 'function') hideProductsLoading();
        }
    }, true);

    window.addEventListener('pagehide', abortOrderProductAjaxRequests);
    window.addEventListener('beforeunload', abortOrderProductAjaxRequests);
    ...
    function loadCustomerOutstandingSnapshot(customerId) {
        ...
        return opAjaxFetch(window.location.href, { method: 'POST', body: formData })
...
function formatStockDisplay(stockValue, unitType) {
    ...
    function getAvailableStock(productId, unitType = null) {
        ...
        // motorpool_item_unit_inventory keeps stock per UoM. Display the selected UoM's own stock,
        ...
        // Fallback for older records without motorpool_item_unit_inventory rows.
    ...
    function initProductUnitTypeFallbacks() {
        inventory.forEach(product => {
            const productId = product.id;
            if (!activeUnitTypes[productId]) {
                activeUnitTypes[productId] = product.default_unit_type_name || product.unit_type || 'Piece';
            }
            if (!productUnitTypes[productId] || !productUnitTypes[productId].length) {
                const conversions = productUnitConversions[productId] || {};
                const fallbackUnits = Object.keys(conversions).length
                    ? Object.entries(conversions).map(([unitName, multiplier]) => ({
                        unit_type_id: 0,
                        unit_type_name: unitName,
                        uom_initial: '',
                        unit_price: product.unit_price || 0,
                        quantity_smallest_pack: parseInt(multiplier) || 1
                    }))
                    : [{
                        unit_type_id: product.default_unit_type_id || 0,
                        unit_type_name: product.default_unit_type_name || product.unit_type || 'Piece',
                        uom_initial: '',
                        unit_price: product.unit_price || 0,
                        quantity_smallest_pack: product.default_unit_multiplier || 1
                    }];
                productUnitTypes[productId] = fallbackUnits;
            }
        });
    }
    ...
    function runWithConcurrency(items, limit, worker) {
        ...
        const runners = Array.from({ length: Math.min(max, items.length) }, async () => {
            while (index < items.length && !orderProductPageUnloading) {
    ...
    function loadProductUnitTypes(productId, priceLevel = 'Standard') {
        if (orderProductPageUnloading) return Promise.resolve();
        const formData = new FormData();
        ...
        return opAjaxFetch(window.location.href, { method: 'POST', body: formData })
            .then(response => response.json())
            .then(data => {
                if (orderProductPageUnloading) return;
                if (data.success && data.unit_types && data.unit_types.length > 0) {
                    ...
                    data.unit_types.forEach(ut => {
                        ...
                        if (!unitName || seenUnitTypes.has(key)) return;
                    ...
                    if (!activeUnitTypes[productId]) activeUnitTypes[productId] = smallestUnit.unit_type_name;
            ...
            .catch(error => {
                ...
                // Keep fallback UoM data and never block page navigation.
            });
    }
    
    function loadAllProductUnitTypes(priceLevel = 'Standard', limit = 6) {
        return runWithConcurrency(inventory, limit, product => loadProductUnitTypes(product.id, priceLevel));
    }
    
    function loadProductImages(productId) {
        if (orderProductPageUnloading) return Promise.resolve();
        const product = getProductById(productId);
        if (product && product.image) return Promise.resolve();
    ...
    function loadAllProductImages(limit = 4) {
        const productsWithoutInlineImage = inventory.filter(product => !product.image);
        return runWithConcurrency(productsWithoutInlineImage, limit, product => loadProductImages(product.id));
    }

    function preloadProductDataInBackground(priceLevel = 'Standard') {
        const visibleIds = Array.from(document.querySelectorAll('#productsContainer tr[id^="row-"]'))
            .map(row => parseInt(String(row.id || '').replace('row-', ''), 10))
            .filter(Boolean);
        const visibleProducts = visibleIds.length
            ? visibleIds.map(id => getProductById(id)).filter(Boolean)
            : inventory.slice(0, 30);
        runWithConcurrency(visibleProducts, 4, product => loadProductUnitTypes(product.id, priceLevel).then(() => loadProductImages(product.id)))
            .then(() => {
                if (!orderProductPageUnloading) renderProducts();
            });
    }
    ...
    function reloadProductPrices(priceLevel) {
        ...
        return loadAllProductUnitTypes(priceLevel, 6).then(() => {
            if (!orderProductPageUnloading) {
                hideProductsLoading();
                renderProducts();
            }
        }).catch(() => {
    ...
    document.addEventListener('DOMContentLoaded', function() { toggleSIDetails(); toggleBillingTypeFields(); updateInvoiceDocumentTitle(); });
...
if (!custId) {
    ...
    const submitWithSoSuffix = (manualSoSuffix, approvalExplanation = '', approvalAcknowledged = false, approvalType = 'credit_limit') => {
        ...
        opAjaxFetch(window.location.href, { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, body: formBody })
    ...
    function showProductInfo(pid) {
        ...
        opAjaxFetch(window.location.href, { method: 'POST', body: fd })
            .then(r => r.json())
            .then(data => {
                if (data.success) {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // FAST LOAD FIX: render immediately using PHP-loaded data, then enhance UoM/images quietly.
        initProductUnitTypeFallbacks();
        hideProductsLoading();
        renderProducts();
        updateCartBadge();
        setupSearch();
        if (lockedCustomerId) loadCustomerDiscount(lockedCustomerId);
        setTimeout(() => preloadProductDataInBackground(initialPriceLevel), 100);
    ...
    function printSingleOrderFromOrderProduct(orderId) {
        ...
        opAjaxFetch('orderproduct.php', { method: 'POST', body: formData })
            .then(response => response.json())
            .then(data => {
                if (data.success) {
    ...
    function saveEditedOrderFromOrderProduct(event) {
        ...
        opAjaxFetch('orderproduct.php', { method: 'POST', body: formData, headers: {'X-Requested-With':'XMLHttpRequest'} })
            .then(r => r.json())
            .then(data => {
...
<script>
/* ===== FINAL FIX: persistent mobile dropdown active state + logout closes open dropdown ===== */
(function () {
    'use strict';
    ...
    function getDropdownIdFromButton(btn) {
        if (!btn) return '';
        var direct = btn.getAttribute('data-dropdown') || '';
    ...
    function getButtonForMenu(menuId) {
        if (!menuId) return null;
        return document.querySelector(
            '.mobile-nav .more-btn[data-dropdown="' + menuId + '"], ' +
            '.mobile-nav .more-btn[onclick*="' + menuId + '"]'
        );
    }

    function keepCurrentParentActive() {
        var nav = document.getElementById('mobileNav');
        if (!nav) return;

        var currentPage = normalizePage(window.location.pathname);

        nav.querySelectorAll('.nav-link, .dropdown-item').forEach(function (item) {
            item.classList.remove('active-page', 'has-active');
        });

        nav.querySelectorAll('.dropdown-item[href]').forEach(function (item) {
            var href = normalizePage(item.getAttribute('href'));
            if (href && href === currentPage) {
                item.classList.add('active', 'active-page');

                var menu = item.closest('.more-dropdown');
                if (menu) {
                    var parentBtn = getButtonForMenu(menu.id);
                    if (parentBtn) {
                        parentBtn.classList.add('active', 'active-page', 'has-active');
        ...
        nav.querySelectorAll('.nav-link:not(.more-btn):not(.logout-btn)[href]').forEach(function (item) {
            var href = normalizePage(item.getAttribute('href'));
            if (href && href === currentPage) {
                item.classList.add('active', 'active-page');
    ...
    function closeAllMobileDropdowns(exceptId, forceRemoveActive) {
        ...
        document.querySelectorAll('.mobile-nav .more-btn').forEach(function (btn) {
            var menuId = getDropdownIdFromButton(btn);
            var isExcept = exceptId && menuId === exceptId;
            var menu = menuId ? document.getElementById(menuId) : null;
            var hasActiveChild = menu && menu.querySelector('.dropdown-item.active-page, .dropdown-item.active');
            ...
            if (forceRemoveActive || !hasActiveChild) {
                btn.classList.remove('active');
            }

            if (hasActiveChild && !forceRemoveActive) {
                btn.classList.add('active', 'active-page', 'has-active');
            }
        });

        if (!forceRemoveActive) keepCurrentParentActive();
    }
    ...
    function toggleMobileDropdown(event, dropdownId) {
        ...
        var btn = event && event.currentTarget ? event.currentTarget : getButtonForMenu(dropdownId);

        if (!dropdown || !btn) return false;

        var wasOpen = dropdown.classList.contains('show');

        closeAllMobileDropdowns(dropdownId, false);

        if (wasOpen) {
            dropdown.classList.remove('show');
            btn.setAttribute('aria-expanded', 'false');

            var hasActiveChild = dropdown.querySelector('.dropdown-item.active-page, .dropdown-item.active');
            if (!hasActiveChild) btn.classList.remove('active');
            keepCurrentParentActive();
        } else {
            dropdown.classList.add('show');
            btn.classList.add('active');
            btn.setAttribute('aria-expanded', 'true');
    ...
    function closeMobileDropdownsForLogout() {
        closeAllMobileDropdowns('', true);
    ...
    window.closeAllMobileDropdowns = closeAllMobileDropdowns;
    window.toggleMobileDropdown = toggleMobileDropdown;
    window.setActiveMobileNav = keepCurrentParentActive;

    var oldShowProfileModal = window.showProfileModal;
    window.showProfileModal = function () {
        closeMobileDropdownsForLogout();
        if (typeof oldShowProfileModal === 'function') {
            return oldShowProfileModal.apply(this, arguments);
        }
    ...
    var oldConfirmLogout = window.confirmLogout;
    window.confirmLogout = function () {
        closeMobileDropdownsForLogout();
        if (typeof oldConfirmLogout === 'function') {
            return oldConfirmLogout.apply(this, arguments);
        }
        return false;
    };

    document.addEventListener('click', function (event) {
        var logoutBtn = event.target && event.target.closest ? event.target.closest('.mobile-nav .logout-btn, .logout-btn') : null;
        if (logoutBtn) {
            closeMobileDropdownsForLogout();
            return;
        }

        var btn = event.target && event.target.closest ? event.target.closest('.mobile-nav .more-btn') : null;
        if (btn) {
            var menuId = getDropdownIdFromButton(btn);
            if (menuId) {
                toggleMobileDropdown(event, menuId);
            }
            return;
        }

        if (!(event.target && event.target.closest && event.target.closest('.mobile-nav .more-dropdown'))) {
            closeAllMobileDropdowns('', false);
        }
    }, true);

    document.addEventListener('keydown', function (event) {
        if (event.key === 'Escape') closeAllMobileDropdowns('', false);
    });

    document.addEventListener('DOMContentLoaded', keepCurrentParentActive);
    window.addEventListener('load', keepCurrentParentActive);
})();
</script>
