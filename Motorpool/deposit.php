...
function ensureMotorpoolDepositBranchContext(mysqli $conn): array {
    $branchName = 'Motorpool';
    $check = $conn->query("SHOW TABLES LIKE 'branches'");
    if ($check && $check->num_rows > 0) {
        ...
        if ($stmt) {
            ...
            $stmt->close();
            if ($row) return [(int)$row['branch_id'], trim((string)$row['branch_name']) ?: $branchName];
        }
...
[$branch_id, $branch_name] = ensureMotorpoolDepositBranchContext($conn);
...
?>
<?php $page_title = 'Motorpool Deposit'; $page_subtitle = 'Motorpool Undeposited Funds and Deposit History'; $active_page = 'deposit'; ?>
<!DOCTYPE html>
<html lang="en"><head>
    ...
    <div class="mobile-nav" id="mobileNav">
        <ul class="nav">
            ...
            <li class="nav-item dropdown-more">
                ...
                <div class="more-dropdown active" id="bankingMobileMenu">
...
window.closeAllMobileDropdowns = function() {
    ...
    document.querySelectorAll('.mobile-nav .more-btn, .more-btn').forEach(function(btn) {
        ...
        const hasActiveChild = navItem && navItem.querySelector('.more-dropdown .dropdown-item.active');
...
window.setActiveMobileNav = function() {
    const currentPage = (window.location.pathname.split('/').pop() || 'motorpool_inventory.php').split('?')[0].split('#')[0];
    ...
    document.querySelectorAll('.mobile-nav a[href]').forEach(function(link) {
        ...
        let hrefPage = rawHref;
        try {
            hrefPage = new URL(rawHref, window.location.href).pathname.split('/').pop();
        } catch (err) {
            hrefPage = rawHref.split('/').pop();
        }
        hrefPage = (hrefPage || '').split('?')[0].split('#')[0];

        if (hrefPage === currentPage || (currentPage === '' && hrefPage === 'motorpool_inventory.php')) {
            link.classList.add('active');

            const navItem = link.closest('.dropdown-more');
            if (navItem) {
                const moreBtn = navItem.querySelector('.more-btn');
                if (moreBtn) {
...
document.addEventListener('DOMContentLoaded', function() {
    document.querySelectorAll('.mobile-nav .dropdown-item').forEach(function(item) {
        item.addEventListener('click', function() {
            window.closeAllMobileDropdowns();
            if (typeof window.setActiveMobileNav === 'function') {
...
function setActiveSidebarItem() {
    ...
    document.querySelectorAll('.sidebar .nav-link').forEach(link => {
        ...
        if (href === currentPage || (currentPage === '' && href === 'motorpool_inventory.php')) {
            link.classList.add('active');
