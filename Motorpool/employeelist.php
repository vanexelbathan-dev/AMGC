...
function ensureMotorpoolBranchContext(mysqli $conn): array {
    ...
    // Never inherit Branch Admin's current branch. Motorpool uses its own branch/account scope.
    if (employeeTableExists($conn, 'branches')) {
        ...
        if ($stmt) {
            ...
            if ($row) {
                ...
                return [(int)$row['branch_id'], trim((string)$row['branch_name']) ?: 'Motorpool'];
            }
        }
    ...
    try {
        ...
        if ($action === 'save_payroll') {
            ...
            if ($payroll_id <= 0 && $employee_id > 0) {
                $findExistingPayroll = $conn->prepare("SELECT payroll_id FROM employee_payroll_system WHERE employee_id=? AND branch_id=? AND COALESCE(source_account, '') = 'motorpool' ORDER BY payroll_id DESC LIMIT 1");
            ...
            $existingAttachments = [];
            if ($payroll_id > 0) {
                $getAttach = $conn->prepare("SELECT attachments FROM employee_payroll_system WHERE payroll_id=? AND branch_id=? AND COALESCE(source_account, '') = 'motorpool'");
                if ($getAttach) {
            ...
            $payrollColumns = [
                ...
                'pagibig_ee_share', 'sss_er_share', 'philhealth_er_share', 'pagibig_er_share', 'status', 'source_account', 'source_user_id'
            ];
            $payrollTypeMap = [
                ...
                'pagibig_ee_share'=>'d', 'sss_er_share'=>'d', 'philhealth_er_share'=>'d', 'pagibig_er_share'=>'d', 'status'=>'s', 'source_account'=>'s', 'source_user_id'=>'i'
            ];
        ...
        if ($action === 'save_employee') {
            ...
            if ($employee_id > 0) {
                ...
                $stmt->close();
                $conn->query("UPDATE employees SET source_account='motorpool', source_user_id=" . (int)$user_id . " WHERE employee_id=" . (int)$newEmployeeId);
        ...
        if ($action === 'update_employee_job') {
            ...
            if (!$view_all_branches) $checkSql .= " AND e.branch_id=" . (int)$branch_id . " AND COALESCE(e.source_account, '') = 'motorpool'";
            $checkStmt = $conn->prepare($checkSql);
            if (!$checkStmt) throw new Exception('Prepare failed: ' . $conn->error);
        ...
        if ($action === 'save_dtr') {
            ...
            foreach ($employee_ids as $employee_id) {
                ...
                if (!$view_all_branches) $existingSql .= " AND branch_id=" . (int)$branch_id . " AND COALESCE(source_account, '') = 'motorpool'";
        ...
        if ($action === 'set_out_selected') {
            ...
            foreach ($dtr_ids as $dtr_id) {
                ...
                if (!$view_all_branches) $getSql .= " AND branch_id=" . (int)$branch_id . " AND COALESCE(source_account, '') = 'motorpool'";
...
$where = "e.branch_id = " . (int)$branch_id . " AND COALESCE(e.source_account, '') = 'motorpool'";
$employees = [];
...
$payrollEmployees = array_values(array_filter($employees, function($emp) use ($branch_id) {
    return (int)($emp['branch_id'] ?? 0) === (int)$branch_id && (($emp['source_account'] ?? '') === 'motorpool');
}));

$payrollWhere = 'branch_id=' . (int)$branch_id . " AND COALESCE(source_account, '') = 'motorpool'";
$payrollRows = [];
...
$dtrSql = "SELECT d.*, e.employee_name, e.contact_number, b.branch_name,
          ...
          FROM employee_dtr d
          INNER JOIN employees e ON d.employee_id=e.employee_id AND COALESCE(e.source_account, '') = 'motorpool'
...
<title>Motorpool Employee List</title>
...
/* =========================================================
   EMPLOYEE DETAILS + ADD EMPLOYEE MODAL MOBILE FIX
   Mobile preview only:
   - Modal never goes under the bottom mobile navigation.
   - Page/background does not scroll while modal is open.
   - Header/footer stay fixed inside the modal.
   - Only modal body/tab content scrolls.
   - Mobile nav is not hidden or modified.
   ========================================================= */
@media (max-width: 991.98px) {
    :root {
        --employee-mobile-nav-space: 82px;
        --employee-modal-edge-gap: 10px;
    }
    ...
    #employeeDetailsModal {
        ...
        overflow: hidden !important;
        padding: var(--employee-modal-edge-gap) var(--employee-modal-edge-gap) calc(var(--employee-mobile-nav-space) + var(--employee-modal-edge-gap)) var(--employee-modal-edge-gap) !important;
    }
    ...
    #employeeDetailsModal .modal-dialog {
        width: 100% !important;
        max-width: calc(100vw - (var(--employee-modal-edge-gap) * 2)) !important;
        height: calc(100dvh - var(--employee-mobile-nav-space) - (var(--employee-modal-edge-gap) * 2)) !important;
        max-height: calc(100dvh - var(--employee-mobile-nav-space) - (var(--employee-modal-edge-gap) * 2)) !important;
    ...
    @supports not (height: 100dvh) {
        #employeeModal .modal-dialog,
        #employeeDetailsModal .modal-dialog {
            height: calc(100vh - var(--employee-mobile-nav-space) - (var(--employee-modal-edge-gap) * 2)) !important;
            max-height: calc(100vh - var(--employee-mobile-nav-space) - (var(--employee-modal-edge-gap) * 2)) !important;
        }
    }
...
@media (max-width: 576px) {
    :root {
        --employee-mobile-nav-space: 86px;
        --employee-modal-edge-gap: 8px;
    }
    ...
    #employeeDetailsModal {
        ...
        padding-right: 8px !important;
        padding-bottom: calc(var(--employee-mobile-nav-space) + 8px) !important;
    }

    #employeeModal .modal-dialog,
    #employeeDetailsModal .modal-dialog {
        max-width: calc(100vw - 16px) !important;
        height: calc(100dvh - var(--employee-mobile-nav-space) - 16px) !important;
        max-height: calc(100dvh - var(--employee-mobile-nav-space) - 16px) !important;
    }

    @supports not (height: 100dvh) {
        #employeeModal .modal-dialog,
        #employeeDetailsModal .modal-dialog {
            height: calc(100vh - var(--employee-mobile-nav-space) - 16px) !important;
            max-height: calc(100vh - var(--employee-mobile-nav-space) - 16px) !important;
        }
    }
    ...
    #employeeDetailsModal .modal-footer {
        ...
        /* Match Branch Admin tab weight */
        .dashboard-tabs .nav-link,
        .dashboard-tabs .nav-link.active,
        #employee-list-main-tab,
        #payroll-system-main-tab {
            font-weight: 700 !important;
        }
    </style>
</head>
    ...
    <div class="page-content active">
        <div class="navbar-top">
            ...
            <div class="page-title">
                <h2>Employee List</h2>
                <p>Manage motorpool employee profiles</p>
            </div>
        </div>
        ...
        <ul class="nav">
            ...
            <li class="nav-item dropdown-more">
                ...
                <div class="more-dropdown" id="employeeMobileMenu">
                    <a class="dropdown-item active" href="employeelist.php"><i class="bi bi-receipt"></i><span>Employee
                            List</span></a>
