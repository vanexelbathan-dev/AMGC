...
$source_branch_id = (int)($_SESSION['branch_id'] ?? 0);
if ($source_branch_id <= 0) {
    $source_branch_id = (int)$branch_id;
} // Branch Admin/source branch items
...
// Ensure UoM Initial column exists for Rolling UoM button labels
if (!function_exists('rollingOrderProductColumnExists')) {
    function rollingOrderProductColumnExists($conn, $table, $column) {
...
if (!rollingOrderProductColumnExists($conn, 'unit_types', 'uom_initial')) {
...
// ========== ROLLING PRODUCT SOURCE: SHOW ALL ITEMS, STOCK ONLY IF RECEIVED ========== 
// Product catalog still shows all active Branch Admin/source branch items.
// Stock display and ordering availability are computed only from this Rolling user's received quantity.
function ensureRollingTransferTablesForOrderProduct(mysqli $conn): void {
    $conn->query("CREATE TABLE IF NOT EXISTS `rolling_inventory_transfers` (
        `transfer_id` int(11) NOT NULL AUTO_INCREMENT,
        `transfer_number` varchar(60) NOT NULL,
        `source_branch_id` int(11) NOT NULL DEFAULT 0,
        `rolling_branch_id` int(11) NOT NULL DEFAULT 0,
    ...
    $conn->query("CREATE TABLE IF NOT EXISTS `rolling_inventory_transfer_items` (
        ...
        KEY `idx_transfer_item_unit` (`unit_type_id`)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci");
}

function fetchAllProductUnits(mysqli $conn, int $item_id, string $price_level = 'Standard'): array {
    $units = [];
    if ($item_id <= 0) return $units;
    ...
    $columnsOf = function(string $table) use ($conn, $tableExists): array {
    ...
    $hasCol = function(array $cols, string $name): bool {
        foreach ($cols as $c) {
            if (strcasecmp($c, $name) === 0) return true;
        }
        return false;
    };

    $resolveUnitById = function(int $unit_type_id) use ($conn): ?array {
        if ($unit_type_id <= 0) return null;
        $stmt = $conn->prepare("SELECT unit_type_id, unit_type_name, uom_initial, COALESCE(quantity_smallest_pack,1) AS quantity_smallest_pack, COALESCE(is_default_uom,0) AS is_default_uom FROM unit_types WHERE unit_type_id = ? LIMIT 1");
        if (!$stmt) return null;
        $stmt->bind_param('i', $unit_type_id);
        $stmt->execute();
        $res = $stmt->get_result();
    ...
    $resolveUnitByName = function(string $unit_type_name) use ($conn): ?array {
        $unit_type_name = trim($unit_type_name);
        if ($unit_type_name === '') return null;
        $stmt = $conn->prepare("SELECT unit_type_id, unit_type_name, uom_initial, COALESCE(quantity_smallest_pack,1) AS quantity_smallest_pack, COALESCE(is_default_uom,0) AS is_default_uom FROM unit_types WHERE LOWER(TRIM(unit_type_name)) = LOWER(TRIM(?)) AND (status IS NULL OR status = '' OR LOWER(TRIM(status)) = 'active') ORDER BY is_default_uom DESC, unit_type_id ASC LIMIT 1");
        if (!$stmt) return null;
        $stmt->bind_param('s', $unit_type_name);
        $stmt->execute();
        $res = $stmt->get_result();
    ...
    $priceForUnit = function(int $unit_type_id, string $unit_type_name = '') use ($conn, $item_id, $price_level): float {
        if ($unit_type_id > 0) {
            $stmt = $conn->prepare("SELECT unit_price FROM item_unit_pricing WHERE item_id = ? AND unit_type_id = ? AND (price_level = ? OR price_level = 'Standard' OR price_level IS NULL OR price_level = '') ORDER BY CASE WHEN price_level = ? THEN 0 WHEN price_level = 'Standard' THEN 1 ELSE 2 END, CASE WHEN effective_date IS NULL THEN 1 WHEN effective_date <= CURDATE() THEN 0 ELSE 2 END, effective_date DESC, pricing_id DESC LIMIT 1");
            if ($stmt) {
                $stmt->bind_param('iiss', $item_id, $unit_type_id, $price_level, $price_level);
                $stmt->execute();
                $res = $stmt->get_result();
        ...
        if (trim($unit_type_name) !== '') {
            $stmt = $conn->prepare("SELECT iup.unit_price FROM item_unit_pricing iup INNER JOIN unit_types ut ON ut.unit_type_id = iup.unit_type_id WHERE iup.item_id = ? AND LOWER(TRIM(ut.unit_type_name)) = LOWER(TRIM(?)) AND (iup.price_level = ? OR iup.price_level = 'Standard' OR iup.price_level IS NULL OR iup.price_level = '') ORDER BY CASE WHEN iup.price_level = ? THEN 0 WHEN iup.price_level = 'Standard' THEN 1 ELSE 2 END, CASE WHEN iup.effective_date IS NULL THEN 1 WHEN iup.effective_date <= CURDATE() THEN 0 ELSE 2 END, iup.effective_date DESC, iup.pricing_id DESC LIMIT 1");
            if ($stmt) {
                $stmt->bind_param('isss', $item_id, $unit_type_name, $price_level, $price_level);
                $stmt->execute();
                $res = $stmt->get_result();
                if ($row = ($res ? $res->fetch_assoc() : null)) {
                    $stmt->close();
        ...
        $stmt = $conn->prepare("SELECT COALESCE(unit_price,0) AS unit_price FROM items WHERE item_id = ? LIMIT 1");
        if ($stmt) {
            $stmt->bind_param('i', $item_id);
            $stmt->execute();
            $res = $stmt->get_result();
    ...
    $addUnit = function($unit_type_id, $unit_type_name, $unit_price = null, $quantity_smallest_pack = null, $is_default_uom = 0) use (&$units, $resolveUnitById, $resolveUnitByName, $priceForUnit) {
        $unit_type_id = (int)$unit_type_id;
        $unit_type_name = trim((string)$unit_type_name);
        $resolved = null;
        if ($unit_type_id > 0) $resolved = $resolveUnitById($unit_type_id);
        if (!$resolved && $unit_type_name !== '') $resolved = $resolveUnitByName($unit_type_name);
        if ($resolved) {
            $unit_type_id = (int)($resolved['unit_type_id'] ?? $unit_type_id);
            $unit_type_name = trim((string)($resolved['unit_type_name'] ?? $unit_type_name));
            $uom_initial = trim((string)($resolved['uom_initial'] ?? ''));
            if ($quantity_smallest_pack === null || (float)$quantity_smallest_pack <= 0) {
                $quantity_smallest_pack = (float)($resolved['quantity_smallest_pack'] ?? 1);
            }
            $is_default_uom = max((int)$is_default_uom, (int)($resolved['is_default_uom'] ?? 0));
        }
        if ($unit_type_name === '') return;
        if (!isset($uom_initial)) { $uom_initial = ''; }
        $key = $unit_type_id > 0 ? 'id:' . $unit_type_id : 'name:' . strtolower($unit_type_name);
        $nameKey = 'name:' . strtolower($unit_type_name);
        if (isset($units[$key]) || isset($units[$nameKey])) return;
        $unit_price = $unit_price !== null ? (float)$unit_price : 0.0;
        if ($unit_price <= 0) $unit_price = $priceForUnit($unit_type_id, $unit_type_name);
        $units[$key] = [
            'unit_type_id' => $unit_type_id,
            'unit_type_name' => $unit_type_name,
            'uom_initial' => $uom_initial,
            'unit_price' => $unit_price,
            'quantity_smallest_pack' => max(1, (int)($quantity_smallest_pack ?? 1)),
            'is_default_uom' => (int)$is_default_uom
        ];
        $units[$nameKey] = $units[$key];
    };

    // Source 1: all item-specific pricing rows. This is the main source of registered UoM buttons.
    if ($tableExists('item_unit_pricing')) {
        $stmt = $conn->prepare("SELECT iup.unit_type_id, ut.unit_type_name, ut.uom_initial, COALESCE(iup.unit_price,0) AS unit_price, COALESCE(ut.quantity_smallest_pack,1) AS quantity_smallest_pack, COALESCE(ut.is_default_uom,0) AS is_default_uom FROM item_unit_pricing iup INNER JOIN unit_types ut ON ut.unit_type_id = iup.unit_type_id WHERE iup.item_id = ? AND (ut.status IS NULL OR ut.status = '' OR LOWER(TRIM(ut.status)) = 'active') ORDER BY COALESCE(ut.is_default_uom,0) DESC, COALESCE(ut.quantity_smallest_pack,1) ASC, ut.unit_type_name ASC, iup.pricing_id ASC");
        if ($stmt) {
            $stmt->bind_param('i', $item_id);
            $stmt->execute();
            $res = $stmt->get_result();
            while ($row = $res ? $res->fetch_assoc() : null) {
                $addUnit($row['unit_type_id'] ?? 0, $row['unit_type_name'] ?? '', $row['unit_price'] ?? null, $row['quantity_smallest_pack'] ?? 1, $row['is_default_uom'] ?? 0);
            }
            $stmt->close();
        }
    }

    // Source 2: item_unit_types supports either unit_type_id or unit_type_name depending on your schema.
    if ($tableExists('item_unit_types')) {
        $cols = $columnsOf('item_unit_types');
        if ($hasCol($cols, 'item_id')) {
            $selectParts = ['*'];
            $where = "WHERE item_id = ?";
            if ($hasCol($cols, 'status')) {
                $where .= " AND (status IS NULL OR status = '' OR LOWER(TRIM(status)) = 'active')";
            }
            $stmt = $conn->prepare("SELECT * FROM item_unit_types {$where}");
            if ($stmt) {
                $stmt->bind_param('i', $item_id);
                $stmt->execute();
                $res = $stmt->get_result();
                while ($row = $res ? $res->fetch_assoc() : null) {
                    $utId = 0;
                    foreach (['unit_type_id','uom_id','unit_id'] as $idCol) {
                        if (isset($row[$idCol]) && (int)$row[$idCol] > 0) { $utId = (int)$row[$idCol]; break; }
                    }
                    $utName = '';
                    foreach (['unit_type_name','uom','unit','unit_name','unit_of_measure'] as $nameCol) {
                        if (isset($row[$nameCol]) && trim((string)$row[$nameCol]) !== '') { $utName = trim((string)$row[$nameCol]); break; }
                    }
                    $qtySmallest = null;
                    foreach (['quantity_smallest_pack','qty_smallest_pack','unit_quantity','multiplier'] as $qCol) {
                        if (isset($row[$qCol]) && (float)$row[$qCol] > 0) { $qtySmallest = (float)$row[$qCol]; break; }
                    }
                    $isDefault = isset($row['is_default_uom']) ? (int)$row['is_default_uom'] : 0;
                    $addUnit($utId, $utName, null, $qtySmallest, $isDefault);
                }
                $stmt->close();
            }
        }
    ...
    // Source 3: current inventory UoM rows are also item-specific registered UoMs in this system.
    if ($tableExists('item_unit_inventory')) {
        $stmt = $conn->prepare("SELECT iui.unit_type_id, ut.unit_type_name, ut.uom_initial, COALESCE(iui.unit_cost,0) AS unit_cost, COALESCE(ut.quantity_smallest_pack,1) AS quantity_smallest_pack, COALESCE(ut.is_default_uom,0) AS is_default_uom FROM item_unit_inventory iui INNER JOIN unit_types ut ON ut.unit_type_id = iui.unit_type_id WHERE iui.item_id = ? AND (ut.status IS NULL OR ut.status = '' OR LOWER(TRIM(ut.status)) = 'active') ORDER BY COALESCE(ut.is_default_uom,0) DESC, COALESCE(ut.quantity_smallest_pack,1) ASC, ut.unit_type_name ASC");
        if ($stmt) {
            $stmt->bind_param('i', $item_id);
            $stmt->execute();
            $res = $stmt->get_result();
            while ($row = $res ? $res->fetch_assoc() : null) {
                $addUnit($row['unit_type_id'] ?? 0, $row['unit_type_name'] ?? '', $row['unit_cost'] ?? null, $row['quantity_smallest_pack'] ?? 1, $row['is_default_uom'] ?? 0);
            }
            $stmt->close();
        }
    }

    // Source 4: item's default/saved UoM only as fallback, not as a replacement.
    $stmt = $conn->prepare("SELECT default_unit_type_id, COALESCE(NULLIF(TRIM(unit_type), ''), 'Piece') AS unit_type, COALESCE(unit_price, 0) AS unit_price FROM items WHERE item_id = ? LIMIT 1");
    if ($stmt) {
        $stmt->bind_param('i', $item_id);
        $stmt->execute();
        $res = $stmt->get_result();
        if ($row = ($res ? $res->fetch_assoc() : null)) {
            $addUnit((int)($row['default_unit_type_id'] ?? 0), (string)($row['unit_type'] ?? ''), (float)($row['unit_price'] ?? 0), null, 1);
        }
        $stmt->close();
    }
    ...
    $seen = [];
    foreach ($units as $u) {
        $name = trim((string)($u['unit_type_name'] ?? ''));
        if ($name === '') continue;
        $key = strtolower($name);
    ...
    usort($deduped, function($a, $b) {
        $ad = (int)($a['is_default_uom'] ?? 0);
        $bd = (int)($b['is_default_uom'] ?? 0);
        if ($ad !== $bd) return $bd <=> $ad;
        $aq = (int)($a['quantity_smallest_pack'] ?? 1);
        $bq = (int)($b['quantity_smallest_pack'] ?? 1);
        if ($aq !== $bq) return $aq <=> $bq;
        return strcasecmp((string)($a['unit_type_name'] ?? ''), (string)($b['unit_type_name'] ?? ''));
    });
    return $deduped;
}

function fetchRollingReceivedUnits(mysqli $conn, int $item_id, int $rolling_branch_id, int $user_id, string $price_level = 'Standard'): array {
    $rows = [];
    if ($item_id <= 0 || $rolling_branch_id <= 0 || $user_id <= 0) return $rows;
    ...
    $query = "
        SELECT
            ...
            COALESCE(NULLIF(base.unit_price, 0), NULLIF(iup_match.unit_price, 0), NULLIF(iup_std.unit_price, 0), i.unit_price, 0) AS unit_price,
            GREATEST(COALESCE(base.received_quantity, 0) - COALESCE(sold.sold_quantity, 0), 0) AS received_quantity
        FROM (
            SELECT
                rti.item_id,
            ...
            INNER JOIN rolling_inventory_transfers rt
               ...
               AND rt.received_by = ?
               AND LOWER(TRIM(rt.status)) = 'received'
            LEFT JOIN unit_types ut_rx ON ut_rx.unit_type_id = rti.unit_type_id
            WHERE rti.item_id = ?
              AND COALESCE(rti.quantity_received, 0) > 0
            GROUP BY rti.item_id, COALESCE(rti.unit_type_id, 0), COALESCE(NULLIF(TRIM(rti.unit_type_name), ''), ut_rx.unit_type_name, 'Piece')
        ) base
        INNER JOIN items i ON i.item_id = base.item_id
        LEFT JOIN unit_types ut ON ut.unit_type_id = NULLIF(base.unit_type_id, 0)
        LEFT JOIN (
            SELECT soi.item_id, soi.unit_type, SUM(COALESCE(soi.quantity_ordered, 0)) AS sold_quantity
            FROM sales_order_items soi
            INNER JOIN sales_orders so ON so.so_id = soi.so_id
            WHERE so.branch_id = ?
              AND so.created_by = ?
              AND LOWER(TRIM(COALESCE(so.order_status, 'pending'))) NOT IN ('cancelled','canceled','void')
              AND soi.item_id = ?
            GROUP BY soi.item_id, soi.unit_type
        ) sold ON sold.item_id = base.item_id AND LOWER(TRIM(sold.unit_type)) = LOWER(TRIM(base.unit_type_name))
        LEFT JOIN item_unit_pricing iup_match
            ON iup_match.item_id = base.item_id
           AND iup_match.unit_type_id = CASE WHEN base.unit_type_id > 0 THEN base.unit_type_id ELSE i.default_unit_type_id END
           AND iup_match.price_level = ?
        LEFT JOIN item_unit_pricing iup_std
            ON iup_std.item_id = base.item_id
           AND iup_std.unit_type_id = CASE WHEN base.unit_type_id > 0 THEN base.unit_type_id ELSE i.default_unit_type_id END
           AND iup_std.price_level = 'Standard'
        ORDER BY COALESCE(ut.is_default_uom, 0) DESC, COALESCE(ut.quantity_smallest_pack, 1) ASC, base.unit_type_name ASC
    ";
    $stmt = $conn->prepare($query);
    if (!$stmt) return $rows;
    $stmt->bind_param('iiiiiis', $rolling_branch_id, $user_id, $item_id, $rolling_branch_id, $user_id, $item_id, $price_level);
    $stmt->execute();
    $res = $stmt->get_result();
...
function getRollingAvailableQuantityForUom(mysqli $conn, int $item_id, string $unit_type, int $rolling_branch_id, int $user_id): float {
    $available = 0.0;
    foreach (fetchRollingReceivedUnits($conn, $item_id, $rolling_branch_id, $user_id, 'Standard') as $row) {
        if (strcasecmp(trim((string)$row['unit_type_name']), trim($unit_type)) === 0) {
            $available += (float)($row['received_quantity'] ?? 0);
        }
    }
    return $available;
}

ensureRollingTransferTablesForOrderProduct($conn);

// Get ALL active items from the Rolling user's branch/source branch.
// Do NOT inner join received stocks here; items with zero/blank Rolling stock must still appear.
$items = [];
$item_branch_filter_id = (int)($source_branch_id > 0 ? $source_branch_id : $branch_id);
if ($items_branch_column_exists && $item_branch_filter_id > 0) {
    $items_query = "SELECT i.item_id, i.item_code, i.item_name, i.description, COALESCE(NULLIF(i.category, ''), 'Uncategorized') AS category,
                    0 AS stock, 0 AS stock_in_default_uom, i.unit_type, i.unit_price, i.price_case, i.price_inner_pack,
                    i.price_box, i.price_carton, i.reorder_level, i.status, i.product_image_url, b.branch_name
                    FROM items i
                    LEFT JOIN branches b ON i.branch_id = b.branch_id
                    WHERE i.status = 'active' AND i.branch_id = " . intval($item_branch_filter_id) . "
                    ORDER BY category ASC, i.item_name ASC";
} else {
    $items_query = "SELECT i.item_id, i.item_code, i.item_name, i.description, COALESCE(NULLIF(i.category, ''), 'Uncategorized') AS category,
                    0 AS stock, 0 AS stock_in_default_uom, i.unit_type, i.unit_price, i.price_case, i.price_inner_pack,
                    i.price_box, i.price_carton, i.reorder_level, i.status, i.product_image_url,
                    '' AS branch_name
                    FROM items i
...
if ($items_result) {
    ...
    error_log("Rolling all-items query error: " . $conn->error);
}

$all_items_unit_types = [];
$rolling_received_by_item = [];
foreach ($items as $item) {
    $item_id = (int)$item['item_id'];
    $all_units = fetchAllProductUnits($conn, $item_id, 'Standard');
    $item_units = [];
    foreach ($all_units as $unit_row) {
        $uom_name = trim((string)($unit_row['unit_type_name'] ?? ''));
        if ($uom_name === '') continue;
        $item_units[$uom_name] = (int)($unit_row['quantity_smallest_pack'] ?? 1);
    }
    $all_items_unit_types[$item_id] = $item_units;
    $rolling_received_by_item[$item_id] = fetchRollingReceivedUnits($conn, $item_id, (int)$branch_id, (int)$user_id, 'Standard');
}
...
// Build inventory array: all items visible, but stock is blank/zero unless received by this Rolling account.
$inventory_data = [];
foreach ($items as $item) {
    $item_id = (int)$item['item_id'];
    $all_units = fetchAllProductUnits($conn, $item_id, 'Standard');
    $received_units = $rolling_received_by_item[$item_id] ?? [];

    $received_map = [];
    $total_received_qty = 0.0;
    foreach ($received_units as $received_row) {
        $uom_key = strtolower(trim((string)($received_row['unit_type_name'] ?? '')));
        if ($uom_key === '') continue;
        $qty = (float)($received_row['received_quantity'] ?? 0);
        $received_map[$uom_key] = ($received_map[$uom_key] ?? 0) + $qty;
        $total_received_qty += $qty;
    }

    $first_unit = $item['unit_type'] ?: 'Piece';
    $first_multiplier = 1;
    $first_unit_id = 0;
    $first_price = (float)($item['unit_price'] ?? 0);
    if (!empty($all_units)) {
        $first_unit = $all_units[0]['unit_type_name'] ?: $first_unit;
        $first_multiplier = max(1, (int)($all_units[0]['quantity_smallest_pack'] ?? 1));
        $first_unit_id = (int)($all_units[0]['unit_type_id'] ?? 0);
        $first_price = (float)($all_units[0]['unit_price'] ?? $first_price);
    }
    ...
    $inventory_data[] = [
        ...
        'default_stock' => (float)$total_received_qty,
        'stock_smallest' => (float)$total_received_qty,
        'stock_in_default_uom' => (float)$total_received_qty,
        'raw_stock' => (float)$total_received_qty,
        'has_received_stock' => $total_received_qty > 0,
        'received_by_uom' => $received_map,
        'unit_type' => $first_unit,
        'default_unit_type_name' => $first_unit,
        'default_unit_multiplier' => $first_multiplier,
        'default_unit_type_id' => $first_unit_id,
        'received_units' => $received_units,
        'registered_units' => array_map(function($unit_row) use ($received_map) {
            $unit_name = trim((string)($unit_row['unit_type_name'] ?? 'Piece'));
            return [
                'unit_type_id' => (int)($unit_row['unit_type_id'] ?? 0),
                'unit_type_name' => $unit_name,
                'uom_initial' => trim((string)($unit_row['uom_initial'] ?? '')),
                'unit_price' => (float)($unit_row['unit_price'] ?? 0),
                'quantity_smallest_pack' => max(1, (int)($unit_row['quantity_smallest_pack'] ?? 1)),
                'is_default_uom' => (int)($unit_row['is_default_uom'] ?? 0),
                'received_quantity' => (float)($received_map[strtolower($unit_name)] ?? 0)
            ];
        }, $all_units),
        'image' => $item['product_image_url'] ?? null
    ];
}
$inventory_json = json_encode($inventory_data, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP);
$unit_conversions_json = json_encode($all_items_unit_types, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'get_product_unit_types') {
    ...
    try {
        ...
        // SAME UoM BUTTON SOURCE AS SALES ORDERPRODUCT:
        // Get ALL registered UoM rows of this item from item_unit_pricing + unit_types.
        // Then attach Rolling received stock per exact UoM only for highlight/stock display.
        $unit_types_query = "
            SELECT
                ranked_unit_types.unit_type_id,
                ranked_unit_types.unit_type_name,
                ranked_unit_types.uom_initial,
                ranked_unit_types.unit_price,
                ranked_unit_types.quantity_smallest_pack,
                ranked_unit_types.is_default_uom,
                COALESCE(rstock.received_quantity, 0) AS received_quantity
            FROM (
                SELECT
                    ut.unit_type_id,
                    ut.unit_type_name,
                    ...
                    ROW_NUMBER() OVER (
                        ...
                        ORDER BY
                            CASE WHEN iup.price_level = ? THEN 0 ELSE 1 END,
                            CASE
                ...
                JOIN unit_types ut ON iup.unit_type_id = ut.unit_type_id
                WHERE iup.item_id = ?
                  AND (ut.status IS NULL OR ut.status = '' OR LOWER(TRIM(ut.status)) = 'active')
                  AND (iup.price_level = ? OR iup.price_level = 'Standard' OR iup.price_level IS NULL OR iup.price_level = '')
            ) ranked_unit_types
            LEFT JOIN (
                SELECT
                    COALESCE(rti.unit_type_id, ut2.unit_type_id, 0) AS unit_type_id,
                    COALESCE(NULLIF(TRIM(rti.unit_type_name), ''), ut2.unit_type_name, '') AS unit_type_name,
                    SUM(COALESCE(rti.quantity_received, 0)) AS received_quantity
                FROM rolling_inventory_transfer_items rti
                INNER JOIN rolling_inventory_transfers rt
                    ON rt.transfer_id = rti.transfer_id
                ...
                LEFT JOIN unit_types ut2 ON ut2.unit_type_id = rti.unit_type_id
                WHERE rti.item_id = ?
                GROUP BY COALESCE(rti.unit_type_id, ut2.unit_type_id, 0), COALESCE(NULLIF(TRIM(rti.unit_type_name), ''), ut2.unit_type_name, '')
            ) rstock
                ON (rstock.unit_type_id > 0 AND rstock.unit_type_id = ranked_unit_types.unit_type_id)
                OR (LOWER(TRIM(rstock.unit_type_name)) = LOWER(TRIM(ranked_unit_types.unit_type_name)))
            WHERE ranked_unit_types.rn = 1
            ORDER BY ranked_unit_types.is_default_uom DESC, ranked_unit_types.quantity_smallest_pack ASC, ranked_unit_types.unit_type_name ASC
        ";
        
        $unit_types_stmt = $conn->prepare($unit_types_query);
        if (!$unit_types_stmt) {
            throw new Exception('Failed to load unit types: ' . $conn->error);
        }
        $unit_types_stmt->bind_param('sisiii', $price_level, $product_id, $price_level, $branch_id, $user_id, $product_id);
        $unit_types_stmt->execute();
        $unit_types_result = $unit_types_stmt->get_result();
        $unit_types = $unit_types_result ? $unit_types_result->fetch_all(MYSQLI_ASSOC) : [];
        $unit_types_stmt->close();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'get_product_details') {
    ...
    try {
        ...
        $all_units = fetchAllProductUnits($conn, $product_id, $price_level);
        $received_units = fetchRollingReceivedUnits($conn, $product_id, (int)$branch_id, (int)$user_id, $price_level);

        $product_query = "SELECT i.item_id, i.item_code, i.item_name, i.description, i.category, 
                        i.unit_type, i.unit_price, i.price_case, i.price_inner_pack, 
                        i.price_box, i.price_carton, i.reorder_level, i.status,
                        i.product_image_url, b.branch_name
                        FROM items i
        ...
        $received_map = [];
        $total_received_qty = 0.0;
        foreach ($received_units as $unit_row) {
            $uom_key = strtolower(trim((string)($unit_row['unit_type_name'] ?? '')));
            if ($uom_key === '') continue;
            $qty = (float)($unit_row['received_quantity'] ?? 0);
            $received_map[$uom_key] = ($received_map[$uom_key] ?? 0) + $qty;
            $total_received_qty += $qty;
        }
        $first_price = (float)($product['unit_price'] ?? 0);
        $unit_types = [];
        foreach ($all_units as $idx => $unit_row) {
            $uom_name = trim((string)($unit_row['unit_type_name'] ?? 'Piece'));
            if ($idx === 0) $first_price = (float)($unit_row['unit_price'] ?? $first_price);
            $unit_types[] = [
                'unit_type_id' => (int)($unit_row['unit_type_id'] ?? 0),
                'unit_type_name' => $uom_name,
                'uom_initial' => trim((string)($unit_row['uom_initial'] ?? '')),
                'unit_price' => (float)($unit_row['unit_price'] ?? 0),
                'quantity_smallest_pack' => (int)($unit_row['quantity_smallest_pack'] ?? 1),
                'is_default_uom' => (int)($unit_row['is_default_uom'] ?? 0),
                'received_quantity' => (float)($received_map[strtolower($uom_name)] ?? 0)
            ];
        }

        $product['stock'] = $total_received_qty;
        $product['stock_in_default_uom'] = $total_received_qty;
        $product['unit_price'] = $first_price;
        $product['unit_type'] = $unit_types[0]['unit_type_name'] ?? ($product['unit_type'] ?? 'Piece');
        ...
        $images = $images_result ? $images_result->fetch_all(MYSQLI_ASSOC) : [];
        $images_stmt->close();
        ...
        $history_query = "SELECT so.so_number, so.order_date, c.customer_name, so.order_status,
                        ...
                        WHERE soi.item_id = ? AND so.branch_id = ? AND so.created_by = ?
                        ORDER BY so.order_date DESC LIMIT 50";
        $history_stmt = $conn->prepare($history_query);
        $history_stmt->bind_param('iii', $product_id, $branch_id, $user_id);
        $history_stmt->execute();
        $history_result = $history_stmt->get_result();
        $order_history = $history_result ? $history_result->fetch_all(MYSQLI_ASSOC) : [];
        $history_stmt->close();
...
// ========== ROLLING DROP-OFF COLLECTION HELPERS ==========
function rollingTableExists(mysqli $conn, string $table): bool {
    $safe = $conn->real_escape_string($table);
    $res = $conn->query("SHOW TABLES LIKE '{$safe}'");
    return $res && $res->num_rows > 0;
}

function rollingColumnExists(mysqli $conn, string $table, string $column): bool {
    if (!rollingTableExists($conn, $table)) return false;
...
function rollingEnsureDropOffTables(mysqli $conn): void {
    if (!rollingTableExists($conn, 'invoices')) {
    ...
    if (!rollingTableExists($conn, 'payments')) {
    ...
    if (!rollingTableExists($conn, 'collection_records')) {
        $conn->query("CREATE TABLE IF NOT EXISTS `collection_records` (
            `record_id` int(11) NOT NULL AUTO_INCREMENT,
            `invoice_id` int(11) NOT NULL DEFAULT 0,
            `customer_id` int(11) NOT NULL DEFAULT 0,
            `branch_id` int(11) NOT NULL DEFAULT 0,
            `collector_user_id` int(11) NOT NULL DEFAULT 0,
            `payment_method` varchar(40) NOT NULL DEFAULT 'cash',
            `amount` decimal(12,2) NOT NULL DEFAULT 0.00,
...
function rollingAddColumnIfMissing(mysqli $conn, string $table, string $column, string $definition): void {
    if (rollingTableExists($conn, $table) && !rollingColumnExists($conn, $table, $column)) {
        $safeTable = preg_replace('/[^A-Za-z0-9_]/', '', $table);
        @$conn->query("ALTER TABLE `{$safeTable}` ADD COLUMN {$definition}");
    }
}

function rollingInsertDynamic(mysqli $conn, string $table, array $fields): int {
    $safeTable = preg_replace('/[^A-Za-z0-9_]/', '', $table);
    $cols = [];
    $placeholders = [];
    $types = '';
    ...
    foreach ($fields as $col => $val) {
        if (!rollingColumnExists($conn, $safeTable, $col)) continue;
    ...
    if (!$stmt) throw new Exception('Failed to prepare insert for ' . $safeTable . ': ' . $conn->error);
    if (!empty($values)) $stmt->bind_param($types, ...$values);
    if (!$stmt->execute()) throw new Exception('Failed to insert ' . $safeTable . ': ' . $stmt->error);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'submit_order') {
    ...
    try {
        ...
        $order_type = isset($_POST['order_type']) ? strtolower(trim((string)$_POST['order_type'])) : 'delivery';
        $order_type = str_replace('-', '_', $order_type);
        if ($order_type === 'dropoff') {
            $order_type = 'drop_off';
        }
        if (!in_array($order_type, ['delivery', 'drop_off'], true)) {
            $order_type = 'delivery';
        }
        $dropoff_payment_method = isset($_POST['dropoff_payment_method']) ? trim((string)$_POST['dropoff_payment_method']) : 'cash';
        $dropoff_payment_amount = isset($_POST['dropoff_payment_amount']) ? (float)$_POST['dropoff_payment_amount'] : 0;
        $dropoff_reference_number = isset($_POST['dropoff_reference_number']) ? trim((string)$_POST['dropoff_reference_number']) : '';
        $dropoff_bank_name = isset($_POST['dropoff_bank_name']) ? trim((string)$_POST['dropoff_bank_name']) : '';
        $dropoff_bank_branch = isset($_POST['dropoff_bank_branch']) ? trim((string)$_POST['dropoff_bank_branch']) : '';
        $dropoff_check_number = isset($_POST['dropoff_check_number']) ? trim((string)$_POST['dropoff_check_number']) : '';
        $dropoff_check_date = isset($_POST['dropoff_check_date']) ? trim((string)$_POST['dropoff_check_date']) : null;
        $dropoff_cash_tendered = isset($_POST['dropoff_cash_tendered']) ? (float)$_POST['dropoff_cash_tendered'] : 0;
        $dropoff_cash_change = max(0, $dropoff_cash_tendered - $dropoff_payment_amount);
        ...
        $branch_id = isset($_SESSION['rolling_branch_id']) ? (int)$_SESSION['rolling_branch_id'] : (isset($_SESSION['branch_id']) ? (int)$_SESSION['branch_id'] : 0);
        $view_all_branches = false; // Rolling accounts are restricted to their assigned branch
        ...
        rollingAddColumnIfMissing($conn, 'sales_orders', 'fulfillment_type', "`fulfillment_type` varchar(30) NOT NULL DEFAULT 'delivery' AFTER `order_status`");
        rollingAddColumnIfMissing($conn, 'sales_orders', 'payment_status', "`payment_status` varchar(30) NOT NULL DEFAULT 'unpaid' AFTER `fulfillment_type`");
        $has_fulfillment_type_column = rollingColumnExists($conn, 'sales_orders', 'fulfillment_type');
        $has_payment_status_column = rollingColumnExists($conn, 'sales_orders', 'payment_status');
        
        // Drop Off orders should not go through picklist/trip ticket/delivery assignment.
        // They are delivered immediately, BUT any collected amount is NOT considered paid yet.
        // It must first be saved as a collection record and remitted/approved by Branch Admin.
        $dropoff_has_collection = ($order_type === 'drop_off' && $dropoff_payment_amount > 0);
        $initial_order_status = ($order_type === 'drop_off') ? 'delivered' : 'pending';
        $initial_payment_status = ($dropoff_has_collection) ? 'for_remittance' : 'unpaid';

        $insert_fields = ['so_number', 'customer_id', 'branch_id', 'order_date', 'total_amount', 'order_status', 'created_by'];
        $insert_placeholders = ['?', '?', '?', '?', '?', '?', '?'];
        $insert_types = 'siisdss';
        $insert_values = [$so_number, $customer_id, $branch_id, $order_date, $total_amount, $initial_order_status, $user_id];
        ...
        if ($has_fulfillment_type_column) {
            ...
            $insert_placeholders[] = '?';
            $insert_types .= 's';
            $insert_values[] = $order_type;
        }
        if ($has_payment_status_column) {
            $insert_fields[] = 'payment_status';
            $insert_placeholders[] = '?';
            $insert_types .= 's';
            $insert_values[] = $initial_payment_status;
        }
        ...
        foreach ($items_data as $item) {
            ...
            // Rolling can still order even if there is no received stock yet.
            // Do not validate against Branch Admin stock here.
            ...
            // Record a Rolling stock-out transaction for audit/reporting.
            // IMPORTANT: Do NOT update the items table here because that contains Branch Admin totals.
            $rolling_out_ref = 'rolling_sales_order_out';
            $out_qty = -abs((float)$quantity);
            $stock_out_stmt = $conn->prepare("INSERT INTO inventory_transactions (branch_id, item_id, transaction_type, quantity_changed, reference_type, reference_id, created_by, created_at) VALUES (?, ?, 'out', ?, ?, ?, ?, NOW())");
            if ($stock_out_stmt) {
                $stock_out_stmt->bind_param('iidsii', $branch_id, $item_id, $out_qty, $rolling_out_ref, $so_id, $user_id);
            ...
            error_log("  - Rolling order was allowed regardless of available received stock.");
            error_log("  - Ordered: $quantity $unit_type");
            error_log("========== ITEM PROCESSING COMPLETE ==========");
        }

        $invoice_id_for_order = 0;
        $payment_id_for_order = 0;

        // Always create an invoice for BOTH Delivery and Drop Off orders.
        // Delivery: Sales Order stays pending/normal flow, but invoice record is already available.
        // Drop Off: Sales Order is delivered, no picklist/trip ticket/delivery flow; invoice + collection record are created for remittance.
        rollingEnsureDropOffTables($conn);
        rollingAddColumnIfMissing($conn, 'invoices', 'branch_id', "`branch_id` int(11) NOT NULL DEFAULT 0 AFTER `customer_id`");
        rollingAddColumnIfMissing($conn, 'invoices', 'amount_paid', "`amount_paid` decimal(12,2) NOT NULL DEFAULT 0.00 AFTER `total_amount`");
        rollingAddColumnIfMissing($conn, 'invoices', 'balance', "`balance` decimal(12,2) NOT NULL DEFAULT 0.00 AFTER `amount_paid`");
        rollingAddColumnIfMissing($conn, 'payments', 'branch_id', "`branch_id` int(11) NOT NULL DEFAULT 0 AFTER `customer_id`");
        rollingAddColumnIfMissing($conn, 'payments', 'so_id', "`so_id` int(11) DEFAULT NULL AFTER `invoice_id`");

        $dropoff_collected_amount = ($order_type === 'drop_off') ? max(0, (float)$dropoff_payment_amount) : 0.00;
        if ($dropoff_collected_amount > ($total_amount + 0.01)) {
            throw new Exception('Collected amount cannot be greater than order total.');
        }

        // IMPORTANT: Do not mark invoice as paid/partial here.
        // Cash/check/online transfer collected by Rolling is only a COLLECTION until Remit All is approved by Branch Admin.
        $invoice_paid_amount = 0.00;
        $invoice_balance = (float)$total_amount;
        $invoice_status = 'pending';

        if ($order_type === 'drop_off') {
            if ($dropoff_collected_amount > 0 && !in_array($dropoff_payment_method, ['cash', 'check', 'online_transfer'], true)) {
                throw new Exception('Invalid drop off payment method.');
            }
            if ($dropoff_collected_amount > 0 && $dropoff_payment_method === 'check') {
                if ($dropoff_bank_name === '' || $dropoff_check_number === '' || empty($dropoff_check_date)) {
                    throw new Exception('Bank name, check number, and check date are required for check payment.');
                }
                $dropoff_reference_number = $dropoff_check_number;
            }
            if ($dropoff_collected_amount > 0 && $dropoff_payment_method === 'online_transfer') {
                if ($dropoff_reference_number === '' || $dropoff_bank_name === '') {
                    throw new Exception('Reference number and bank/wallet are required for online transfer.');
                }
            }
        }

        $invoice_number = 'INV-' . date('Ymd') . '-' . substr((string)time(), -5) . rand(10, 99);
        $invoice_id_for_order = rollingInsertDynamic($conn, 'invoices', [
            ...
            'amount_paid' => (float)$invoice_paid_amount,
            'balance' => (float)$invoice_balance,
            'status' => $invoice_status,
            'created_at' => '__NOW__',
            'updated_at' => '__NOW__'
        ]);

        // Save Drop Off collection for Remit All approval.
        // Do NOT insert into payments here and do NOT mark invoice paid here.
        // Branch Admin approval of the remittance is the only step that should convert this into paid/payment records.
        if ($order_type === 'drop_off' && $dropoff_collected_amount > 0) {
            rollingInsertDynamic($conn, 'collection_records', [
                'invoice_id' => (int)$invoice_id_for_order,
                'customer_id' => (int)$customer_id,
                'branch_id' => (int)$branch_id,
                'collector_user_id' => (int)$user_id,
                'payment_method' => $dropoff_payment_method,
                'amount' => (float)$dropoff_collected_amount,
                'collection_date' => '__NOW__',
                'reference_number' => $dropoff_reference_number,
                'check_date' => $dropoff_check_date,
                'bank_name' => $dropoff_bank_name,
                'bank_branch' => $dropoff_bank_branch,
                'check_number' => $dropoff_check_number,
                'cash_tendered' => $dropoff_payment_method === 'cash' ? (float)$dropoff_cash_tendered : null,
                'cash_change' => $dropoff_payment_method === 'cash' ? (float)$dropoff_cash_change : null,
                'notes' => 'Drop off collection from Rolling order ' . $so_number . '. For remittance approval before marking invoice as paid.',
                'status' => 'collected',
                'created_at' => '__NOW__'
            ]);
        }
        ...
        echo json_encode([
            ...
            'invoice_id' => $invoice_id_for_order ?? 0,
            'payment_id' => 0,
            'collection_status' => ($order_type === 'drop_off' && $dropoff_collected_amount > 0) ? 'for_remittance' : 'none'
        ]);
        exit;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'cancel_order') {
    ...
    try {
        ...
        foreach ($order_items as $item) {
            ...
            // Restore Rolling order stock by recording a reverse transaction only.
            // Do NOT update items table because it may contain Branch Admin totals.
            $restore_ref = 'rolling_sales_order_cancel_restore';
            $restore_qty = abs((float)$quantity);
            $restore_tx = $conn->prepare("INSERT INTO inventory_transactions (branch_id, item_id, transaction_type, quantity_changed, reference_type, reference_id, created_by, created_at) VALUES (?, ?, 'in', ?, ?, ?, ?, NOW())");
            if ($restore_tx) {
                $restore_tx->bind_param('iidsii', $branch_id, $item_id, $restore_qty, $restore_ref, $order_id, $user_id);
                $restore_tx->execute();
                $restore_tx->close();
            }
...
<head>
    ...
    <title>Order Product - Rolling Account</title>
    ...
    <style>
        * {
        ...
        /* Highlight UoM buttons that have received stock for this Rolling account */
        .unit-btn.has-stock {
            border-color: #047857;
            background: #ecfdf5;
            color: #047857;
            box-shadow: inset 0 -2px 0 rgba(4, 120, 87, 0.28);
            position: relative;
        }

        .unit-btn.has-stock::after {
            content: '';
            position: absolute;
            top: 2px;
            right: 2px;
        ...
        .unit-dropdown option.has-stock-option {
            font-weight: 700;
        }
...
@media (max-width: 480px) {
    ...
    .review-qty-input {
        ...
        /* Rolling mobile more dropdown + responsive fixes */
        ...
        .more-dropdown .dropdown-item:hover { background: #F5F5F5; color: #047857; }
        .more-dropdown .logout-item { color: #dc3545; }
        .more-dropdown .logout-item i { color: #dc3545; }
        .more-dropdown .dropdown-divider { height: 1px; background: #e5e7eb; margin: 0; }

        @media (max-width: 992px) {
            body { padding-bottom: 78px; }
            .main-content { margin-left: 0 !important; padding: 12px !important; }
            .navbar-top { gap: .75rem; padding: 12px 14px; border-radius: 14px; align-items: flex-start; }
            .navbar-top .page-title h2 { font-size: 1.15rem; margin-bottom: 2px; }
            .navbar-top .page-title p { font-size: .78rem; margin-bottom: 0; }
            .mobile-nav { display: block; }
            .mobile-nav .nav { display: flex; align-items: center; justify-content: space-around; gap: 2px; }
            .mobile-nav .nav-item { flex: 1; min-width: 0; text-align: center; }
            .mobile-nav .nav-link { padding: 6px 2px; font-size: .68rem; gap: 2px; }
            .mobile-nav .nav-link i { font-size: 1.16rem; margin-bottom: 2px; }
            .mobile-nav .nav-link span { display: block; line-height: 1.1; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
            .mobile-nav .nav-link.active { color: #047857; font-weight: 800; }
            .more-dropdown { right: 4px; min-width: 215px; }
            .table-responsive { overflow-x: auto; -webkit-overflow-scrolling: touch; }
            .table { min-width: 760px; }
            .modal-dialog { margin: .5rem; }
            .modal-content { border-radius: 16px !important; }
            .product-card, .card, .section-card, .order-summary, .customer-selection { max-width: 100%; }
        }
        @media (max-width: 576px) {
            .main-content { padding: 10px !important; }
            .navbar-top { margin-bottom: 14px; }
            .mobile-nav { padding: 7px 8px; }
            .mobile-nav .nav-link { font-size: .62rem; }
            .mobile-nav .nav-link i { font-size: 1.08rem; }
            .more-dropdown { min-width: 205px; right: 0; }
        }
...
<body>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="alert bg-light">
                        <div class="row g-2">
                            <div class="col-md-6">
                                <label class="form-check border rounded p-3 h-100" style="cursor:pointer;">
                                    <input class="form-check-input me-2" type="radio" name="reviewOrderType" id="orderTypeDelivery" value="delivery" checked onchange="toggleReviewOrderType()">
                                    <strong><i class="bi bi-truck me-1"></i> Delivery</strong>
                                    <div class="text-muted small mt-1">Normal order for delivery processing.</div>
                                </label>
                            </div>
                            <div class="col-md-6">
                                <label class="form-check border rounded p-3 h-100" style="cursor:pointer;">
                                    <input class="form-check-input me-2" type="radio" name="reviewOrderType" id="orderTypeDropOff" value="drop_off" onchange="toggleReviewOrderType()">
                                    <strong><i class="bi bi-bag-check me-1"></i> Drop Off</strong>
                                    <div class="text-muted small mt-1">Direct drop off with optional collection.</div>
                                </label>
                            </div>
                        </div>

                        <div id="dropOffCollectionSection" style="display:none;" class="mt-3 border-top pt-3">
                            <div class="d-flex justify-content-between align-items-center mb-2">
                                <h6 class="mb-0"><i class="bi bi-cash-stack me-1"></i> Drop Off Collection</h6>
                                <small class="text-muted">Same style as driver collection</small>
                            </div>
                            <div class="row g-3">
                                <div class="col-md-4">
                                    <label class="form-label">Payment Method</label>
                                    <select class="form-select" id="dropoffPaymentMethod" onchange="toggleDropoffPaymentFields()">
                                ...
                                <div class="col-md-4">
                                    ...
                                    <input type="number" class="form-control" id="dropoffPaymentAmount" min="0" step="0.01" placeholder="0.00" oninput="updateDropoffCashChange()">
                                </div>
                                <div class="col-md-4 dropoff-cash-fields">
                                    <label class="form-label">Cash Tendered</label>
                                    <input type="number" class="form-control" id="dropoffCashTendered" min="0" step="0.01" placeholder="0.00" oninput="updateDropoffCashChange()">
                                </div>
                                <div class="col-md-4 dropoff-cash-fields">
                                    <label class="form-label">Change</label>
                                    <input type="text" class="form-control" id="dropoffCashChange" value="₱0.00" readonly>
                                </div>
                                <div class="col-md-4 dropoff-check-fields" style="display:none;">
                                    <label class="form-label">Bank Name</label>
                                    <input type="text" class="form-control" id="dropoffCheckBankName" placeholder="Bank name">
                                </div>
                                <div class="col-md-4 dropoff-check-fields" style="display:none;">
                                    <label class="form-label">Bank Branch</label>
                                    <input type="text" class="form-control" id="dropoffCheckBankBranch" placeholder="Branch">
                                </div>
                                <div class="col-md-4 dropoff-check-fields" style="display:none;">
                                    <label class="form-label">Check Number</label>
                                    <input type="text" class="form-control" id="dropoffCheckNumber" placeholder="Check no.">
                                </div>
                                <div class="col-md-4 dropoff-check-fields" style="display:none;">
                                    <label class="form-label">Check Date</label>
                                    <input type="date" class="form-control" id="dropoffCheckDate">
                                </div>
                                <div class="col-md-6 dropoff-transfer-fields" style="display:none;">
                                    <label class="form-label">Bank / Wallet</label>
                                    <input type="text" class="form-control" id="dropoffTransferBank" placeholder="Bank or wallet">
                                </div>
                                <div class="col-md-6 dropoff-transfer-fields" style="display:none;">
                                    <label class="form-label">Reference Number</label>
                                    <input type="text" class="form-control" id="dropoffReferenceNumber" placeholder="Reference no.">
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <div class="mobile-nav" id="mobileNav">
        <ul class="nav">
            ...
            <li class="nav-item">
                <a class="nav-link active" href="customer_orderproduct.php">
                    <i class="bi bi-people"></i><span>Orders</span>
                </a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="collections.php">
            ...
            <li class="nav-item dropdown-more" id="moreDropdown">
                ...
                <div class="more-dropdown" id="moreDropdownMenu">
                    ...
                    <a href="#" class="dropdown-item logout-item" onclick="showProfileModal(); return false;"><i class="bi bi-box-arrow-right"></i><span>Logout</span></a>
                </div>
            </li>
        </ul>
    </div>
    ...
    <script>
        ...
        // Preload ALL registered UoM from PHP so every UoM button appears immediately.
        // AJAX refresh is still allowed, but empty AJAX responses will not erase these buttons.
        inventory.forEach(product => {
            if (Array.isArray(product.registered_units) && product.registered_units.length > 0) {
                productUnitTypes[product.id] = product.registered_units;
                productUnitConversions[product.id] = productUnitConversions[product.id] || {};
                product.registered_units.forEach(ut => {
                    if (ut && ut.unit_type_name) {
                        productUnitConversions[product.id][ut.unit_type_name] = parseInt(ut.quantity_smallest_pack || 1, 10) || 1;
                    }
                });
            }
        });
        ...
        function loadProductUnitTypes(productId, priceLevel = 'Standard') {
            return new Promise((resolve) => {
                ...
                fetch('orderproduct.php', { method: 'POST', body: formData })
                    .then(response => response.json())
                    .then(data => {
                        if (data.success && Array.isArray(data.unit_types)) {
                            const uniqueUnitTypes = [];
                            const seenUnitTypes = new Set();

                            data.unit_types.forEach(ut => {
                                const unitName = (ut.unit_type_name || '').trim();
                                if (!unitName || seenUnitTypes.has(unitName.toLowerCase())) {
                                    return;
                                }
                                seenUnitTypes.add(unitName.toLowerCase());
                                ut.unit_type_name = unitName;
                                uniqueUnitTypes.push(ut);
                            });

                            // Never overwrite the preloaded registered UoMs with an empty response.
                            if (uniqueUnitTypes.length > 0) {
                                productUnitTypes[productId] = uniqueUnitTypes;
                            }

                            const finalUnitTypes = productUnitTypes[productId] || [];
                            const defaultUnit = finalUnitTypes.find(ut => parseFloat(ut.received_quantity || 0) > 0) || finalUnitTypes.find(ut => parseInt(ut.is_default_uom || 0) === 1) || finalUnitTypes[0];
                            if (defaultUnit) {
                                activeUnitTypes[productId] = defaultUnit.unit_type_name;
                            }
                            
                            // Store conversion using quantity_smallest_pack from database
                            productUnitConversions[productId] = productUnitConversions[productId] || {};
                            finalUnitTypes.forEach(ut => {
        ...
        async function init() {
            ...
            inventory.forEach(product => {
                ...
                if (unitTypes.length > 0) {
                    const defaultUnit = unitTypes.find(ut => parseFloat(ut.received_quantity || 0) > 0) || unitTypes.find(ut => parseInt(ut.is_default_uom || 0) === 1) || unitTypes[0];
                    activeUnitTypes[product.id] = defaultUnit.unit_type_name;
                } else {
                    activeUnitTypes[product.id] = product.unit_type || product.default_unit_type_name || 'Piece';
                }
            });
        ...
        // Get available Rolling stock for the exact selected UoM.
        // Items with no received stock still appear in the catalog and can still be ordered.
        function getAvailableStockForUnit(productId, unitType) {
            const p = inventory.find(p => p.id === productId);
            if (!p) return null;
            const key = String(unitType || '').trim().toLowerCase();
            const receivedMap = p.received_by_uom || {};
            if (!key || !Object.prototype.hasOwnProperty.call(receivedMap, key)) {
                return null;
            }

            const receivedQty = parseFloat(receivedMap[key] || 0);
            if (receivedQty <= 0) return null;

            const inCartQty = cart
                .filter(i => i.id === productId && String(i.unit_type || '').trim().toLowerCase() === key)
                .reduce((t, i) => t + (parseFloat(i.quantity) || 0), 0);

            return receivedQty - inCartQty;
        }

        // Backward-compatible helper: total received quantity across all received UoMs.
        function getAvailableStockSmallest(productId) {
            const p = inventory.find(p => p.id === productId);
            if (!p) return 0;
            return parseFloat(p.stock_smallest || 0);
        }
        ...
        // For Rolling, stock display must be the exact received quantity of the selected UoM only.
        function getConvertedStock(productId, unitType) {
            return getAvailableStockForUnit(productId, unitType);
        }
        ...
        function renderFilteredProducts(filteredInventory) {
            ...
            filteredInventory.forEach(p => {
                ...
                // Initialize active unit using registered UoM only.
                if (!activeUnitTypes[p.id] || (unitTypes.length > 0 && !unitTypes.some(ut => ut.unit_type_name === activeUnitTypes[p.id]))) {
                    if (unitTypes.length > 0) {
                        const defaultUnit = unitTypes.find(ut => parseInt(ut.is_default_uom || 0) === 1) || unitTypes[0];
                        activeUnitTypes[p.id] = defaultUnit.unit_type_name;
                    } else {
                        activeUnitTypes[p.id] = p.unit_type || p.default_unit_type_name || 'Piece';
                    }
                }
                
                const activeUnit = activeUnitTypes[p.id];
                const convertedStock = getConvertedStock(p.id, activeUnit);
                const hasReceivedStockForUnit = convertedStock !== null && convertedStock > 0;
                const low = hasReceivedStockForUnit && convertedStock < 5;
                const out = !hasReceivedStockForUnit;
                ...
                if (unitTypes.length > 0) {
                    unitTypes.forEach(ut => {
                        const unitName = String(ut.unit_type_name || '').trim();
                        const shortLabel = getUomInitial(ut);
                        const displayName = getUomDisplayName(ut);
                        const isActive = activeUnitTypes[p.id] === unitName ? 'active' : '';
                        const receivedQty = parseFloat(ut.received_quantity || 0);
                        const hasStockClass = receivedQty > 0 ? 'has-stock' : '';
                        const stockTitle = receivedQty > 0 ? `${displayName} has stock: ${formatStockDisplay(receivedQty, displayName)}` : `${displayName} has no received stock`;
                        const optionLabel = receivedQty > 0 ? `${displayName} • ${formatStockDisplay(receivedQty, displayName)}` : displayName;
                        const safeUnitName = escapeHtml(unitName);
                        const safeTitle = escapeHtml(stockTitle);
                        
                        unitButtonsHtml += `<button class="unit-btn ${hasStockClass} ${isActive}" title="${safeTitle}" data-product-id="${p.id}" data-unit-type="${safeUnitName}" data-has-stock="${receivedQty > 0 ? '1' : '0'}" onclick="event.stopPropagation(); setActiveUnit(${p.id}, '${escapeJs(unitName)}')">${shortLabel}</button>`;
                        unitDropdownOptions += `<option value="${safeUnitName}" ${receivedQty > 0 ? 'class="has-stock-option"' : ''} ${isActive ? 'selected' : ''}>${escapeHtml(optionLabel)}</option>`;
                    });
                } else {
                    // No registered UoM rows found. Show only the item's own saved UoM.
                    const fallbackUnit = (p.unit_type || p.default_unit_type_name || 'Piece').trim();
                    const shortLabel = fallbackUnit.length <= 2 ? fallbackUnit.toUpperCase() : fallbackUnit.substring(0, 2).toUpperCase();
                    activeUnitTypes[p.id] = fallbackUnit;
                    unitButtonsHtml += `<button class="unit-btn active" data-product-id="${p.id}" data-unit-type="${fallbackUnit}" onclick="event.stopPropagation(); setActiveUnit(${p.id}, '${escapeJs(fallbackUnit)}')">${shortLabel}</button>`;
                    unitDropdownOptions += `<option value="${fallbackUnit}" selected>${fallbackUnit}</option>`;
                }
                
                // Display stock only when this Rolling account actually received this exact UoM.
                const stockDisplay = hasReceivedStockForUnit ? formatStockDisplay(convertedStock, activeUnit) : '';
                ...
                html += `<tr id="row-${p.id}" onclick="showProductInfo(${p.id})" style="cursor: pointer;">
                    ...
                    <td class="product-cell">
                        <div class="product-info">
                            ...
                            <span id="stock-${p.id}" class="${hasReceivedStockForUnit ? 'stock-info' : 'stock-info text-muted'}">Stock: ${stockDisplay}</span>
        ...
        function getUnitLabelByName(pid, type) {
            const unitTypes = productUnitTypes[pid] || [];
            const matched = unitTypes.find(ut => String(ut.unit_type_name || '').trim() === String(type || '').trim());
            return matched ? getUomDisplayName(matched) : String(type || '');
        }
        ...
        function setActiveUnit(pid, type) {
            ...
            if (priceEl) {
                ...
                if (unitLabel) {
                    unitLabel.textContent = `/${getUnitLabelByName(pid, type)}`;
                }
            }
            ...
            if (mobilePriceInput && mobileUnitSpan) {
                ...
                mobileUnitSpan.textContent = `/${getUnitLabelByName(pid, type)}`;
            }
            ...
            document.querySelectorAll(`[data-product-id="${pid}"]`).forEach(btn => {
                ...
                if (btn.getAttribute('data-has-stock') === '1') {
        ...
        function validateQuantity(pid) {
            ...
            if (rawValue === '') {
                const desktopInp = document.getElementById(`qty-desktop-${pid}`);
                if (desktopInp && window.innerWidth <= 768) desktopInp.value = '';
                return 0;
            }
            ...
            const desktopInp = document.getElementById(`qty-desktop-${pid}`);
            if (desktopInp && window.innerWidth <= 768) desktopInp.value = v;

            return v;
        }

        function getEnteredQty(pid) {
            const mobileInp = document.getElementById(`qty-${pid}`);
            const desktopInp = document.getElementById(`qty-desktop-${pid}`);
            const mobileQty = mobileInp ? (parseInt(String(mobileInp.value || '0'), 10) || 0) : 0;
            const desktopQty = desktopInp ? (parseInt(String(desktopInp.value || '0'), 10) || 0) : 0;

            // Desktop has its own visible input. The old code read only qty-{id},
            // so desktop users were still getting "Please enter quantity".
            let qty = window.innerWidth > 768 ? desktopQty : mobileQty;
            if (qty <= 0) qty = Math.max(mobileQty, desktopQty);
            qty = Math.max(0, qty);

            if (mobileInp) mobileInp.value = qty;
            if (desktopInp) desktopInp.value = qty;
            return qty;
        }

        function clearEnteredQty(pid) {
            const mobileInp = document.getElementById(`qty-${pid}`);
            const desktopInp = document.getElementById(`qty-desktop-${pid}`);
            if (mobileInp) mobileInp.value = '0';
            if (desktopInp) desktopInp.value = '0';
        ...
        function addToCart(pid) {
            ...
            // Rolling can order even without received stock. Stock display remains blank when not received.
        ...
        function bulkAddToCart() {
            ...
            allProducts.forEach(row => {
                ...
                if (qty > 0) {
                    // Rolling can bulk add items even when stock is blank/zero.
                    // Get price from mobile input (if on mobile) or table price input
                    let price;
                    const mobilePriceInput = document.getElementById(`mobile-price-input-${pid}`);
                    if (mobilePriceInput && window.innerWidth <= 768) {
        ...
        function updateModalStock(productId) {
            ...
            const convertedStock = getConvertedStock(productId, unitType);
            
            document.getElementById('modalProductStock').innerHTML = (convertedStock !== null && convertedStock > 0)
                ? formatStockDisplay(convertedStock, unitType)
                : '';
        }
...
function updateReviewTotals() {
    ...
    showToast('Item removed from cart');
        ...
        function toggleReviewOrderType() {
            const dropOff = document.getElementById('orderTypeDropOff')?.checked;
            const section = document.getElementById('dropOffCollectionSection');
            if (section) section.style.display = dropOff ? 'block' : 'none';
            if (dropOff) {
                const totalText = (document.getElementById('reviewTotal')?.textContent || '0').replace(/[₱,]/g, '');
                const total = parseFloat(totalText) || 0;
                const amount = document.getElementById('dropoffPaymentAmount');
                if (amount && (!amount.value || parseFloat(amount.value) <= 0)) amount.value = total.toFixed(2);
            }
            toggleDropoffPaymentFields();
            updateDropoffCashChange();
        }

        function toggleDropoffPaymentFields() {
            const method = document.getElementById('dropoffPaymentMethod')?.value || 'cash';
            document.querySelectorAll('.dropoff-cash-fields').forEach(el => el.style.display = method === 'cash' ? '' : 'none');
            document.querySelectorAll('.dropoff-check-fields').forEach(el => el.style.display = method === 'check' ? '' : 'none');
            document.querySelectorAll('.dropoff-transfer-fields').forEach(el => el.style.display = method === 'online_transfer' ? '' : 'none');
        }

        function updateDropoffCashChange() {
            const amount = parseFloat(document.getElementById('dropoffPaymentAmount')?.value || 0);
            const tendered = parseFloat(document.getElementById('dropoffCashTendered')?.value || 0);
            const change = Math.max(0, tendered - amount);
            const changeEl = document.getElementById('dropoffCashChange');
        ...
        function submitOrder() {
            ...
            const selectedOrderType = document.getElementById('orderTypeDropOff')?.checked ? 'drop_off' : 'delivery';
            if (selectedOrderType === 'drop_off') {
                const method = document.getElementById('dropoffPaymentMethod')?.value || 'cash';
                const amount = parseFloat(document.getElementById('dropoffPaymentAmount')?.value || 0);
                if (amount < 0) {
                ...
                if (amount > totalAmount + 0.01) {
                    showToast('Payment amount cannot exceed total');
                    return;
                }
                if (method === 'check') {
                    if (!(document.getElementById('dropoffCheckBankName')?.value || '').trim() || !(document.getElementById('dropoffCheckNumber')?.value || '').trim() || !(document.getElementById('dropoffCheckDate')?.value || '').trim()) {
                        showToast('Please complete check payment details');
                        return;
                    }
                }
                if (method === 'online_transfer') {
                    if (!(document.getElementById('dropoffTransferBank')?.value || '').trim() || !(document.getElementById('dropoffReferenceNumber')?.value || '').trim()) {
                        showToast('Please complete online transfer details');
                        return;
                    }
                }
            }
            ...
            captureLocation().then(locationData => {
                const postData = {
                    ...
                    order_type: document.getElementById('orderTypeDropOff')?.checked ? 'drop_off' : 'delivery',
                    dropoff_payment_method: document.getElementById('dropoffPaymentMethod')?.value || 'cash',
                    dropoff_payment_amount: document.getElementById('dropoffPaymentAmount')?.value || '0',
                    dropoff_cash_tendered: document.getElementById('dropoffCashTendered')?.value || '0',
                    dropoff_bank_name: (document.getElementById('dropoffPaymentMethod')?.value === 'check') ? (document.getElementById('dropoffCheckBankName')?.value || '') : (document.getElementById('dropoffTransferBank')?.value || ''),
                    dropoff_bank_branch: document.getElementById('dropoffCheckBankBranch')?.value || '',
                    dropoff_check_number: document.getElementById('dropoffCheckNumber')?.value || '',
                    dropoff_check_date: document.getElementById('dropoffCheckDate')?.value || '',
                    dropoff_reference_number: document.getElementById('dropoffReferenceNumber')?.value || ''
                };
                ...
                const submitPromises = pendingOrders.map((order, index) => {
                    return new Promise((resolve) => {
                        setTimeout(() => {
                            ...
                            formData.append('order_type', order.order_type || 'delivery');
