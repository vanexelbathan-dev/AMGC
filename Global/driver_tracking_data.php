 ...
 * Returns latest locations for ALL drivers (active and offline/idle)
 * Shows last known position even for offline drivers
 */
...
try {
    ...
    $minutes_back = isset($_GET['minutes']) ? intval($_GET['minutes']) : 480; // Default to 8 hours for offline tracking
    $driver_id = isset($_GET['driver_id']) ? trim($_GET['driver_id']) : null; // Optional specific driver
    $show_all = isset($_GET['show_all']) && $_GET['show_all'] === 'true'; // Show all drivers including offline
    ...
    // Build the base query for ALL drivers with their latest location (online or offline)
    ...
    SELECT 
        ...
        CASE 
            WHEN dl.is_active = TRUE AND dl.last_updated >= DATE_SUB(NOW(), INTERVAL 1 MINUTE) THEN 'LIVE'
            WHEN dl.is_active = TRUE AND dl.last_updated >= DATE_SUB(NOW(), INTERVAL 5 MINUTE) THEN 'RECENT'
            WHEN dl.is_active = TRUE AND dl.last_updated >= DATE_SUB(NOW(), INTERVAL 30 MINUTE) THEN 'STALE'
            WHEN dl.last_updated >= DATE_SUB(NOW(), INTERVAL ? MINUTE) THEN 'OFFLINE'
            ELSE 'UNKNOWN'
        END as status
    ...
    if ($driver_id) {
        $stmt->bind_param("is", $minutes_back, $driver_id);
    } else {
        $stmt->bind_param("i", $minutes_back);
    }
