...
function getOnlineTransferSubAccountsForDropdown($conn, $view_all_branches, $branch_id)
{
    ...
    if (!$view_all_branches && $branch_id > 0)
        $sql .= " AND (b.branch_id = ? OR b.branch_id = 0)";
    ...
    foreach ($rows as &$row) {
        ...
        if (!empty($row['parent_bank_name']))
            $label_parts[] = $row['parent_bank_name'];
        if (!empty($row['bank_name']))
            $label_parts[] = $row['bank_name'];
...
{
    if (!collectionTableExists($conn, $table))
        return false;
    $table = $conn->real_escape_string($table);
...
function enrichInvoicesWithBeginningBalanceAttachments($conn, &$invoices)
{
    ...
    foreach ($invoices as $invoice) {
        $invoice_id = (int) ($invoice['invoice_id'] ?? 0);
        if ($invoice_id > 0)
            $invoiceIds[$invoice_id] = $invoice_id;
    }
...
function saveCollectionAssignment($conn, $invoice_id, $customer_id, $branch_id, $assigned_user_id, $assigned_by, $collection_date = '')
{
    ...
    if (!$stmt)
        throw new Exception('Failed to prepare collection assignment: ' . $conn->error);
    $stmt->bind_param('iiiiis', $invoice_id, $customer_id, $branch_id, $assigned_user_id, $assigned_by, $collection_date);
    if (!$stmt->execute())
        throw new Exception('Failed to save collection assignment: ' . $stmt->error);
    $stmt->close();
}
...
{
    $invoice_id = (int) $invoice_id;
    if ($invoice_id <= 0 || !collectionTableExists($conn, 'payments'))
        return 0.0;
...
function collectionsSyncInvoicePaymentTotals($conn, $invoice_id, $invoice_total = null, $fallback_status = 'pending')
{
    $invoice_id = (int) $invoice_id;
    if ($invoice_id <= 0)
        return ['paid' => 0.0, 'balance' => 0.0, 'status' => 'pending'];
...
function collectionsEnsureReceivePaymentAccountingTables($conn)
{
    ...
    foreach ($extra as $table => $cols) {
        ...
        foreach ($cols as $col => $sql) {
            if (!collectionColumnExists($conn, $table, $col))
                @$conn->query($sql);
        }
    }
}
...
{
    $customer_id = (int) $customer_id;
    if ($customer_id <= 0 || !collectionTableExists($conn, 'customers'))
        return '';
...
{
    $invoice_id = (int) $invoice_id;
    if ($invoice_id <= 0 || !collectionTableExists($conn, 'invoices'))
        return '';
...
function collectionsFindOrCreateAccount($conn, $titles, $type, $branch_id, $user_id)
{
    ...
    foreach ($titles as $title) {
        ...
        if ($branch_id > 0 && collectionColumnExists($conn, 'chart_of_accounts', 'branch_id')) {
            ...
            $stmt = $conn->prepare($sql);
            if ($stmt)
                $stmt->bind_param('sii', $title, $branch_id, $branch_id);
        } else {
    ...
    if ($title === '')
        throw new Exception('Missing account title for accounting entry.');
    ...
    if (!$stmt)
        throw new Exception('Failed to create chart account: ' . $conn->error);
    $stmt->bind_param('iissssdi', $target_branch, $parent, $account_code, $title, $type, $description, $balance, $user_id);
    if (!$stmt->execute())
        throw new Exception('Failed to create chart account ' . $title . ': ' . $stmt->error);
...
function collectionsNextReceivePaymentEntryNo($conn)
{
    ...
    foreach ([['journal_entries', 'entry_no'], ['chart_account_transactions', 'transaction_no']] as $src) {
        [$table, $col] = $src;
        if (!collectionTableExists($conn, $table) || !collectionColumnExists($conn, $table, $col))
            continue;
...
function collectionsUpdateAccountBalance($conn, $account_id, $debit, $credit)
{
    ...
    if (!$stmt)
        throw new Exception('Failed to prepare COA balance update: ' . $conn->error);
    $stmt->bind_param('ddi', $debit, $credit, $account_id);
    if (!$stmt->execute())
        throw new Exception('Failed to update COA balance: ' . $stmt->error);
    $stmt->close();
...
function collectionsRecalculateCollectionLinkedCoaBalances($conn, $branch_id, $user_id = 0)
{
    ...
    if (!collectionTableExists($conn, 'chart_of_accounts') || !collectionTableExists($conn, 'invoices'))
        return;
    ...
    if ($branch_id > 0) {
        if (collectionColumnExists($conn, 'invoices', 'branch_id')) {
            $invoiceBranchFilter = " AND (i.branch_id = {$branch_id} OR i.branch_id IS NULL OR i.branch_id = 0)";
        }
...
function collectionsPostReceivePaymentAccounting($conn, $payment_id, $invoice_id, $customer_id, $branch_id, $user_id, $amount, $payment_date = null)
{
    ...
    if ($payment_id <= 0 || $invoice_id <= 0 || $customer_id <= 0 || $amount <= 0)
        return;
    ...
    if (!$h)
        throw new Exception('Failed to prepare receive payment journal header: ' . $conn->error);
    $h->bind_param('ssii', $entryNo, $date, $branch_id, $user_id);
    if (!$h->execute())
        throw new Exception('Failed to save receive payment journal header: ' . $h->error);
    ...
    if (!$detail)
        throw new Exception('Failed to prepare receive payment journal details: ' . $conn->error);
    ...
    if (!$cat)
        throw new Exception('Failed to prepare receive payment COA transaction: ' . $conn->error);
    ...
    foreach ($lines as $line) {
        ...
        if (!$detail->execute())
            throw new Exception('Failed to save receive payment journal line: ' . $detail->error);
        ...
        if (!$cat->execute())
            throw new Exception('Failed to save receive payment COA transaction: ' . $cat->error);
    }
    $detail->close();
    $cat->close();
}
...
{
    $parts = [];
    if (trim((string) $barangay) !== '')
        $parts[] = trim((string) $barangay);
    if (trim((string) $city) !== '')
        $parts[] = trim((string) $city);
    if (trim((string) $province) !== '')
        $parts[] = trim((string) $province);
    if (trim((string) $region) !== '')
        $parts[] = trim((string) $region);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($action === 'update_journal_payment') {
            ...
            if ($payment_id <= 0)
                throw new Exception('Invalid payment record selected for update.');
            if (empty($remit))
            ...
            if (!$old_stmt)
                throw new Exception('Failed to prepare payment lookup: ' . $conn->error);
            ...
            if ($invoice_id <= 0 || $customer_id <= 0)
                throw new Exception('Payment is not linked to a valid invoice/customer.');
            ...
            if (!$other_stmt)
                throw new Exception('Failed to prepare payment total lookup: ' . $conn->error);
            ...
            if ($payment_method === 'check') {
                if ($check_date === null || $bank_name === null || $check_number === null)
            ...
            try {
                ...
                if (!$upd->execute())
                    throw new Exception('Failed to update payment: ' . $upd->error);
                $upd->close();
                ...
                if ($posting_branch_id <= 0)
                    $posting_branch_id = (int) $branch_id;
        ...
        if ($action === 'add_customer_from_beginning_balance' || $action === 'add_customer') {
            ...
            if ($customer_code === '')
                $customer_code = collectionsGenerateCustomerCode($conn);

            $dup_stmt = $conn->prepare("SELECT customer_id FROM customers WHERE customer_code = ? LIMIT 1");
            if ($dup_stmt) {
        ...
        elseif ($action === 'approve_remittance') {
            ...
            if (!$remit_stmt)
                throw new Exception('Failed to prepare remittance lookup: ' . $conn->error);
            ...
            $dup_stmt = $conn->prepare("SELECT payment_id FROM payments WHERE source_table = 'collection_records' AND source_id = ? LIMIT 1");
            if ($dup_stmt) {
            ...
            if (!$update_record)
                throw new Exception('Failed to prepare remittance update: ' . $conn->error);
            $update_record->bind_param('ii', $user_id, $remittance_id);
            $update_record->execute();
            if ($update_record->affected_rows <= 0)
                throw new Exception('Remittance was already processed. Please refresh.');
            $update_record->close();
        ...
        elseif ($action === 'reject_remittance') {
            ...
            if (!$update_stmt)
                throw new Exception('Failed to prepare rejection: ' . $conn->error);
            $update_stmt->bind_param('sii', $rejection_reason, $user_id, $remittance_id);
            if (!$update_stmt->execute())
                throw new Exception('Failed to reject remittance: ' . $update_stmt->error);
        ...
        elseif ($action === 'approve_return_ticket') {
            ...
            if (!$return_ticket)
                throw new Exception('Return ticket not found or already processed');
            ...
            if (!$upd_ret)
                throw new Exception('Failed to prepare approve return: ' . $conn->error);
            $upd_ret->bind_param('ii', $user_id, $return_id);
            $upd_ret->execute();
            if ($upd_ret->affected_rows <= 0)
                throw new Exception('Return ticket was already processed. Please refresh.');
            $upd_ret->close();
        ...
        elseif ($action === 'reject_return_ticket') {
            ...
            if (!$upd_ret)
                throw new Exception('Failed to prepare reject return: ' . $conn->error);
            ...
            if ($assignment_id > 0) {
                $reactivate = $conn->prepare("UPDATE collection_assignments
                                             ...
                                             WHERE assignment_id = ?");
                if (!$reactivate)
                    throw new Exception('Failed to prepare assignment reactivation: ' . $conn->error);
                $reactivate->bind_param('i', $assignment_id);
                if (!$reactivate->execute())
                    throw new Exception('Failed to reactivate assignment: ' . $reactivate->error);
            ...
            if ($reactivated_rows <= 0 && $invoice_id > 0 && $returned_by > 0) {
                ...
                if ($reactivate2) {
                    ...
                    if (!$reactivate2->execute())
                        throw new Exception('Failed to reactivate collector assignment: ' . $reactivate2->error);
            ...
            if ($reactivated_rows <= 0 && $invoice_id > 0 && $customer_id > 0 && $returned_by > 0) {
                ...
                if (!$insert_assign)
                    throw new Exception('Failed to recreate assignment: ' . $conn->error);
                ...
                if (!$insert_assign->execute())
                    throw new Exception('Failed to return invoice to collector: ' . $insert_assign->error);
                $insert_assign->close();
            }
        ...
        elseif ($action === 'submit_remittance') {
            ...
            foreach ($remittances_data as $remit) {
                ...
                $invoice_stmt = $conn->prepare("SELECT i.invoice_id, i.customer_id, i.branch_id, i.total_amount, i.amount_paid, i.balance, i.status,
                                               ...
                                               LIMIT 1");
                if (!$invoice_stmt)
                    throw new Exception('Failed to prepare invoice lookup: ' . $conn->error);
                ...
                if ($customer_id <= 0)
                    $customer_id = (int) ($invoice_row['customer_id'] ?? 0);
                if ($customer_id <= 0)
        ...
        elseif ($action === 'add_beginning_balance') {
            $data = $_POST;
            if (empty($data) && is_array($json))
                $data = $json;
            if (!$data || !is_array($data))
                throw new Exception('Invalid beginning balance data received');

            $customer_id = (int) ($data['customer_id'] ?? 0);
            $document_type = strtolower(trim($data['document_type'] ?? 'so'));
            if (!in_array($document_type, ['so', 'si'], true))
                $document_type = 'so';
            ...
            if ($document_type === 'so') {
                if (!preg_match('/^\d{5,6}$/', $so_digits))
                    throw new Exception('SO number must be 5 to 6 numbers only');
                $si_number_input = '';
                ...
                if (strlen($si_number_input) > 100)
                    throw new Exception('SI Number is too long');
                if ($registered_business_name === '')
                    throw new Exception('Please enter Registered Business Name');
                if ($tin === '')
                    throw new Exception('Please enter TIN');
                if ($business_address === '')
                    throw new Exception('Please enter Address');
            }
            if ($amount <= 0)
                throw new Exception('Beginning balance amount must be greater than zero');

            if ($due_date === '')
                $due_date = $invoice_date;

            $cust_sql = "SELECT customer_id, customer_name, branch_id FROM customers WHERE customer_id = ? AND status = 'active'";
            if ($customers_branch_column_exists && !$view_all_branches && $branch_id > 0) {
            ...
            if (!$cust_stmt)
                throw new Exception('Failed to prepare customer lookup: ' . $conn->error);
            ...
            for ($try = 0; $try < 10; $try++) {
                ...
                if (!$dup_stmt)
                    throw new Exception('Failed to prepare duplicate invoice check: ' . $conn->error);
            ...
            for ($try = 0; $try < 10; $try++) {
                ...
                if (!$dup_so_stmt)
                    throw new Exception('Failed to prepare duplicate SO check: ' . $conn->error);
                $dup_so_stmt->bind_param('s', $so_number);
                $dup_so_stmt->execute();
                $dup_so_row = $dup_so_stmt->get_result()->fetch_assoc();
                $dup_so_stmt->close();
                if ((int) ($dup_so_row['cnt'] ?? 0) === 0)
                    break;
                if ($document_type === 'so')
                    throw new Exception('SO number already exists: ' . $so_number);
                $auto_so_digits = substr(date('His') . random_int(1000, 9999), 0, 6);
            ...
            if ($si_number !== null && ($sales_orders_has_si || $invoices_has_si)) {
                ...
                if (!$dup_si_stmt)
                    throw new Exception('Failed to prepare duplicate SI check: ' . $conn->error);
                collectionBindParamsDynamic($dup_si_stmt, $types, $values);
                $dup_si_stmt->execute();
                $dup_si_row = $dup_si_stmt->get_result()->fetch_assoc();
                $dup_si_stmt->close();
                if ((int) ($dup_si_row['cnt'] ?? 0) > 0)
                    throw new Exception('SI number already exists: ' . $si_number);
            }
        ...
        elseif ($action === 'update_beginning_balance') {
            ...
            if (empty($data) && is_array($json))
                $data = $json;
            if (!$data || !is_array($data))
                throw new Exception('Invalid beginning balance update data received');
            ...
            if ($invoice_id <= 0)
                throw new Exception('Invalid beginning balance record');
            if ($customer_id <= 0)
                throw new Exception('Please select a customer');
            if ($document_type === 'so') {
                if (!preg_match('/^\d{5,6}$/', $so_digits))
            ...
            if (!$lookup_stmt)
                throw new Exception('Failed to prepare beginning balance lookup: ' . $conn->error);
            $lookup_stmt->bind_param('i', $invoice_id);
            $lookup_stmt->execute();
            $existing = $lookup_stmt->get_result()->fetch_assoc();
            $lookup_stmt->close();
            if (!$existing)
                throw new Exception('Only beginning balance records can be edited. Sales/order records are not editable here.');
            ...
            $dup_inv_stmt->close();
            if ((int) ($dup_inv['cnt'] ?? 0) > 0)
                throw new Exception('Invoice number already exists: ' . $invoice_number);
            ...
            if ($so_id > 0) {
                ...
                $dup_so_stmt->close();
                if ((int) ($dup_so['cnt'] ?? 0) > 0)
                    throw new Exception('SO number already exists: ' . $so_number);
            }

            $conn->begin_transaction();
            $paid_amount = (float) ($existing['amount_paid'] ?? 0);
            $pay_stmt = $conn->prepare("SELECT COALESCE(SUM(amount), 0) AS total_paid FROM payments WHERE invoice_id = ? AND (status IS NULL OR status = 'completed')");
            if ($pay_stmt) {
            ...
            if (!$invoice_stmt->execute())
                throw new Exception('Failed to update beginning balance invoice: ' . $invoice_stmt->error);
            $invoice_stmt->close();
            ...
            if ($so_id > 0) {
                ...
                if (!$so_stmt)
                    throw new Exception('Failed to prepare sales order update: ' . $conn->error);
                collectionBindParamsDynamic($so_stmt, $so_types, $so_values);
                if (!$so_stmt->execute())
                    throw new Exception('Failed to update beginning balance sales order: ' . $so_stmt->error);
                $so_stmt->close();
            }
            ...
            recalcCustomerCreditUsed($conn, $customer_id);
            if ((int) ($existing['customer_id'] ?? 0) !== $customer_id)
                recalcCustomerCreditUsed($conn, (int) $existing['customer_id']);
            $conn->commit();
        ...
        elseif ($action === 'get_all_pending_invoices') {
            ...
            if ($search_query !== '') {
                ...
                if (collectionColumnExists($conn, 'invoices', 'si_number'))
                    $search_fields[] = "COALESCE(i.si_number, '')";
                if (collectionColumnExists($conn, 'sales_orders', 'si_number'))
                    $search_fields[] = "COALESCE(so.si_number, '')";
                if (collectionColumnExists($conn, 'invoices', 'registered_business_name'))
                    $search_fields[] = "COALESCE(i.registered_business_name, '')";
                if (collectionColumnExists($conn, 'sales_orders', 'registered_business_name'))
                    $search_fields[] = "COALESCE(so.registered_business_name, '')";
                if (collectionColumnExists($conn, 'invoices', 'tin'))
                    $search_fields[] = "COALESCE(i.tin, '')";
                if (collectionColumnExists($conn, 'sales_orders', 'tin'))
                    $search_fields[] = "COALESCE(so.tin, '')";
                if (collectionColumnExists($conn, 'invoices', 'remarks'))
                    $search_fields[] = "COALESCE(i.remarks, '')";
                if (collectionColumnExists($conn, 'sales_orders', 'remarks'))
                    $search_fields[] = "COALESCE(so.remarks, '')";
            ...
            if (!$view_all_branches && $branch_id > 0)
                $sql .= " AND COALESCE(i.branch_id, so.branch_id, c.branch_id, 0) = " . intval($branch_id);
            $sql .= " ORDER BY i.invoice_date DESC";

            $stmt = $conn->prepare($sql);
            if (!$stmt)
                throw new Exception('Failed to prepare invoice query: ' . $conn->error);
            if (!empty($filter_values))
                collectionBindParamsDynamic($stmt, $filter_types, $filter_values);
            $stmt->execute();
        ...
        elseif ($action === 'assign_multiple_collectors') {
            $data = is_array($json) ? $json : $_POST;
            if (!$data || !is_array($data))
                throw new Exception('Invalid assignment data received');
            ...
            if ($branch_id > 0 && ($has_user_branch || ($has_driver_branch && $has_user_driver_id))) {
                ...
                if (!empty($branchParts))
                    $collector_sql .= " AND (" . implode(' OR ', $branchParts) . ")";
            }

            $collector_stmt = $conn->prepare($collector_sql);
            if (!$collector_stmt)
                throw new Exception('Failed to prepare collector lookup: ' . $conn->error);
            ...
            if (!$collector_exists)
                throw new Exception('Selected collector must be an active Sales Agent or Driver registered in your branch.');
...
$collection_records_report_query = "SELECT
    ...
    WHERE cr.status IN ('approved','collected')
    {$report_branch_condition_cr}
...
if ($collection_records_report_result) {
    while ($row = $collection_records_report_result->fetch_assoc()) {
        ...
        if (isset($collection_report_totals[$role]))
            $collection_report_totals[$role] += $amount;
...
foreach ($receivables as $inv) {
    ...
    if ($aging_days <= 7)
        $aging_1_7 += $amount;
    elseif ($aging_days <= 14)
        $aging_8_14 += $amount;
    elseif ($aging_days <= 21)
        $aging_15_21 += $amount;
    elseif ($aging_days <= 28)
        $aging_22_28 += $amount;
    else
        $aging_beyond_28 += $amount;
}
    ...
    if ($journal_payment_id <= 0 && ($record_id_param > 0 || $source_table_for_journal === 'collection_records')) {
        ...
        if ($record_lookup_id > 0) {
            if (collectionColumnExists($conn, 'payments', 'source_table') && collectionColumnExists($conn, 'payments', 'source_id')) {
                $lookup_stmt = $conn->prepare("SELECT payment_id FROM payments WHERE source_table = 'collection_records' AND source_id = ? ORDER BY payment_id DESC LIMIT 1");
                if ($lookup_stmt) {
...
<head>
    ...
    <style>
        ...
        .customer-payment-modal .modal-dialog {
            max-width: 96vw;
        }
        ...
        .cp-detail-panel .form-control:focus,
        .cp-input:focus,
        .cp-select:focus,
        .cp-row-payment:focus {
            border-color: #44D34E;
            box-shadow: 0 0 0 .16rem rgba(68, 211, 78, .16);
        }
        ...
        <div class="main-content" id="mainContent">
            <div class="navbar-top no-print">
                ...
                <div class="page-title">
                    <h2>Collections</h2>
                    <p>Record customer payments and manage receivables</p>
                </div>
            </div>
            ...
            <div class="row stat-card-row g-2 g-md-3 mb-4">
                <div class="col-12 col-sm-6 col-lg-3">
                    <div class="stat-card total h-100"><i class="bi bi-cash-stack stat-icon"></i>
                        <div class="stat-content">
                            <div class="stat-value">₱<?= number_format($total_receivables, 2) ?></div>
                            <div class="stat-label">Total Receivables</div><small class="d-block">Unpaid
                                Receivables</small>
                        </div>
                    </div>
                </div>
                <div class="col-12 col-sm-6 col-lg-3">
                    <div class="stat-card pending h-100"><i class="bi bi-clock-history stat-icon"></i>
                        <div class="stat-content">
                            <div class="stat-value"><?= $avg_collection_days ?> days</div>
                            <div class="stat-label">Average Collection Period</div><small class="d-block">Days invoice
                                to today (unpaid)</small>
                        </div>
                    </div>
                </div>
                <div class="col-12 col-sm-6 col-lg-3">
                    <div class="stat-card overdue h-100"><i class="bi bi-exclamation-triangle stat-icon"></i>
                        <div class="stat-content">
                            <div class="stat-value">₱<?= number_format($overdue_receivables, 2) ?></div>
                            <div class="stat-label">Overdue Receivables</div><small class="d-block">Past due date
                                only</small>
                        </div>
                    </div>
                </div>
                <div class="col-12 col-sm-6 col-lg-3">
                    <div class="stat-card aging h-100" id="agingCardBtn" style="cursor: pointer;"><i
                            class="bi bi-pie-chart stat-icon"></i>
                        <div class="stat-content">
                            <div class="stat-value">
                                ₱<?= number_format(($aging_1_7 + $aging_8_14 + $aging_15_21 + $aging_22_28 + $aging_beyond_28), 2) ?>
                            </div>
                            <div class="stat-label">Aging Breakdown</div><small class="d-block">Click to view
                                details</small>
                        </div>
                    </div>
                </div>
            ...
            <?php if (!empty($pending_remittances)): ?>
                <div class="pending-remittances-section mb-4">
                    ...
                    <div class="section-header d-flex justify-content-between align-items-center" data-bs-toggle="collapse"
                        data-bs-target="#pendingRemittancesCollapse" style="cursor:pointer;">
                        ...
                        <div class="d-flex align-items-center gap-2">
                            <button class="btn-approve-all" id="approveAllRemittancesBtn"
                            ...
                            <table class="table table-hover mb-0">
                                ...
                                <tbody>
                                    <?php foreach ($pending_remittances as $remit): ?>
                                        <tr class="remittance-row" data-remittance-id="<?= $remit['remittance_id'] ?>"
                                            data-photo="<?= htmlspecialchars($remit['attachment_path'] ?? '') ?>"
                                            data-title="Remittance - <?= htmlspecialchars($remit['invoice_number'] ?? '') ?>">
                                            <td><?= htmlspecialchars(($remit['collector_first'] ?? '') . ' ' . ($remit['collector_last'] ?? '')) ?>
                                            </td>
                                            <td><?= htmlspecialchars($remit['customer_name'] ?? 'Unknown') ?></td>
                                            <td><?= htmlspecialchars($remit['invoice_number'] ?? '') ?></td>
                                            <td class="text-end fw-bold text-success">₱<?= number_format($remit['amount'], 2) ?>
                                            </td>
                                            <td><span
                                                    class="badge bg-info"><?= ucfirst(str_replace('_', ' ', $remit['payment_method'])) ?></span>
                                            </td>
                                            ...
                                            <td>
                                                <?php if (!empty($remit['attachment_path'])): ?>
                                                    <span class="badge bg-primary" style="cursor: pointer;"
                                                        onclick="event.stopPropagation(); openAttachmentPhotoModal('<?= htmlspecialchars($remit['attachment_path']) ?>', 'Remittance - <?= htmlspecialchars($remit['invoice_number'] ?? '') ?>')"><i
                                                            class="bi bi-image"></i> View</span>
                                                <?php else: ?>
                                            ...
                                            <td class="remittance-actions">
                                                <button class="btn-approve"
                                                    onclick="event.stopPropagation(); approveRemittance(<?= $remit['remittance_id'] ?>)"><i
                                                        class="bi bi-check-lg"></i> Approve</button>
                                                <button class="btn-reject"
                                                    onclick="event.stopPropagation(); rejectRemittance(<?= $remit['remittance_id'] ?>)"><i
                    ...
                    <div class="d-md-none p-3">
                        <?php foreach ($pending_remittances as $remit): ?>
                            <div class="remittance-card" data-remittance-id="<?= $remit['remittance_id'] ?>"
                                data-photo="<?= htmlspecialchars($remit['attachment_path'] ?? '') ?>"
                                data-title="Remittance - <?= htmlspecialchars($remit['invoice_number'] ?? '') ?>">
                                <div class="remittance-header">
                                    <strong><i class="bi bi-person-badge"></i>
                                        <?= htmlspecialchars(($remit['collector_first'] ?? '') . ' ' . ($remit['collector_last'] ?? '')) ?></strong>
                                    <span
                                        class="badge bg-info"><?= ucfirst(str_replace('_', ' ', $remit['payment_method'])) ?></span>
                                </div>
                                <div><i class="bi bi-building"></i>
                                    <?= htmlspecialchars($remit['customer_name'] ?? 'Unknown') ?></div>
                                <div><i class="bi bi-receipt"></i> Invoice:
                                    <?= htmlspecialchars($remit['invoice_number'] ?? '') ?>
                                </div>
                                <div class="fw-bold text-success mt-1">₱<?= number_format($remit['amount'], 2) ?></div>
                                <div class="text-muted small"><i class="bi bi-calendar"></i> Collected:
                                    <?= date('M d, Y', strtotime($remit['collection_date'])) ?>
                                </div>
                                <div class="mt-1">
                                    <?php if (!empty($remit['attachment_path'])): ?>
                                        <span class="badge bg-primary" style="cursor: pointer;"
                                            onclick="event.stopPropagation(); openAttachmentPhotoModal('<?= htmlspecialchars($remit['attachment_path']) ?>', 'Remittance - <?= htmlspecialchars($remit['invoice_number'] ?? '') ?>')"><i
                                                class="bi bi-image"></i> View Photo</span>
                                    <?php else: ?>
                                ...
                                <div class="mt-2 d-flex gap-2">
                                    <button class="btn-approve btn-sm"
                                        onclick="approveRemittance(<?= $remit['remittance_id'] ?>)"><i
                                            class="bi bi-check-lg"></i> Approve</button>
                                    <button class="btn-reject btn-sm"
                                        onclick="rejectRemittance(<?= $remit['remittance_id'] ?>)"><i class="bi bi-x-lg"></i>
                                        Reject</button>
                                </div>
                            </div>
                        <?php endforeach; ?>
                    ...
                    <div class="alert alert-warning m-3 small"
                        style="background: #fff8e7; border-color: #fed7aa; color: #92400e;">
            ...
            <?php if (!empty($returned_invoices)): ?>
                <div class="pending-remittances-section mb-4">
                        ...
                        <table class="table table-hover mb-0">
                            ...
                            <tbody>
                                <?php foreach ($returned_invoices as $ret): ?>
                                    <tr class="return-row" data-photo="<?= htmlspecialchars($ret['attachment_path'] ?? '') ?>"
                                        data-title="Return Ticket - <?= htmlspecialchars($ret['invoice_number'] ?? '') ?>">
                                        <td><?= htmlspecialchars(trim(($ret['returned_first'] ?? '') . ' ' . ($ret['returned_last'] ?? ''))) ?>
                                        </td>
                                        <td><?= htmlspecialchars($ret['customer_name'] ?? 'Unknown') ?></td>
                                        <td><?= htmlspecialchars($ret['invoice_number'] ?? '') ?></td>
                                        <td class="text-end"><strong
                                                class="text-danger">₱<?= number_format((float) ($ret["balance_amount"] ?? 0), 2) ?></strong>
                                            <div class="small text-muted">Paid:
                                                ₱<?= number_format((float) ($ret["paid_amount"] ?? 0), 2) ?></div>
                                        </td>
                                        ...
                                        <td class="remittance-actions" onclick="event.stopPropagation();">
                                            <button class="btn-approve"
                                                onclick="approveReturnTicket(<?= $ret['return_id'] ?>)"><i
                                                    class="bi bi-check-lg"></i> Approve</button>
                                            <button class="btn-reject" onclick="rejectReturnTicket(<?= $ret['return_id'] ?>)"><i
                                                    class="bi bi-x-lg"></i> Reject</button>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                    ...
                    <div class="d-md-none p-3">
                        <?php foreach ($returned_invoices as $ret): ?>
                            <div class="remittance-card return-row"
                                data-photo="<?= htmlspecialchars($ret['attachment_path'] ?? '') ?>"
                                data-title="Return Ticket - <?= htmlspecialchars($ret['invoice_number'] ?? '') ?>">
                                <div class="remittance-header">
                                    <strong><i class="bi bi-person-badge"></i>
                                        <?= htmlspecialchars(trim(($ret['returned_first'] ?? '') . ' ' . ($ret['returned_last'] ?? ''))) ?></strong>
                                    <span class="badge bg-dark">Returned</span>
                                </div>
                                <div><i class="bi bi-building"></i> <?= htmlspecialchars($ret['customer_name'] ?? 'Unknown') ?>
                                </div>
                                <div><i class="bi bi-receipt"></i> Invoice:
                                    <?= htmlspecialchars($ret['invoice_number'] ?? '') ?>
                                </div>
                                <div class="fw-bold mt-1 text-danger">Remaining:
                                    ₱<?= number_format((float) ($ret["balance_amount"] ?? 0), 2) ?></div>
                                <div class="text-muted small">Paid: ₱<?= number_format((float) ($ret["paid_amount"] ?? 0), 2) ?>
                                </div>
                                <div class="text-muted small mt-1"><i class="bi bi-chat-left-text"></i>
                                    <?= htmlspecialchars($ret['return_reason'] ?? '') ?></div>
                                <div class="text-muted small"><i class="bi bi-calendar"></i> Returned:
                                    <?= date('M d, Y', strtotime($ret['created_at'])) ?>
                                </div>
                                <div class="mt-1">
                                    <?php if (!empty($ret['attachment_path'])): ?>
                                ...
                                <div class="mt-2 d-flex gap-2" onclick="event.stopPropagation();">
                                    <button class="btn-approve btn-sm"
                                        onclick="approveReturnTicket(<?= $ret['return_id'] ?>)"><i class="bi bi-check-lg"></i>
                                        Approve</button>
                                    <button class="btn-reject btn-sm" onclick="rejectReturnTicket(<?= $ret['return_id'] ?>)"><i
                                            class="bi bi-x-lg"></i> Reject</button>
                                </div>
                            </div>
                        <?php endforeach; ?>
            ...
            <div id="batchAssignBar" class="batch-assign-bar" style="display: none;">
                ...
                <div class="d-flex gap-2">
                    ...
                    <button class="btn-assign-batch" id="batchAssignBtn"><i class="bi bi-person-plus"></i> Assign to
                        Collector</button>
                </div>
            </div>
            ...
            <div class="supplier-filter-card mb-4">
                <div class="supplier-filter-header">
                    <h5><i class="bi bi-funnel"></i> Filter Invoices</h5>
                </div>
                    ...
                    <div class="supplier-filter-one-line">
                        <div class="filter-item" style="flex: 2;">
                            ...
                            <div class="supplier-search-wrapper">
                                ...
                                <input type="text" id="invoiceGlobalSearch" class="supplier-filter-input"
                                    placeholder="Search customer, customer group, invoice no., SI, SO, status, remarks..."
                                    autocomplete="off">
                            </div>
                        </div>
                        <div class="filter-item"><label class="supplier-filter-label">DATE FROM</label><input
                                type="date" class="supplier-filter-input" id="dateFrom"></div>
                        <div class="filter-item"><label class="supplier-filter-label">DATE TO</label><input type="date"
                                class="supplier-filter-input" id="dateTo"></div>
                    </div>
                </div>
            </div>

            <!-- Credit Summary (shown only when a specific customer is selected) -->
            <div id="creditSummary" class="credit-summary" style="display: none;">
                <div class="credit-item"><strong>Credit Limit:</strong> <span id="creditLimit">0.00</span></div>
                <div class="credit-item"><strong>Outstanding Balance:</strong> <span id="outstandingBalance">0.00</span>
                </div>
                <div class="credit-item"><strong>Available Credit:</strong> <span id="availableCredit">0.00</span></div>
            </div>
            ...
            <div class="data-table">
                <div class="table-header">
                    ...
                    <div class="d-flex gap-2 flex-wrap justify-content-end">
                        <button type="button" class="btn btn-success" id="openBeginningBalanceBtn"
                            style="border-radius: 10px; font-weight: 600;">
                            <i class="bi bi-plus-circle me-1"></i> Add Beginning Balance
                        </button>
                        <button type="button" class="btn btn-primary" id="openCustomerPaymentBtn"
                            style="border-radius: 10px; font-weight: 600; background:#052A47; border-color:#052A47;">
                ...
                <div class="customer-group-tabs-wrap" id="customerGroupTabsWrap">
                    <div class="customer-group-tabs" id="customerGroupTabs">
                        ...
                        <?php foreach ($customer_group_tabs as $tab): ?>
                            <button type="button" class="customer-group-tab"
                                data-group="<?= htmlspecialchars($tab['group_name'], ENT_QUOTES) ?>">
                                <i class="bi bi-tag"></i>
                ...
                <div class="table-responsive">
                    <table class="table table-hover" id="invoicesTable">
                        <thead>
                            <tr>
                                <th class="checkbox-column"><input type="checkbox" id="selectAllCheckbox"
                            ...
                            <?php else: ?>
                                <?php foreach ($default_invoices as $invoice):
                                    ...
                                    <tr class="invoice-row" data-invoice-id="<?= $invoice['invoice_id'] ?>"
                                        ...
                                        data-branch-id="<?= $invoice['branch_id'] ?? 0 ?>"
                                        data-customer-group="<?= htmlspecialchars($invoice['customer_group'] ?? 'Ungrouped', ENT_QUOTES) ?>">
                                        <td class="checkbox-column"><input type="checkbox" class="row-checkbox"
                                                data-invoice-id="<?= $invoice['invoice_id'] ?>"
                                                data-customer-id="<?= $invoice['customer_id'] ?>"
                                                data-branch-id="<?= $invoice['branch_id'] ?? 0 ?>"></td>
                        ...
                        <div class="modal-body">
                            <div class="row g-3">
                                <div class="col-md-12">
                                    ...
                                    <div class="d-flex gap-2">
                                        <div id="bbCustomerDropdownWrap" class="bb-customer-picker"
                                            style="flex: 1; min-height: 38px;">
                                            <input type="hidden" name="customer_id" id="bbCustomerId">
                                            <input type="hidden" id="bbEditInvoiceId" value="">
                                            <input type="text" id="bbCustomerSearch" class="bb-customer-input"
                                                placeholder="-- Select registered customer --" autocomplete="off">
                                            ...
                                            <div class="bb-customer-menu" id="bbCustomerMenu">
                                                <?php foreach ($branch_customers as $customer): ?>
                                                    ...
                                                    if ($bb_customer_group === '')
                                                        $bb_customer_group = 'Ungrouped';
                                                    ...
                                                    ?>
                                                    <div class="bb-customer-item"
                                                        data-value="<?= (int) $customer['customer_id'] ?>"
                                                        data-label="<?= htmlspecialchars($bb_customer_label, ENT_QUOTES) ?>">
                                                        <div class="bb-customer-main">
                                                            <strong><?= htmlspecialchars($bb_customer_base_label) ?></strong>
                                                            <span
                                                                class="bb-customer-group-badge"><?= htmlspecialchars($bb_customer_group) ?></span>
                                                        </div>
                                                    </div>
                                                <?php endforeach; ?>
                                                <div class="bb-customer-empty" id="bbCustomerEmpty">No customer found
                                                </div>
                                            </div>
                                        </div>
                                        <button type="button" class="btn btn-success" id="openBbAddCustomerBtn"
                                ...
                                <div class="col-md-12">
                                    ...
                                    <div class="btn-group w-100" role="group"
                                        aria-label="Beginning Balance Document Type">
                                        <input type="radio" class="btn-check" name="bbDocumentType" id="bbDocTypeSo"
                                            value="so" autocomplete="off" checked>
                                        <label class="btn btn-outline-success" for="bbDocTypeSo">SO</label>
                                        <input type="radio" class="btn-check" name="bbDocumentType" id="bbDocTypeSi"
                                            value="si" autocomplete="off">
                                        <label class="btn btn-outline-success" for="bbDocTypeSi">SI</label>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label fw-bold" id="bbDocumentDigitsLabel">SO Number <span
                                            class="text-danger">*</span></label>
                                    <div class="input-group">
                                        <span class="input-group-text"
                                            id="bbDocumentPrefix">SO-<?= date('Ymd') ?>-</span>
                                        <input type="text" class="form-control" id="bbDocumentDigits"
                                            inputmode="numeric" maxlength="6" placeholder="5 to 6 digits" required
                                            style="min-width: 0;">
                                    </div>
                                ...
                                <div class="col-md-12" id="bbSiBusinessFields" style="display:none;">
                                    <div class="card border-0 bg-light">
                                        <div class="card-body p-3">
                                            <div class="row g-3">
                                                <div class="col-md-12">
                                                    <label class="form-label fw-bold">Registered Business Name <span
                                                            class="text-danger">*</span></label>
                                                    <input type="text" class="form-control"
                                                        id="bbRegisteredBusinessName"
                                                        placeholder="Registered Business Name" required>
                                                </div>
                                                <div class="col-md-6">
                                                    <label class="form-label fw-bold">TIN <span
                                                            class="text-danger">*</span></label>
                                                    <input type="text" class="form-control" id="bbTinNumber"
                                                        placeholder="TIN" required>
                                                </div>
                                                <div class="col-md-6">
                                                    <label class="form-label fw-bold">Address <span
                                                            class="text-danger">*</span></label>
                                                    <input type="text" class="form-control" id="bbBusinessAddress"
                                ...
                                <div class="col-md-6">
                                    <label class="form-label fw-bold">Beginning Balance Amount (₱) <span
                                            class="text-danger">*</span></label>
                                    <input type="text" class="form-control" id="bbAmount" inputmode="decimal"
                                        placeholder="0.00" required>
                                </div>

                                <div class="col-md-12">
                                    <label class="form-label fw-bold">Attachments <span
                                    ...
                                    <div id="bbAttachmentsContainer" class="d-flex flex-column gap-2">
                                        <div class="bb-attachment-row border rounded p-2 bg-light">
                                            <div class="input-group">
                                                <input type="file" class="form-control bb-attachment-input"
                                                    name="attachments[]"
                                                    accept=".jpg,.jpeg,.png,.gif,.webp,.pdf,.doc,.docx,.xls,.xlsx,.csv,.txt">
                                                <button type="button"
                                                    class="btn btn-outline-danger bb-remove-attachment-btn"
                                                    style="display:none;">
                                    ...
                                    <button type="button" class="btn btn-outline-success btn-sm mt-2"
                                        id="addBeginningBalanceAttachmentBtn">
                                        <i class="bi bi-plus-circle me-1"></i> Add Attachment
                                    </button>
                                    <div class="form-text">Optional. Allowed: images, PDF, Word, Excel, CSV, TXT. Max
                                        10MB each file.</div>
                                </div>
                                <div class="col-md-12">
                                    <label class="form-label fw-bold">Remarks</label>
                                    <textarea class="form-control" id="bbRemarks" rows="2"
                                        placeholder="Optional notes">Beginning balance</textarea>
                                </div>
                            </div>
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" id="saveCloseBeginningBalanceBtn"><i
                                    class="bi bi-save me-1"></i> Save & Close</button>
                            <button type="button" class="btn btn-success" id="saveBeginningBalanceBtn"><i
                                    class="bi bi-save me-1"></i> Save & New</button>
                        </div>
                    </div>
                </div>
            </div>
                            ...
                            <div class="modal-body">
                                ...
                                <div class="row mb-4">
                                    <div class="col-12">
                                        ...
                                        <div class="code-preview" id="customerCodePreview">
                                            ...
                                            <i class="bi bi-arrow-repeat refresh-code" onclick="refreshCustomerCode()"
                                                title="Generate new code"></i>
                                        </div>
                                        <input type="hidden" name="customer_code" id="customerCodeInput"
                                            value="<?php echo $preview_code; ?>">
                                ...
                                <div class="row">
                                    <div class="col-md-6 mb-3">
                                        <label class="form-label">
                                            <i class="bi bi-person-badge"></i> Customer Name <span
                                                class="text-danger">*</span>
                                        </label>
                                        <input type="text" class="form-control" name="customer_name"
                                            id="addCustomerName" required placeholder="Enter full name">
                                    </div>
                                    <div class="col-md-6 mb-3">
                                        <label class="form-label">
                                        ...
                                        <input type="text" class="form-control" name="contact_person"
                                            id="addContactPerson" placeholder="Enter contact person name">
                                    </div>
                                </div>
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-6 mb-3">
                                        ...
                                        <input type="tel" class="form-control" name="phone_number" id="addPhoneNumber"
                                ...
                                <div class="row">
                                    <div class="col-md-6 mb-3">
                                        ...
                                        <input type="text" class="form-control" name="store_name" id="addStoreName"
                                ...
                                <div class="row">
                                    <div class="col-md-6 mb-3">
                                        ...
                                        <small class="text-muted">Select applicable price level for this
                                            customer</small>
                                    </div>
                                    <div class="col-md-6 mb-3">
                                        <label class="form-label">
                                        ...
                                        <select class="form-select customer-group-select" id="addCustomerGroupSelect"
                                            onchange="toggleCustomerGroupInput('add')">
                                        ...
                                        <input type="text" class="form-control mt-2 d-none" name="customer_group"
                                            id="addCustomerGroup" placeholder="Type new customer group">
                                ...
                                <div class="row">
                                    <div class="col-md-6 mb-3">
                                        ...
                                        <select class="form-select region-select" id="addRegion" name="region">
                                            ...
                                            <?php foreach ($regions as $region_code => $region_name): ?>
                                                <option value="<?php echo $region_code; ?>"><?php echo $region_name; ?>
                                                </option>
                                            <?php endforeach; ?>
                                        </select>
                                    </div>
                                    <div class="col-md-6 mb-3">
                                        ...
                                        <select class="form-select province-select" id="addProvince" name="province"
                                            disabled>
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-6 mb-3">
                                        ...
                                        <div class="manual-toggle-btn" id="manualBarangayToggle" style="display: none;">
                                            <i class="bi bi-pencil-square"></i> Can't find barangay? Click to type
                                            manually
                                        </div>
                                    </div>
                                </div>
            ...
            <!-- Customer Payment Modal -->
            <div class="modal fade customer-payment-modal" id="customerPaymentModal" tabindex="-1"
                ...
                <div class="modal-dialog modal-fullscreen-xl-down modal-xl modal-dialog-scrollable">
                    <div class="modal-content">
                        <div class="modal-header">
                            ...
                            <button type="button" class="cp-close-x" data-bs-dismiss="modal"
                            ...
                            <div class="customer-payment-box p-3">
                                <div class="row g-3 align-items-start cp-payment-top-layout">
                                            ...
                                            <div class="cp-form-area">
                                                <div class="cp-form-row">
                                                    ...
                                                    <div class="cp-field-wrap">
                                                        <select class="form-select cp-select d-none"
                                                            id="cpCustomerSelect">
                                                            ...
                                                            <?php foreach ($branch_customers as $cust): ?>
                                                                ...
                                                                if ($cp_customer_group === '')
                                                                    $cp_customer_group = 'Ungrouped';
                                                                ?>
                                                                <option value="<?= (int) $cust['customer_id'] ?>"
                                                                    data-group="<?= htmlspecialchars($cp_customer_group, ENT_QUOTES) ?>">
                                                                    <?= htmlspecialchars($cp_customer_name) ?>
                                                                </option>
                                                            <?php endforeach; ?>
                                                        </select>
                                                        <div class="cp-customer-picker" id="cpCustomerPicker">
                                                            <div class="cp-customer-toggle" id="cpCustomerToggle"
                                                                aria-expanded="false">
                                                                <input type="text" class="cp-customer-search"
                                                                    id="cpCustomerSearch" placeholder="Select customer"
                                                                    autocomplete="off">
                                                                <i
                                                                    class="bi bi-chevron-down cp-customer-toggle-icon"></i>
                                                            </div>
                                                            <div class="cp-customer-menu" id="cpCustomerMenu">
                                                                <?php foreach ($branch_customers as $cust): ?>
                                                                    ...
                                                                    if ($cp_customer_group === '')
                                                                        $cp_customer_group = 'Ungrouped';
                                                                    ?>
                                                                    <button type="button" class="cp-customer-option"
                                                                        data-value="<?= (int) $cust['customer_id'] ?>"
                                                                        data-label="<?= htmlspecialchars($cp_customer_name, ENT_QUOTES) ?>"
                                                                        data-group="<?= htmlspecialchars($cp_customer_group, ENT_QUOTES) ?>">
                                                                        <span
                                                                            class="cp-customer-option-name"><?= htmlspecialchars($cp_customer_name) ?></span>
                                                                        <span
                                                                            class="cp-customer-option-group"><?= htmlspecialchars($cp_customer_group) ?></span>
                                                                    </button>
                                                                <?php endforeach; ?>
                                                            </div>
                                                        </div>
                                                ...
                                                <div class="cp-form-row cp-amount-row">
                                                    <label class="cp-label" id="cpPaymentAmountLabel">Payment
                                                        Amount</label>
                                                    <div class="cp-field-wrap">
                                                        <input type="text" class="form-control cp-input cp-amount-input"
                                                            id="cpPaymentAmount" value="0.00" inputmode="decimal">
                                                    </div>
                                                </div>
                                                <div class="cp-form-row">
                                                    <label class="cp-label">Date</label>
                                                    <div class="cp-field-wrap"><input type="date"
                                                            class="form-control cp-input" id="cpPaymentDate"
                                    ...
                                    <div class="col-lg cp-methods-balance-col">
                                        <div class="cp-balance-box mb-4">CUSTOMER BALANCE <span class="amount"
                                                id="cpCustomerBalance">0.00</span></div>
                                        <div class="cp-method-grid cp-method-grid-three">
                                            <button type="button" class="cp-method-btn active" data-method="cash"><i
                                                    class="bi bi-cash-stack"></i><span>CASH</span></button>
                                            <button type="button" class="cp-method-btn" data-method="check"><i
                                                    class="bi bi-check2-circle"></i><span>CHECK</span></button>
                                            <button type="button" class="cp-method-btn" data-method="online_transfer"><i
                                                    class="bi bi-globe2"></i><span>ONLINE<br>TRANSFER</span></button>
                                        </div>
                                        ...
                                        <tbody id="cpInvoicesBody">
                                            <tr>
                                                <td colspan="6" class="p-0">
                                                    <div class="cp-message-row">Select the customer or job in the
                                                        Received From field</div>
                                                </td>
                                            </tr>
                                        </tbody>
                                        <tfoot>
                                ...
                                <div class="cp-bottom-panel">
                                    <div class="cp-memo-box">
                                        ...
                                        <textarea class="cp-memo-input" id="cpMemo"
                                            placeholder="Enter memo (optional)..."></textarea>
                                    </div>
                                    <div class="cp-summary-box">
                                        <div class="cp-summary-title">Amounts for Selected Invoices</div>
                                        <div class="cp-summary-line"><span>Amount Due</span><span class="value"
                                                id="cpSummaryAmountDue">0.00</span></div>
                                        <div class="cp-summary-line"><span>Total Payment</span><span class="value"
                                                id="cpSummaryTotalPayment">0.00</span></div>
                                        <div class="cp-summary-line"><span>Change</span><span class="value"
                                                id="cpSummaryChange">0.00</span></div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-success" id="cpSaveCloseBtn"><i
                                    class="bi bi-save me-1"></i>Save & Close</button>
                            <button type="button" class="btn btn-primary" id="cpSaveNewBtn"
                                style="background:#052A47;border-color:#052A47;"><i class="bi bi-save2 me-1"></i>Save &
                                New</button>
                            <button type="button" class="btn btn-outline-secondary" id="cpClearBtn"><i
            ...
            <div class="modal fade" id="paymentModal" tabindex="-1" data-bs-backdrop="static">
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        <div class="modal-header bg-success text-white">
                            <h5 class="modal-title"><i class="bi bi-cash-stack me-2"></i>Record Payment</h5><button
                                type="button" class="btn-close" data-bs-dismiss="modal"></button>
                        </div>
                        <div class="modal-body"><input type="hidden" id="payInvoiceId"><input type="hidden"
                                id="payInvoiceAmount">
                            <div class="row mb-4">
                                <div class="col-md-6">
                                    <div class="invoice-summary-card">
                                        <div class="invoice-summary-label">Invoice Number</div>
                                        <div class="invoice-summary-value" id="payInvoiceNumber">-</div>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="invoice-summary-card">
                                        <div class="invoice-summary-label">Amount Due</div>
                                        <div class="invoice-summary-value text-success" id="payAmountDue">₱0.00</div>
                                    </div>
                                </div>
                            </div>
                            <div class="alert alert-warning mb-3">
                                <i class="bi bi-info-circle me-1"></i>
                                <strong>Note:</strong> Recording a payment will submit it for your approval. Once
                                approved, the invoice will be marked as paid.
                            </div>
                            <div class="form-card p-3 mb-3"
                                style="background:#f8fafc;border-radius:12px;border:1px solid #e9ecef;">
                                <label class="fw-bold mb-2"><i class="bi bi-person-check me-1"></i>Assign Collector (if
                                    not assigned yet)</label>
                                <select class="form-select" id="payAssignCollectorSelect">
                                    ...
                                    <?php foreach ($assignable_collectors as $collector): ?>
                                        ...
                                        <option value="<?= (int) $collector['user_id'] ?>">
                                            <?= htmlspecialchars(trim(($collector['first_name'] ?? '') . ' ' . ($collector['last_name'] ?? ''))) ?>
                                            - <?= $collectorRole ?>
                                        </option>
                                    <?php endforeach; ?>
                                </select>
                            ...
                            <div id="paymentFormSection">
                                ...
                                <div class="row g-3 mb-4">
                                    <div class="col-md-4">
                                        <div class="payment-method-option" data-method="cash"><i
                                                class="bi bi-cash-stack"></i><span>Cash</span></div>
                                    </div>
                                    <div class="col-md-4">
                                        <div class="payment-method-option" data-method="check"><i
                                                class="bi bi-check2-circle"></i><span>Check</span></div>
                                    </div>
                                    <div class="col-md-4">
                                        <div class="payment-method-option" data-method="online_transfer"><i
                                                class="bi bi-globe2"></i><span>Online Transfer</span></div>
                                    </div>
                                </div>
                                <div id="paymentDetailsContainer"></div>
                                <div id="cashFields" style="display: none;">
                                    <div class="mb-3"><label class="form-label">Payment Amount (₱)</label><input
                                            type="text" class="form-control" id="cashTendered"
                                            placeholder="Enter partial or full cash payment" inputmode="decimal">
                                        <div class="form-text">Change: <span id="cashChangeDisplay">₱0.00</span></div>
                                    </div>
                                </div>
                                <div id="otherAmountFields" style="display: none;">
                                    <div class="mb-3"><label class="form-label">Payment Amount (₱)</label><input
                                            type="text" class="form-control format-number" id="paymentAmount"
                                            placeholder="Enter partial or full payment amount"></div>
                                </div>
                            </div>
                        </div>
                        <div class="modal-footer"><button type="button" class="btn btn-secondary"
                                data-bs-dismiss="modal">Cancel</button><button type="button" class="btn btn-warning"
                                id="submitPaymentBtn"><i class="bi bi-send"></i> Submit for Approval</button></div>
                    </div>
                </div>
            </div>
            ...
            <div class="modal fade" id="batchAssignModal" tabindex="-1" data-bs-backdrop="static">
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <div class="mb-3">
                                <label class="form-label fw-bold"><i class="bi bi-list-check me-1"></i>Selected
                                    Invoices</label>
                                <div id="selectedInvoicesList" class="selected-invoices-list"></div>
                            </div>
                            <div class="mb-3">
                                <label class="form-label fw-bold"><i class="bi bi-person-badge me-1"></i>Assign to
                                    Collector</label>
                            ...
                            <div class="mb-3">
                                <label class="form-label fw-bold"><i class="bi bi-calendar me-1"></i>Collection
                                    Date</label>
                                <input type="date" class="form-control" id="batchCollectionDate"
                                    value="<?= date('Y-m-d') ?>">
                            </div>
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                            <button type="button" class="btn btn-success" id="confirmBatchAssignBtn"><i
                                    class="bi bi-check-circle"></i> Assign</button>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header bg-danger text-white">
                            <h5 class="modal-title"><i class="bi bi-exclamation-triangle me-2"></i>Reject Remittance
                            </h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                        </div>
                        <div class="modal-body">
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control" id="rejectionReason" rows="3"
                                    placeholder="Please provide a reason for rejecting this remittance..."></textarea>
                            </div>
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                            <button type="button" class="btn btn-danger" id="confirmRejectBtn"><i
                                    class="bi bi-x-lg"></i> Confirm Rejection</button>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body" style="min-height: 450px; max-height: 70vh; overflow-y: auto;">
                            <div id="agingMainView" style="display: block;">
                                    ...
                                    <div class="col-6">
                                        <div class="summary-card total-summary">
                                            ...
                                            <div class="summary-content">
                                                ...
                                                <div class="summary-value">₱<?= number_format($total_receivables, 2) ?>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-6">
                                        <div class="summary-card overdue-summary">
                                            ...
                                            <div class="summary-content">
                                                ...
                                                <div class="summary-value" id="agingOverdueAmount">
                                                    ₱<?= number_format($overdue_receivables, 2) ?></div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="aging-item clickable" data-days-range="0-7" data-min-days="0"
                                    data-max-days="7">
                                    <div class="aging-header">
                                        <div class="aging-range">
                                            <span class="range-badge bg-warning text-dark">0 - 7 days</span>
                                            <span
                                                class="percentage"><?= $total_receivables > 0 ? round(($aging_1_7 / $total_receivables) * 100) : 0 ?>%</span>
                                        </div>
                                        <div class="aging-amount">₱<?= number_format($aging_1_7, 2) ?></div>
                                    </div>
                                    <div class="progress">
                                        <div class="progress-bar bg-warning"
                                            style="width: <?= $total_receivables > 0 ? ($aging_1_7 / $total_receivables * 100) : 0 ?>%">
                                        </div>
                                    </div>
                                </div>

                                <div class="aging-item clickable" data-days-range="8-14" data-min-days="8"
                                    data-max-days="14">
                                    <div class="aging-header">
                                        <div class="aging-range">
                                            <span class="range-badge bg-orange">8 - 14 days</span>
                                            <span
                                                class="percentage"><?= $total_receivables > 0 ? round(($aging_8_14 / $total_receivables) * 100) : 0 ?>%</span>
                                        </div>
                                        <div class="aging-amount">₱<?= number_format($aging_8_14, 2) ?></div>
                                    </div>
                                    <div class="progress">
                                        <div class="progress-bar bg-orange"
                                            style="width: <?= $total_receivables > 0 ? ($aging_8_14 / $total_receivables * 100) : 0 ?>%">
                                        </div>
                                    </div>
                                </div>

                                <div class="aging-item clickable" data-days-range="15-21" data-min-days="15"
                                    data-max-days="21">
                                    <div class="aging-header">
                                        <div class="aging-range">
                                            <span class="range-badge bg-info">15 - 21 days</span>
                                            <span
                                                class="percentage"><?= $total_receivables > 0 ? round(($aging_15_21 / $total_receivables) * 100) : 0 ?>%</span>
                                        </div>
                                        <div class="aging-amount">₱<?= number_format($aging_15_21, 2) ?></div>
                                    </div>
                                    <div class="progress">
                                        <div class="progress-bar bg-info"
                                            style="width: <?= $total_receivables > 0 ? ($aging_15_21 / $total_receivables * 100) : 0 ?>%">
                                        </div>
                                    </div>
                                </div>

                                <div class="aging-item clickable" data-days-range="22-28" data-min-days="22"
                                    data-max-days="28">
                                    <div class="aging-header">
                                        <div class="aging-range">
                                            <span class="range-badge bg-danger">22 - 28 days</span>
                                            <span
                                                class="percentage"><?= $total_receivables > 0 ? round(($aging_22_28 / $total_receivables) * 100) : 0 ?>%</span>
                                        </div>
                                        <div class="aging-amount">₱<?= number_format($aging_22_28, 2) ?></div>
                                    </div>
                                    <div class="progress">
                                        <div class="progress-bar bg-danger"
                                            style="width: <?= $total_receivables > 0 ? ($aging_22_28 / $total_receivables * 100) : 0 ?>%">
                                        </div>
                                    </div>
                                </div>

                                <div class="aging-item clickable" data-days-range="28+" data-min-days="29"
                                    data-max-days="999">
                                    <div class="aging-header">
                                        <div class="aging-range">
                                            <span class="range-badge bg-dark">Beyond 28 days</span>
                                            <span
                                                class="percentage"><?= $total_receivables > 0 ? round(($aging_beyond_28 / $total_receivables) * 100) : 0 ?>%</span>
                                        </div>
                                        <div class="aging-amount">₱<?= number_format($aging_beyond_28, 2) ?></div>
                                    </div>
                                    <div class="progress">
                                        <div class="progress-bar bg-dark"
                                            style="width: <?= $total_receivables > 0 ? ($aging_beyond_28 / $total_receivables * 100) : 0 ?>%">
                                        </div>
                                    </div>
                                </div>

                                <div class="legend-container">
                                    <div class="legend-title"><i class="bi bi-info-circle-fill"></i><span>Aging based on
                                            overdue days from due date</span></div>
                            ...
                            <div id="agingDetailView" style="display: none;">
                                <div class="d-flex align-items-center mb-3 sticky-top bg-light p-2 rounded"
                                    style="background: #f8fafc; position: sticky; top: -1.25rem; margin-top: -1rem; padding-top: 1rem; z-index: 10;">
                                    <button type="button" class="btn btn-sm btn-outline-secondary me-3"
                                        id="backToAgingBtn" style="border-radius: 30px;">
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="alert alert-info mb-0">
                                <i class="bi bi-info-circle me-1"></i>
                                Select ALL or specific collector before print preview. This report is only for the
                ...
                <div class="modal-dialog modal-md">
                    <div class="modal-content">
                        <div class="modal-header bg-info text-white">
                            <h5 class="modal-title"><i class="bi bi-info-circle me-2"></i>Payment Details</h5><button
                                type="button" class="btn-close" data-bs-dismiss="modal"></button>
                        </div>
                        <div class="modal-body payment-details-modal">
                            <div class="payment-detail-card">
                                <div class="row mb-2">
                                    ...
                                    <div class="col-7 payment-detail-value" id="detailPaymentMethod">-</div>
                                </div>
                                <div class="row mb-2" id="detailCheckDateRow" style="display: none;">
                                    <div class="col-5 payment-detail-label">Check Date:</div>
                                    <div class="col-7 payment-detail-value" id="detailCheckDate">-</div>
                                </div>
                                <div class="row mb-2" id="detailBankNameRow" style="display: none;">
                                    <div class="col-5 payment-detail-label">Bank:</div>
                                    <div class="col-7 payment-detail-value" id="detailBankName">-</div>
                                </div>
                                <div class="row mb-2" id="detailBankBranchRow" style="display: none;">
                                    <div class="col-5 payment-detail-label">Branch:</div>
                                    <div class="col-7 payment-detail-value" id="detailBankBranch">-</div>
                                </div>
                                <div class="row mb-2" id="detailCheckNumberRow" style="display: none;">
                                    <div class="col-5 payment-detail-label">Check No.:</div>
                                    <div class="col-7 payment-detail-value" id="detailCheckNumber">-</div>
                                </div>
                                <div class="row mb-2">
                                    <div class="col-5 payment-detail-label">Amount:</div>
                                    <div class="col-7 payment-detail-value fw-bold text-success" id="detailAmount">-
                                    </div>
                                </div>
                                <div class="row mb-2" id="detailRefNoRow" style="display: none;">
                                    <div class="col-5 payment-detail-label">Ref. No.:</div>
                                    <div class="col-7 payment-detail-value" id="detailRefNo">-</div>
                                </div>
                                <div class="row mb-2">
                                    <div class="col-5 payment-detail-label">Payment Date:</div>
                                    <div class="col-7 payment-detail-value" id="detailPaymentDate">-</div>
                                </div>
                                <div class="row mb-2">
                                    <div class="col-5 payment-detail-label">Received By:</div>
                                    <div class="col-7 payment-detail-value" id="detailReceivedBy">-</div>
                                </div>
                            </div>
                        </div>
                        <div class="modal-footer"><button type="button" class="btn btn-secondary"
            ...
            <div class="mobile-nav" id="mobileNav">
                ...
                <ul class="nav">
                    <!-- Dashboard -->
                    <li class="nav-item">
                        <a class="nav-link <?php echo ($current_page == 'branchdashboard.php') ? 'active' : ''; ?>"
                            href="branchdashboard.php">
                            <i class="bi bi-speedometer2"></i>
                            <span>Dashboard</span>
                        </a>
                    </li>
                    ...
                    <li class="nav-item dropdown-more" id="warehouseMobileDropdown">
                        <a class="nav-link more-btn <?php echo $is_warehouse_page ? 'active' : ''; ?>" href="#"
                            onclick="toggleMobileDropdown(event, 'warehouseMobileMenu')">
                            <i class="bi bi-shop"></i>
                            <span>Warehouse</span>
                        </a>
                        <div class="more-dropdown" id="warehouseMobileMenu">
                            <a href="current_inventory.php"
                                class="dropdown-item <?php echo ($current_page == 'current_inventory.php') ? 'active' : ''; ?>">
                                <i class="bi bi-bar-chart-line"></i><span>Current Inventory</span>
                            </a>
                            <a href="bad_orders.php"
                                class="dropdown-item <?php echo ($current_page == 'bad_orders.php') ? 'active' : ''; ?>">
                                <i class="bi bi-recycle"></i><span>Bad Orders</span>
                            </a>
                            <a href="pick_list_items.php"
                                class="dropdown-item <?php echo ($current_page == 'pick_list_items.php') ? 'active' : ''; ?>">
                                <i class="bi bi-list-check"></i><span>Pick List Items</span>
                            </a>
                            <a href="warehouses.php"
                                class="dropdown-item <?php echo ($current_page == 'warehouses.php') ? 'active' : ''; ?>">
                    ...
                    <li class="nav-item dropdown-more" id="supplierMobileDropdown">
                        <a class="nav-link more-btn <?php echo $is_supplier_page ? 'active' : ''; ?>" href="#"
                            onclick="toggleMobileDropdown(event, 'supplierMobileMenu')">
                            <i class="bi bi-building"></i>
                            <span>Supplier</span>
                        </a>
                        <div class="more-dropdown" id="supplierMobileMenu">
                            <a href="purchase_order.php"
                                class="dropdown-item <?php echo ($current_page == 'purchase_order.php') ? 'active' : ''; ?>">
                                <i class="bi bi-box"></i><span>Receive Inventory</span>
                            </a>
                            <a href="supplier.php"
                                class="dropdown-item <?php echo ($current_page == 'supplier.php') ? 'active' : ''; ?>">
                    ...
                    <li class="nav-item dropdown-more" id="customerMobileDropdown">
                        <a class="nav-link more-btn <?php echo $is_customer_page ? 'active' : ''; ?>" href="#"
                            onclick="toggleMobileDropdown(event, 'customerMobileMenu')">
                            <i class="bi bi-people"></i>
                            <span>Customer</span>
                        </a>
                        <div class="more-dropdown" id="customerMobileMenu">
                            <a href="customer_list.php"
                                class="dropdown-item <?php echo ($current_page == 'customer_list.php') ? 'active' : ''; ?>">
                                <i class="bi bi-person-badge"></i><span>Customer List</span>
                            </a>
                            <a href="approve_credit_requests.php"
                                class="dropdown-item <?php echo ($current_page == 'approve_credit_requests.php') ? 'active' : ''; ?>">
                                <i class="bi bi-pencil-square"></i><span>Approve Credit Request</span>
                            </a>
                            <a href="sales_order.php"
                                class="dropdown-item <?php echo ($current_page == 'sales_order.php') ? 'active' : ''; ?>">
                                <i class="bi bi-cart"></i><span>Sales Order</span>
                            </a>
                            <a href="collections.php"
                                class="dropdown-item <?php echo ($current_page == 'collections.php') ? 'active' : ''; ?>">
                    ...
                    <li class="nav-item dropdown-more" id="deliveryMobileDropdown">
                        <a class="nav-link more-btn <?php echo $is_delivery_page ? 'active' : ''; ?>" href="#"
                            onclick="toggleMobileDropdown(event, 'deliveryMobileMenu')">
                            <i class="bi bi-truck"></i>
                            <span>Delivery</span>
                        </a>
                        <div class="more-dropdown" id="deliveryMobileMenu">
                            <a href="trip_tickets.php"
                                class="dropdown-item <?php echo ($current_page == 'trip_tickets.php') ? 'active' : ''; ?>">
                    ...
                    <li class="nav-item dropdown-more" id="bankingMobileDropdown">
                        <a class="nav-link more-btn <?php echo $is_banking_page ? 'active' : ''; ?>" href="#"
                            onclick="toggleMobileDropdown(event, 'bankingMobileMenu')">
                            <i class="bi bi-bank2"></i>
                            <span>Banking</span>
                        </a>
                        <div class="more-dropdown" id="bankingMobileMenu">
                            <a href="deposit.php"
                                class="dropdown-item <?php echo ($current_page == 'deposit.php') ? 'active' : ''; ?>">
                                <i class="bi bi-arrow-down-circle"></i><span>Deposit</span>
                            </a>
                            <a href="Withdrawal.php"
                                class="dropdown-item <?php echo ($current_page == 'Withdrawal.php') ? 'active' : ''; ?>">
                                <i class="bi bi-arrow-up-circle"></i><span>Withdrawal</span>
                            </a>
                            <a href="bank_statement.php"
                                class="dropdown-item <?php echo ($current_page == 'bank_statement.php') ? 'active' : ''; ?>">
                                <i class="bi bi-receipt"></i><span>Bank Statement</span>
                            </a>
                            <a href="expenses.php"
                                class="dropdown-item <?php echo ($current_page == 'expenses.php') ? 'active' : ''; ?>">
                    ...
                    <li class="nav-item dropdown-more" id="sharedServicesMobileDropdown">
                        <a class="nav-link more-btn" href="#"
                            onclick="toggleMobileDropdown(event, 'sharedServicesMobileMenu')">
                    ...
                    <!-- Users -->
                    <li class="nav-item">
                        <a class="nav-link <?php echo ($current_page == 'drivers.php') ? 'active' : ''; ?>"
                            href="drivers.php">
                            <i class="bi bi-people-fill"></i>
                            <span>Users</span>
                        </a>
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body text-center">
                            ...
                            <p class="text-muted mb-3"><span
                                    class="badge bg-success"><?php echo ucfirst($user_role); ?></span></p>
                            ...
                            <div class="user-id text-muted small mb-4"><i class="bi bi-hash"></i> User ID:
                                <?php echo $user_id; ?>
                            </div><button class="btn btn-danger btn-lg w-100" onclick="confirmLogout()"><i
            ...
            <div class="modal fade" id="attachmentPhotoModal" tabindex="-1" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered modal-lg">
                    <div class="modal-content" style="border-radius:18px;overflow:hidden;">
                        <div class="modal-header" style="background:#052A47;color:#fff;">
                            <h5 class="modal-title" id="attachmentPhotoTitle"><i class="bi bi-image me-2"></i>Attachment
                                Photo</h5>
                            <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                        </div>
                        <div class="modal-body text-center bg-light">
                            <img id="attachmentPhotoImg" src="" alt="Attachment" class="img-fluid rounded shadow-sm"
                ...
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content" style="border-radius:18px;overflow:hidden;">
                        ...
                        <div class="modal-body">
                            ...
                            <label class="form-label fw-bold">Reason for rejection</label>
                            <textarea class="form-control" id="returnRejectionReason" rows="4"
                                placeholder="Enter reason why this return ticket is rejected..."></textarea>
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                            <button type="button" class="btn btn-danger" id="confirmRejectReturnBtn"><i
                                    class="bi bi-x-lg me-1"></i>Reject Return</button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            `);
                ...
                function toggleSidebarDropdown(event, targetId) { 
                    ...
                    const sidebar = document.getElementById('sidebar'); if (!target || !sidebar) return; 
                    ...
                    if (target.classList.contains('show')) { 
                        ...
                        document.querySelectorAll('.sidebar .dropdown-nav > .nav-link .dropdown-arrow').forEach(arrowIcon => { arrowIcon.style.transform = 'translateY(-50%) rotate(0deg)'; 
                ...
                window.toggleMobileDropdown = function (event, dropdownId) {
                    ...
                    // close lahat bago mag open ng bago
                    window.closeAllMobileDropdowns();

                    // kung sarado dati, open sya
                    if (!alreadyOpen) {
                ...
                // compatibility sa old onclick
                window.toggleDropdown = function (event, dropdownId) {
                    return window.toggleMobileDropdown(event, dropdownId);
                };


                // profile modal auto close dropdowns
                ...
                // kapag pumili sa dropdown auto close
