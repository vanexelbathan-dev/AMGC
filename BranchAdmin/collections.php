    ...
    try {
            ...
            if (!empty($_FILES['attachments']) && !empty($_FILES['attachments']['name'])) {
                ...
                foreach ($attachment_names as $attachment_name) {
                    if (trim((string)$attachment_name) !== '') {
                        $has_required_attachment = true;
                        break;
                    }
                }
            }
            if (!$has_required_attachment) throw new Exception('Please upload at least one attachment for the beginning balance');
        ...
        elseif ($action === 'get_all_invoices') {
            ...
            $invoice_remarks_select = collectionColumnExists($conn, 'invoices', 'remarks')
                ? (collectionColumnExists($conn, 'sales_orders', 'remarks') ? "COALESCE(i.remarks, so.remarks) AS remarks" : "i.remarks AS remarks")
                : (collectionColumnExists($conn, 'sales_orders', 'remarks') ? "so.remarks AS remarks" : "NULL AS remarks");
            $sql = "SELECT i.invoice_id, i.invoice_number, {$invoice_si_select}, i.invoice_date, i.due_date, i.total_amount, i.status,
            ...
            <ul class="nav flex-column">
                ...
                <li class="nav-item dropdown-nav">
                    <a class="nav-link" href="#" onclick="toggleSidebarDropdown(event, 'warehouseMenu')">
                        <i class="bi bi-shop"></i><span class="nav-text">Warehouse</span><i class="bi bi-chevron-down dropdown-arrow"></i>
                    </a>
                    <div class="collapse" id="warehouseMenu">
                        <ul class="nav flex-column ps-4">
                            <li><a class="nav-link" href="current_inventory.php"><i class="bi bi-bar-chart-line"></i><span class="nav-text">Current Inventory</span></a></li>
                            <li><a class="nav-link" href="bad_orders.php"><i class="bi bi-recycle"></i><span class="nav-text">Bad Orders</span></a></li>
                            <li><a class="nav-link" href="pick_list_items.php"><i class="bi bi-list-check"></i><span class="nav-text">Pick List Items</span></a></li>
                                                            <li class="nav-item">
                ...
                <li class="nav-item dropdown-nav">
                    <a class="nav-link" href="#" onclick="toggleSidebarDropdown(event, 'supplierMenu')">
                        <i class="bi bi-building"></i><span class="nav-text">Supplier</span><i class="bi bi-chevron-down dropdown-arrow"></i>
                    </a>
                    <div class="collapse" id="supplierMenu">
                        <ul class="nav flex-column ps-4">
                            <li><a class="nav-link" href="purchase_order.php"><i class="bi bi-box"></i><span class="nav-text">Receive Inventory</span></a></li>
                            <li><a class="nav-link" href="supplier.php"><i class="bi bi-people"></i><span class="nav-text">Supplier List</span></a></li>
                        </ul>
                    </div>
                </li>
                <!-- Customer Dropdown -->
                <li class="nav-item dropdown-nav">
                    ...
                    <div class="collapse" id="customerMenu">
                        <ul class="nav flex-column ps-4">
                            <li><a class="nav-link" href="customer_list.php"><i class="bi bi-person-badge"></i><span class="nav-text">Customer List</span></a></li>
                            <li><a class="nav-link" href="approve_credit_requests.php"><i class="bi bi-pencil-square"></i><span class="nav-text">Approve Credit Request</span></a></li>
                            <li><a class="nav-link" href="sales_order.php"><i class="bi bi-cart"></i><span class="nav-text">Sales Order</span></a></li>
                            <li><a class="nav-link active" href="collections.php"><i class="bi bi-cash-stack"></i><span class="nav-text">Collections</span></a></li>
                        </ul>
                    </div>
                </li>
                <!-- Delivery Dropdown -->
                <li class="nav-item dropdown-nav">
                    <a class="nav-link" href="#" onclick="toggleSidebarDropdown(event, 'deliveryMenu')">
                        <i class="bi bi-truck"></i><span class="nav-text">Delivery</span><i class="bi bi-chevron-down dropdown-arrow"></i>
                    </a>
                    <div class="collapse" id="deliveryMenu">
                        <ul class="nav flex-column ps-4">
                            <li><a class="nav-link" href="trip_tickets.php"><i class="bi bi-ticket-perforated"></i><span class="nav-text">Trip Tickets</span></a></li>
                        </ul>
                    </div>
                </li>
                <!-- Banking Dropdown -->
...
<li class="nav-item dropdown-nav">
    ...
    <div class="main-content" id="mainContent">
        ...
        <div class="row stat-card-row g-2 g-md-3 mb-4">
            <div class="col-12 col-sm-6 col-lg-3"><div class="stat-card total h-100"><i class="bi bi-cash-stack stat-icon"></i><div class="stat-content"><div class="stat-value">₱<?= number_format($total_receivables, 2) ?></div><div class="stat-label">Total Receivables</div><small class="d-block">Unpaid Receivables</small></div></div></div>
...
<?php endif; ?>
                    ...
                    <tbody id="invoicesTableBody">
                        ...
                        <?php else: ?>
                            <?php foreach ($default_invoices as $invoice): 
                                ...
                                $paymentButton = $orderStatusLower === 'delivered' ? '<div class="invoice-action-buttons"><button class="btn-pay btn-action-icon" title="Record Payment" aria-label="Record Payment" onclick="event.stopPropagation(); openPaymentModal(' . $invoice['invoice_id'] . ', \'' . addslashes($invoice['invoice_number']) . '\', ' . ($invoice['total_amount'] ?? 0) . ')"><i class="bi bi-cash-stack"></i></button></div>' : ($orderStatusLower === 'confirmed' ? '<span class="text-muted small">Await Delivery</span>' : '<span class="text-muted small">Not Ready</span>');
    ...
    <div class="form-text" id="bbDocumentDigitsHelp">Last 5 to 6 digits only.</div>
                            ...
                            <div class="col-md-12">
                                <label class="form-label fw-bold">Attachments <span class="text-danger">*</span></label>
                                <div id="bbAttachmentsContainer" class="d-flex flex-column gap-2">
                                    <div class="bb-attachment-row border rounded p-2 bg-light">
                                        <div class="input-group">
                                            <input type="file" class="form-control bb-attachment-input" name="attachments[]" accept=".jpg,.jpeg,.png,.gif,.webp,.pdf,.doc,.docx,.xls,.xlsx,.csv,.txt" required>
                                ...
                                <div class="form-text">Required. Allowed: images, PDF, Word, Excel, CSV, TXT. Max 10MB each file.</div>
                            </div>
                            <div class="col-md-12">
...
function loadSpecificCustomerInvoices(customerId, dateFrom = '', dateTo = '') {
    ...
    fetch(window.location.href, { method: 'POST', body: formData }).then(async response => { const text = await response.text(); try { return JSON.parse(text); } catch (e) { console.error('Invalid JSON response:', text.substring(0, 500)); throw new Error('Invalid server response'); } }).then(data => { Swal.close(); if (data.success) { currentInvoices = data.invoices || []; currentCustomerGroupFilter = 'all'; buildCustomerGroupTabs(currentInvoices); renderCustomerInvoicesTable(getFilteredInvoicesByCustomerGroup(currentInvoices)); updateCreditSummary(data.credit_limit, data.credit_used); clearAllSelections(); } else Swal.fire('Error', data.message || 'Failed to load invoices', 'error'); }).catch(error => { Swal.close(); console.error(error); Swal.fire('Error', error.message || 'Failed to load invoices', 'error'); });
}
...
function buildInvoiceActionButtons(inv) {
    const orderStatusLower = String(inv.order_status || '').toLowerCase();
...
function createBeginningBalanceAttachmentRow(showRemove = true) {
    ...
    row.innerHTML = `
        <div class="input-group">
            <input type="file" class="form-control bb-attachment-input" name="attachments[]" accept=".jpg,.jpeg,.png,.gif,.webp,.pdf,.doc,.docx,.xls,.xlsx,.csv,.txt" ${showRemove ? '' : 'required'}>
...
function saveBeginningBalance(closeAfterSave = false) {
    ...
    const hasAttachment = Array.from(attachmentInputs).some(input => input.files && input.files.length > 0);
    if (!isEditMode && !hasAttachment) { Swal.fire('Required', 'Please upload at least one attachment.', 'warning'); return; }
