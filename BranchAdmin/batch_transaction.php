...
// Prevent browser/form cache from restoring stale Select2/dropdown state after saving.
// This avoids the issue where the Transaction Type display and the opened dropdown
// become out of sync until a hard refresh is done.
if (!headers_sent()) {
...
$view_all_brancher = $_SESSION['view_all_branhes'] ?? false;
...
function batch_post_chart_entry($conn, $account, $branchId, $transactionDate, $transactionType, $transactionNo, $referenceNo, $memo, $sourceTable, $sourceId, $debit, $credit, $createdBy) {
    ...
    $insert->bind_param('iissssssdddsii', $accountId, $branchId, $transactionDateOnly, $transactionType, $transactionNo, $referenceNo, $memo, $accountName, $debit, $credit, $balanceAfter, $sourceTable, $sourceId, $createdBy);
...
function batch_save_check_like_withdrawal($conn, $branchId, $userId, $bankAccountId, $row, $accountLabel = '') {
    ...
    // Same accounting effect as Write Check in Withdrawal.php:
    // Credit Bank, Debit selected expense/account.
    batch_post_chart_entry($conn, $bankAccount, $branchId, $transactionDate, 'Check', $transactionNo, $referenceNumber, $memo, 'bank_transactions', $transactionId, 0.00, $amount, $userId);
    batch_post_chart_entry($conn, $expenseAccount, $branchId, $transactionDate, 'Check', $transactionNo, $referenceNumber, $memo, 'bank_transactions', $transactionId, $amount, 0.00, $userId);
...
<head>
    ...
    <title>Batch Transaction - Branch Admin</title>
    ...
    <style>
       ...
       .batch-grid-box {
        ...
        /* Hide the browser placeholder text like mm/dd/yyyy while keeping the calendar button visible. */
        ...
        /* Select2 dropdown styling para hindi na blue ang hover/selected item */
...
function applyResponsiveTableWidth(headers) {
    ...
    /*
        Keep the Batch Transaction grid fitted inside the main content.
        Customize Columns can add more fields, so we use weighted percentage
        widths instead of fixed pixel widths. This prevents right-side scrolling.
    */
...
function buildBatchRowHtml(headers, config, rowIndex = 0) {
    ...
    const cells = headers.map((header) => {
        ...
        if (selectedType === 'checks' && header === 'Payee') {
            return `<td>
                <input type="text" class="payee-search-input" autocomplete="off" onfocus="showPayeeDropdown(this)" oninput="showPayeeDropdown(this); handleBatchCellInput(this)" onchange="handleBatchCellInput(this)">
            </td>`;
        }

        if ((selectedType === 'bills' || selectedType === 'credit_card') && header === 'Vendor') {
            return `<td>
                <input type="text" class="vendor-search-input" autocomplete="off" onfocus="showVendorDropdown(this)" oninput="showVendorDropdown(this); handleBatchCellInput(this)" onchange="handleBatchCellInput(this)">
            </td>`;
        }

        if (selectedType === 'invoices' && header === 'Customer') {
            return `<td>
                <input type="text" class="customer-search-input" autocomplete="off" onfocus="showCustomerDropdown(this)" oninput="showCustomerDropdown(this); handleBatchCellInput(this)" onchange="handleBatchCellInput(this)">
            </td>`;
        }

        if (selectedType === 'invoices' && header === 'Item') {
            return `<td>
                <input type="text" class="item-search-input" autocomplete="off" onfocus="showItemDropdown(this)" oninput="showItemDropdown(this); handleBatchCellInput(this)" onchange="syncInvoiceItemDependentFields(this, true); handleBatchCellInput(this)">
            </td>`;
        }

        if (['Account', 'Account From'].includes(header)) {
            return `<td>
                <input type="text" class="account-search-input" autocomplete="off" onfocus="showBatchAccountDropdown(this)" oninput="showBatchAccountDropdown(this); handleBatchCellInput(this)" onchange="handleBatchCellInput(this)">
            </td>`;
        }
