...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    try {
        ...
        if ($action === 'save_employee') {
            ...
            if ($employee_id > 0) {
                $sql = "UPDATE employees SET employee_name=?, contact_number=?, email=?, birthday=?, tin=?, philhealth=?, sss=?, pagibig=?, status=?, updated_at=NOW() WHERE employee_id=?";
                ...
                $stmt = $conn->prepare("INSERT INTO employees (branch_id, employee_name, contact_number, email, birthday, tin, philhealth, sss, pagibig, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())");
        ...
        if ($action === 'save_dtr') {
            ...
            foreach ($dtrEntries as $entry) {
                ...
                if (!$view_all_branches) $preserveSql .= " AND branch_id=" . (int)$branch_id;
                ...
                if (!$view_all_branches) $deleteSql .= " AND branch_id=" . (int)$branch_id;
                ...
                foreach ($pairs as $pair) {
                    if ($pair['out'] === '') {
                        ...
                        $stmt = $conn->prepare("INSERT INTO employee_dtr (employee_id, branch_id, attendance_date, start_time, end_time, duration_minutes, holiday_type, holiday_name, scheduled_work_minutes, regular_minutes, overtime_minutes, ot_requested_minutes, ot_approved_minutes, ot_approval_status, monthly_rate, daily_rate, hourly_rate, basic_pay, overtime_pay, total_pay, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())");
                ...
                if (in_array($preservedOtStatus, ['approved', 'rejected'], true)) {
                    $newOtSql = "SELECT dtr_id, ot_requested_minutes, hourly_rate, basic_pay, holiday_type
                                 ...
                                 WHERE employee_id=? AND attendance_date=?
                                   AND COALESCE(payroll_submitted,0)=0
                                   AND COALESCE(ot_requested_minutes,0)>0";
                    if (!$view_all_branches) $newOtSql .= " AND branch_id=" . (int)$branch_id;
        ...
        if ($action === 'approve_ot') {
            ...
            if (!$view_all_branches) $fetchSql .= " AND branch_id=" . (int)$branch_id;
        ...
        if ($action === 'submit_for_payroll') {
            ...
            $conn->begin_transaction();
            try {
                $insert = $conn->prepare("INSERT INTO employee_payroll_submissions (branch_id, submitted_by, submitted_by_name, submitted_at, notes) VALUES (?, ?, ?, NOW(), ?)");
...
<title>Employee Attendance - Branch Admin</title>
...
/* =========================================================
   Badge overlap fix - keeps existing colors/UI, only adjusts
   badge size, spacing, wrapping, and row vertical alignment.
========================================================= */
.user-table tbody td,
.employee-table tbody td,
.dtr-qb-sheet tbody td,
.table tbody td {
    vertical-align: middle !important;
    overflow: visible !important;
    height: auto !important;
...
.user-table .branch-badge,
.employee-table .badge,
.employee-table .status-badge,
.employee-table .duration-badge,
.employee-table .role-badge,
.employee-table .branch-badge,
.dtr-qb-sheet .badge,
.dtr-qb-sheet .status-badge,
.dtr-qb-sheet .duration-badge,
.table .badge,
.table .status-badge,
...
.user-table td .branch-badge,
.employee-table td .badge,
.employee-table td .status-badge,
.employee-table td .duration-badge,
.employee-table td .role-badge,
.employee-table td .branch-badge,
.dtr-qb-sheet td .badge,
.dtr-qb-sheet td .status-badge,
.dtr-qb-sheet td .duration-badge,
.table td .badge,
.table td .status-badge,
.table td .duration-badge {
...
/* =========================================================
   FINAL BADGE OVERLAP FIX
   Target actual badge classes used in this page:
   - .dtr-qb-status
   - .duration-badge
   - .status-badge / .role-badge / .branch-badge
   This only changes badge size, wrapping, and table row spacing.
========================================================= */
.dtr-qb-sheet tbody tr,
.employee-table tbody tr,
.user-table tbody tr,
.table tbody tr {
    height: auto !important;
...
/* Main badges used in the DTR rows */
.dtr-qb-status,
.duration-badge,
.status-badge,
.role-badge,
...
/* Status badges had a fixed 82px min-width causing overlap on narrow cells */
.dtr-qb-status {
    min-width: 0 !important;
    gap: 3px !important;
}

/* Duration badge had bigger padding in earlier CSS */
.duration-badge {
    gap: 3px !important;
    padding: 2px 6px !important;
}

/* Keep multiple badges inside cells from sitting on top of each other */
.dtr-qb-sheet td,
.employee-table td,
.user-table td,
.table td {
...
/* Better spacing for stacked time values so badges do not touch next row */
.time-stack {
    gap: 3px !important;
    line-height: 1.15 !important;
}
...
/* Mobile/table-card layout: keep badges compact on small widths */
@media (max-width: 768px) {
    .dtr-qb-status,
    .duration-badge,
    .status-badge,
...
<div class="mobile-nav" id="mobileNav">
    <ul class="nav">
        ...
        <li class="nav-item"><a href="employeelist.php" class="nav-link"><i class="bi bi-people-fill"></i><span>Employee<br>List</span></a></li>
        <li class="nav-item"><a href="employee.php" class="nav-link active"><i class="bi bi-clock-history"></i><span>Employee<br>Attendance</span></a></li>
