        ...
        if (!$fromAccount) {
            ...
            if ($error_message === '') {
                ...
                try {
                    ...
                    $_SESSION['transfer_success_redirect'] = ($saveAction === 'close') ? 'branchdashboard.php' : 'transferfunds.php';
...
<head>
    ...
    <title>Transfer Funds - Branch Admin</title>
...
<div class="sidebar" id="sidebar">
                ...
                <li class="nav-item dropdown-nav">
                    ...
                    <div class="collapse" id="bankingMenu">
                        <ul class="nav flex-column ps-4">
                            ...
                            <li class="nav-item">
                                <a class="nav-link active" href="transferfunds.php">
    ...
    <script>
        ...
        function applySidebarState() {
            ...
            openActiveSidebarDropdown();
        }

        function openActiveSidebarDropdown() {
            document.querySelectorAll('.sidebar .nav-link.active').forEach(activeLink => {
                const activeMenu = activeLink.closest('.collapse');
                if (!activeMenu) return;

                activeMenu.classList.add('show');

                const parentToggle = document.querySelector(`[onclick*="${activeMenu.id}"]`);
                if (parentToggle) {
                    parentToggle.classList.add('active-parent');
                }

                const arrow = parentToggle ? parentToggle.querySelector('.dropdown-arrow') : null;
                if (arrow) {
        ...
        function toggleSidebarDropdown(event, menuId) {
            ...
            document.querySelectorAll('.sidebar .collapse.show').forEach(openMenu => {
                if (openMenu.id !== menuId) {
                    const openCollapse = bootstrap.Collapse.getOrCreateInstance(openMenu, {toggle:false});
                    openCollapse.hide();

                    const openToggle = document.querySelector(`[onclick*="${openMenu.id}"]`);
                    const openArrow = openToggle ? openToggle.querySelector('.dropdown-arrow') : null;
                    if (openToggle) openToggle.classList.remove('active-parent');
                    if (openArrow) openArrow.style.transform = 'translateY(-50%) rotate(0deg)';
                }
            });

            const bsCollapse = bootstrap.Collapse.getOrCreateInstance(menu, {toggle:false});
            const toggleBtn = event.currentTarget;
            const arrow = toggleBtn ? toggleBtn.querySelector('.dropdown-arrow') : null;

            if (menu.classList.contains('show')) {
                bsCollapse.hide();
                toggleBtn?.classList.remove('active-parent');
