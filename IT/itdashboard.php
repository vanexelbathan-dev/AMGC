...
?>
<?php
if (!in_array($user_role, ['it', 'admin', 'super_duper_admin'], true)) {
    header('Location: ../login.php');
    exit;
}
$success_message = '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'update_request') {
    try {
        ...
        if (!in_array($status, ['pending', 'in_progress', 'resolved', 'cancelled'], true))
            $status = 'pending';
        ...
        if (in_array((string) ($old_request['status'] ?? ''), ['resolved', 'cancelled'], true)) {
            throw new Exception('Resolved or cancelled requests can no longer be edited.');
        }
        ...
        if ($it_notes !== trim((string) ($old_request['it_notes'] ?? '')))
            $history_notes .= ($history_notes ? '; ' : '') . 'IT notes updated';
...
$params = [];
$types = '';
if ($status_filter !== '' && in_array($status_filter, ['pending', 'in_progress', 'resolved', 'cancelled'], true)) {
    $where[] = 'ir.status=?';
...
if ($source_filter !== '' && in_array($source_filter, ['branch_admin', 'motorpool'], true)) {
    $where[] = 'ir.source_account=?';
...
if (tableExists($conn, 'branches')) {
    $res = $conn->query("SELECT branch_id, branch_name FROM branches ORDER BY branch_name ASC");
    if ($res) {
        while ($row = $res->fetch_assoc())
            $branches[] = $row;
    }
...
<head>
    ...
    <style>
        ...
        /* Request History Table - no horizontal scroll, readable text */
        .request-system-modal .table-responsive {
            overflow-x: hidden !important;
            width: 100% !important;
        }

        .request-system-modal .table {
            width: 100% !important;
            table-layout: fixed !important;
            margin-bottom: 0 !important;
        }

        .request-system-modal .table thead th {
            background: #047857 !important;
            color: #fff !important;
        ...
        .request-system-modal .table tbody td {
        ...
        .request-system-modal .table th:nth-child(1),
        .request-system-modal .table td:nth-child(1) {
            width: 18% !important;
            white-space: normal !important;
        }

        .request-system-modal .table th:nth-child(2),
        .request-system-modal .table td:nth-child(2) {
            width: 15% !important;
            white-space: normal !important;
            font-weight: 700 !important;
        }

        .request-system-modal .table th:nth-child(3),
        .request-system-modal .table td:nth-child(3) {
            width: 16% !important;
            white-space: nowrap !important;
        }

        .request-system-modal .table th:nth-child(4),
        .request-system-modal .table td:nth-child(4) {
            width: 16% !important;
            white-space: nowrap !important;
        }

        .request-system-modal .table th:nth-child(5),
        .request-system-modal .table td:nth-child(5) {
            width: 17% !important;
            white-space: normal !important;
        }

        .request-system-modal .table th:nth-child(6),
        .request-system-modal .table td:nth-child(6) {
            width: 18% !important;
            white-space: normal !important;
        ...
        .attachment-viewer-modal .btn-close {
            filter: brightness(0) invert(1) !important;
            opacity: .9 !important;
        }
        ...
        .attachment-viewer-modal+.modal-backdrop {
    ...
    <div id="appPage">
        ...
        <div class="main-content" id="mainContent">
                ...
                <div class="mb-4">
                    <div class="tab-card">
                        <div class="payroll-tab-toolbar">
                            <div><strong><i class="bi bi-funnel me-2"></i>Filters</strong></div>
                        </div>
                        <form method="GET" class="row g-3 align-items-end">
                            <div class="col-lg-3"><label class="form-label">Status</label><select name="status"
                                    class="form-select">
                                    ...
                                    <option value="in_progress" <?= $status_filter === 'in_progress' ? 'selected' : '' ?>>In
                                        Progress</option>
                                    <option value="resolved" <?= $status_filter === 'resolved' ? 'selected' : '' ?>>Resolved
                                    </option>
                                    <option value="cancelled" <?= $status_filter === 'cancelled' ? 'selected' : '' ?>>Cancelled
                                    </option>
                                </select></div>
                            <div class="col-lg-3"><label class="form-label">Branch</label><select name="branch_id"
                                    class="form-select">
                                    <option value="0">All Branches</option><?php foreach ($branches as $branch): ?>
                                        <option value="<?= (int) $branch['branch_id'] ?>"
                                            <?= $branch_filter === (int) $branch['branch_id'] ? 'selected' : '' ?>>
                                            <?= h($branch['branch_name']) ?></option><?php endforeach; ?>
                                </select></div>
                            <div class="col-lg-3"><label class="form-label">Source</label><select name="source"
                                    class="form-select">
                                    <option value="">All Sources</option>
                                    <option value="branch_admin" <?= $source_filter === 'branch_admin' ? 'selected' : '' ?>>
                                        Branch Admin</option>
                                    <option value="motorpool" <?= $source_filter === 'motorpool' ? 'selected' : '' ?>>Motorpool
                                    </option>
                                </select></div>
                            <div class="col-lg-3 d-flex gap-2"><button type="submit"
                                    class="btn btn-success flex-fill"><i class="bi bi-funnel me-1"></i>Apply</button><a
                                    href="itdashboard.php" class="btn btn-payroll-light"><i
                                    ...
                                    <?php else:
                                        foreach ($requests as $request): ?>
                                            <tr class="clickable-request-row" data-bs-toggle="modal"
                                                ...
                                                <td><?= h($request['branch_name'] ?: ('Branch #' . (int) $request['branch_id'])) ?>
                                                </td>
                                                <td><?= h($request['source_account'] === 'motorpool' ? 'Motorpool' : 'Branch Admin') ?>
                                                </td>
                                                ...
                                                <td><?php $ac = count($attachments_by_request[(int) $request['request_id']] ?? []); ?><span
                                                        class="it-badge <?= $ac > 0 ? 'medium' : 'low' ?>"><i
                                                            class="bi bi-paperclip me-1"></i><?= $ac ?></span></td>
            ...
            <?php foreach ($requests as $request): ?>
                <?php $is_locked_request = in_array((string) ($request['status'] ?? ''), ['resolved', 'cancelled'], true); ?>
                            ...
                            <form method="POST" class="sweet-submit-form">
                                ...
                                <div class="modal-header">
                                    <div>
                                        ...
                                        <small class="text-muted">Requested by
                                            <?= h(trim((string) ($request['requested_by'] ?? '')) ?: 'User') ?> •
                                    ...
                                    <div class="row g-4">
                                        <div class="col-lg-8">
                                                ...
                                                <?php else: ?>
                                                    <div class="list-group">
                                                        <?php foreach ($files as $file): ?>
                                                            <button type="button"
                                                                class="list-group-item list-group-item-action attachment-open-btn d-flex justify-content-between align-items-center"
                                                                data-file-url="<?= h($file['file_path'] ?? '#') ?>"
                                                                data-file-name="<?= h($file['original_name'] ?? 'Attachment') ?>"
                                                                data-file-mime="<?= h($file['mime_type'] ?? '') ?>">
                                                                <span><i
                                            ...
                                            <div class="tab-card mb-0">
                                                <strong class="d-block mb-3"><i class="bi bi-clock-history me-1"></i>Request
                                                    History</strong>
                                                            ...
                                                            <tbody>
                                                                <?php foreach ($histories as $history): ?>
                                                                    <tr>
                                                                        ...
                                                                        <td><?= h(($history['priority_from'] ?: '-') . ' → ' . ($history['priority_to'] ?: '-')) ?>
                                                                        </td>
                                        ...
                                        <div class="col-lg-4">
                                            <?php if ($is_locked_request): ?>
                                                <div class="alert alert-light border mb-3" style="border-radius:12px;color:#052A47;">
                                                    <i class="bi bi-lock-fill me-1 text-success"></i>
                                                    This request is already <?= h(str_replace('_', ' ', $request['status'] ?? '')) ?> and can no longer be edited.
                                                </div>
                                            <?php endif; ?>
                                            <div class="row g-3">
                                                <div class="col-md-6"><label class="form-label">Status</label><select
                                                        name="status" class="form-select" <?= $is_locked_request ? 'disabled' : '' ?>>
                                                        <option value="pending" <?= ($request['status'] ?? '') === 'pending' ? 'selected' : '' ?>>Pending</option>
                                                        <option value="in_progress" <?= ($request['status'] ?? '') === 'in_progress' ? 'selected' : '' ?>>In Progress</option>
                                                        <option value="resolved" <?= ($request['status'] ?? '') === 'resolved' ? 'selected' : '' ?>>Resolved</option>
                                                        <option value="cancelled" <?= ($request['status'] ?? '') === 'cancelled' ? 'selected' : '' ?>>Cancelled</option>
                                                    </select></div>
                                                <div class="col-md-6"><label class="form-label">Priority</label><select
                                                        name="priority" class="form-select" <?= $is_locked_request ? 'disabled' : '' ?>>
                                                        <option value="low" <?= ($request['priority'] ?? '') === 'low' ? 'selected' : '' ?>>Low</option>
                                                        <option value="medium" <?= ($request['priority'] ?? '') === 'medium' ? 'selected' : '' ?>>Medium</option>
                                                        <option value="high" <?= ($request['priority'] ?? '') === 'high' ? 'selected' : '' ?>>High</option>
                                                        <option value="urgent" <?= ($request['priority'] ?? '') === 'urgent' ? 'selected' : '' ?>>Urgent</option>
                                                    </select></div>
                                                <div class="col-12"><label class="form-label">Resolved At</label><input
                                                        type="text" class="form-control"
                                                        value="<?= !empty($request['resolved_at']) ? h(date('M d, Y h:i A', strtotime($request['resolved_at']))) : 'Not resolved yet' ?>"
                                                        readonly></div>
                                                <div class="col-12"><label class="form-label">Last Updated By</label><input
                                                        type="text" class="form-control"
                                                        value="<?= h(trim((string) ($request['handled_by'] ?? '')) ?: '-') ?>"
                                                        readonly></div>
                                                <div class="col-12"><label class="form-label">IT Notes</label><textarea
                                                        name="it_notes" class="form-control"
                                                        rows="8" <?= $is_locked_request ? 'readonly' : '' ?>><?= h($request['it_notes'] ?? '') ?></textarea></div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="modal-footer">
                                    <?php if (!$is_locked_request): ?>
                                        ...
                                        <button type="submit" class="btn btn-payroll-green"><i class="bi bi-save me-1"></i>Save Update</button>
                                    <?php else: ?>
                    ...
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title"><i class="bi bi-paperclip me-2"></i><span
                                    id="attachmentViewerTitle">Attachment</span></h5>
                        ...
                        <div class="modal-footer">
                            <a href="#" id="attachmentViewerDownload" class="btn btn-payroll-green" download><i
